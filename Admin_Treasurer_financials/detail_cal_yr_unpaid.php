...
$query_inv_det = sprintf("SELECT * FROM inv_to WHERE inv_cal_year = %s AND inv_status<> 'Paid' ORDER BY {$sqlorderby_inv_det}", GetSQLValueString($colname_inv_det, "date"));
...
$query_inv_det = sprintf("SELECT * FROM inv_to WHERE inv_cal_year = %s AND inv_status<> 'Paid' AND club_id is null ORDER BY {$sqlorderby_inv_det}", GetSQLValueString($colname_inv_det, "date"));
$inv_det = mysql_query($query_inv_det, $connvbsa) or die(mysql_error());
$row_inv_det = mysql_fetch_assoc($inv_det);
$totalRows_inv_det = mysql_num_rows($inv_det);
...
<table width="1000" align="center" cellpadding="5" cellspacing="5">
  <tr>
    <td align="center" class="red_bold"><?php echo $row_fin_cal_year['cal_year']; ?> OUTSTANDING CALENDAR YEAR INVOICES (Not clubs)</td>
