...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE SBSA SET pdf_name=IFNULL(%s,pdf_name) WHERE SBSA_id=%s",
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_pdfup = "SELECT SBSA_id, SBSA_type, pdf_name, SBSA.item_title FROM SBSA WHERE SBSA_id = '$pdfup'";
...
<form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1" onsubmit="checkFileUpload(this,'',true,500,'','','','','','');return document.MM_returnValue"><table width="940" align="center" cellpadding="5" cellspacing="5">
  ...
  <tr>
    <td colspan="2" align="left"><span class="red_bold">SBSA - Upload a .pdf file (Maximum File size 500kb)</span></td>
  ...
  <tr>
    ...
    <td colspan="2">
      <?php 
	  if ($row_pdfup['SBSA_type'] =="a_info")
	  {echo "Information";}
	  elseif ($row_pdfup['SBSA_type'] =="c_zone1") 
      {echo "Zone 1";}
	  elseif ($row_pdfup['SBSA_type'] =="d_zone2") 
      {echo "Zone 2";}
	  elseif ($row_pdfup['SBSA_type'] =="e_zone3") 
      {echo "Zone 3";}
	  elseif ($row_pdfup['SBSA_type'] =="f_zone4") 
      {echo "Zone 4";}
	  elseif ($row_pdfup['SBSA_type'] =="g_zone5") 
      {echo "Zone 5";}
	  elseif ($row_pdfup['SBSA_type'] =="f_history") 
      {echo "History";}	  
	  ?></td>
    </tr>
  <tr>
  ...
  <input type="hidden" name="SBSA_id" value="<?php echo $row_pdfup['SBSA_id']; ?>" />
