...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form2")) {
  $updateSQL = sprintf("UPDATE DVSA SET item_title=%s, current=%s, list_order=%s, edited_on=%s, news_content=%s WHERE DVSA_id=%s",
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_DVSAedit = "SELECT date_format(uploaded_on,'%D %M %Y %H:%i') AS uploaded_on_date, date_format(edited_on,'%D %M %Y %H:%i') AS edited_on_date, DVSA_id, DVSA_type, item_title, list_order, news_content, pdf_name, edited_on, current FROM DVSA WHERE DVSA_id = '$news'";
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form2" id="form2">
  <table align="center" cellpadding="5" cellspacing="5">
    ...
    <tr valign="baseline">
      ...
      <td><?php echo $row_DVSAedit['DVSA_id']; ?></td>
    </tr>
    <tr valign="baseline">
    ...
    <tr valign="baseline">
      ...
      <td><textarea name="news_content" cols="80" rows="5"><?php echo htmlentities($row_DVSAedit['news_content'], ENT_COMPAT, 'utf-8'); ?></textarea></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Visible: </td>
      <td><input type="checkbox" name="current" id="current"  <?php if (!(strcmp(htmlentities($row_DVSAedit['current'], ENT_COMPAT, 'utf-8'),"1"))) {echo "checked=\"checked\"";} ?> />
(if unchecked item will not appear on the site and will be &quot;Archived&quot;)</td>
    </tr>
    <tr valign="baseline">
      <td align="right" nowrap="nowrap">Uploaded on :</td>
      <td><?php echo $row_DVSAedit['uploaded_on_date']; ?></td>
    </tr>
    <tr valign="baseline">
      <td align="right" nowrap="nowrap">Edited on</td>
      <td><?php echo $row_DVSAedit['edited_on_date']; ?> (Will only show if an item has been edited)</td>
    </tr>
    <tr valign="baseline">
