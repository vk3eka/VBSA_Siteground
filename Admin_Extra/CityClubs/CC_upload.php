...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE CC SET pdf_name=IFNULL(%s,pdf_name) WHERE CC_id=%s",
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_pdfup = "SELECT CC_id, CC_type, pdf_name, CC.item_title FROM CC WHERE CC_id = '$pdfup'";
...
<form action="<?php echo $editFormAction; ?>" method="post" enctype="multipart/form-data" name="form1" id="form1" onsubmit="checkFileUpload(this,'',true,500,'','','','','','');return document.MM_returnValue"><table width="940" align="center" cellpadding="5" cellspacing="5">
  ...
  <tr>
    <td colspan="2" align="left"><span class="red_bold">City Clubs - Upload a .pdf file (Maximum File size 500kb)</span></td>
  ...
  <tr>
    ...
    <td colspan="2">
      <?php 
	  if ($row_pdfup['CC_type'] =="a_info")
	  {echo "Information";}
	  elseif ($row_pdfup['CC_type'] =="c_zone1") 
      {echo "Zone 1";}
	  elseif ($row_pdfup['CC_type'] =="d_zone2") 
      {echo "Zone 2";}
	  elseif ($row_pdfup['CC_type'] =="e_zone3") 
      {echo "Zone 3";}
	  elseif ($row_pdfup['CC_type'] =="f_zone4") 
      {echo "Zone 4";}
	  elseif ($row_pdfup['CC_type'] =="g_zone5") 
      {echo "Zone 5";}
	  elseif ($row_pdfup['CC_type'] =="f_history") 
      {echo "History";}	  
	  ?></td>
    </tr>
  <tr>
  ...
  <input type="hidden" name="CC_id" value="<?php echo $row_pdfup['CC_id']; ?>" />
