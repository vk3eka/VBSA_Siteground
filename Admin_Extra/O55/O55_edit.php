...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE O55 SET O55_type=%s, item_title=%s, current=%s, list_order=%s, edited_on=%s WHERE O55_id=%s",
                       GetSQLValueString($_POST['O55_type'], "text"),
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_O55edit = "SELECT date_format(uploaded_on,'%D %M %Y %H:%i') AS uploaded_on_date, date_format(edited_on,'%D %M %Y %H:%i') AS edited_on_date, O55_id, O55_type, item_title, list_order, news_content, pdf_name, edited_on, current FROM O55 WHERE O55_id = '$zoned'";
...
<form action="<?php echo $editFormAction; ?>" method="post" name="form1" id="form1">
  <table width="909" align="center" cellpadding="5" cellspacing="5">
    ...
    <tr valign="baseline">
      ...
      <td colspan="2"><select name="O55_type">
        <option value="a_info" <?php if (!(strcmp("a_info", htmlentities($row_O55edit['O55_type'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>Information</option>
        <option value="b_news" <?php if (!(strcmp("b_news", htmlentities($row_O55edit['O55_type'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>News Item</option>
        <option value="c_zone1" <?php if (!(strcmp("c_zone1", htmlentities($row_O55edit['O55_type'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>Zone 1</option>
        <option value="d_zone2" <?php if (!(strcmp("d_zone2", htmlentities($row_O55edit['O55_type'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>Zone 2</option>
        <option value="e_zone3" <?php if (!(strcmp("e_zone3", htmlentities($row_O55edit['O55_type'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>Zone 3</option>
        <option value="f_zone4" <?php if (!(strcmp("f_zone4", htmlentities($row_O55edit['O55_type'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>Zone 4</option>
        <option value="g_zone5" <?php if (!(strcmp("g_zone5", htmlentities($row_O55edit['O55_type'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>Zone 5</option>
        <option value="f_history" <?php if (!(strcmp("f_history", htmlentities($row_O55edit['O55_type'], ENT_COMPAT, 'utf-8')))) {echo "SELECTED";} ?>>History</option>
      </select>        
    ...
    <tr valign="baseline">
      ...
      <td colspan="2"><input type="checkbox" name="current" id="current"  <?php if (!(strcmp(htmlentities($row_O55edit['current'], ENT_COMPAT, 'utf-8'),"1"))) {echo "checked=\"checked\"";} ?> />
      (if unchecked item will not appear on the site and will be &quot;Archived&quot;)</td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Uploaded on :</td>
      <td colspan="2"> <?php echo $row_O55edit['uploaded_on_date']; ?></td>
    </tr>
    <tr valign="baseline">
      <td nowrap="nowrap" align="right">Edited on</td>
      <td colspan="2"><?php echo $row_O55edit['edited_on_date']; ?> (Will only show if an item has been edited)</td>
    </tr>
    <tr valign="baseline">
