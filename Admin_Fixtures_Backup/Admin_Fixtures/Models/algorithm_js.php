...
function show_fixtures(names, team_grade, form_no, year, season) {
    ...
    for (let i = 0; i < rounds.length; i++) {
        text += "\nRound " + (i + 1) + "\n";
        text += "date => (A_" + form_no + "_date_" + i + ")\n";
        rounds[i].forEach((r, x) => {
            let [home, away] = r.split(" v ");
            text += "A_" + team_grade + "_home_" + (i + 1) + "_" + (x + 1) + " = " + home + "\n";
            text += "A_" + team_grade + "_away_" + (i + 1) + "_" + (x + 1) + " = " + away + "\n";
        });
    }
    ...
    for (let b = 0; b < rounds.length; b++) {
        text += "\nRound " + round_counter + "\n";
        text += "A_" + form_no + "_date_" + (round_counter - 1) + "\n";
        rounds[b].forEach((r, y) => {
            let [home, away] = r.split(" v ");
            text += "A_" + team_grade + "_home_" + round_counter + "_" + (y + 1) + " = " + away + "\n";
            text += "A_" + team_grade + "_away_" + round_counter + "_" + (y + 1) + " = " + home + "\n";
        });
        round_counter++;
    }

    // Set final output in textarea
    $("#output").val(text);
}
