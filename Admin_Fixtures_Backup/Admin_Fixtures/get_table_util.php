...
function GetAnalyisData($team_grades, $year, $season, $dayplayed, $max_no_of_teams) {
    ...
    for($i = 0; $i < $max_no_of_teams; $i++)
    {
        $analysis_output .= ("<td align='center' id='date_" . $i . "'></td>");
    }
    $analysis_output .= ('</tr>');
...
for($i = 0; $i < $max_no_of_teams; $i++)
{
    ...
    $sql_playing_dates = 'Select * from tbl_create_fixtures where year = ' . $year . ' and season = "' . $season . '" and round = ' . ($i+1);
    ...
    //{
    $row = $result_playing_dates->fetch_assoc();
    $row_date = $row['date'];
    //$analysis_output .= ("document.getElementById('A_" . $form_no . "_date_" . $i . "').value = '" . $row_date . "';");
    //$analysis_output .= ("document.getElementById('B_" . $form_no . "_date_" . $i . "').value = '" . $row_date . "';");
    $output .=  ("<td align='center' id='table_date_" . $i . "'>" . $row_date . "</td>");
    //}
    //echo("</script>");
...
$output .=  ("<tr>");
$output .=  ("<td colspan=22 align='center'>Number of Clashes&nbsp;&nbsp;<input type='text' id='no_of_clashes' value='" . $clashes . "' style='width:50px'></td>");
$output .=  ("</tr>");
