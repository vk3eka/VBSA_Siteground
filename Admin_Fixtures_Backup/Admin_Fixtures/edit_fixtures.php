...
$sql = "Select grade_name, grade, type, dayplayed  FROM Team_grade where fix_cal_year = ". $current_year . " and season = '". $current_season . "' and current = 'Yes' Order By grade";
//echo($sql . "<br>");
$result_fixture = mysql_query($sql, $connvbsa) or die(mysql_error());
while($build_fixture = $result_fixture->fetch_assoc()) 
{
    echo("<option value='" . $build_fixture['grade'] . "'>" . $build_fixture['grade'] . " " . $build_fixture['grade_name'] . " (" . $build_fixture['dayplayed'] . ")</option>");
}
echo("</select></b>");
?>
        </td>
