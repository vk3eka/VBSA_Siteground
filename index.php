...
<?php } while ($row_Cal = mysql_fetch_assoc($Cal)); ?>
  ...
  <?php
		if(empty ($row_page_items['item_image']))
          ...
          <img class="img-responsive pull-right" style="padding-left:10px; padding-bottom:10px" src="images_frontpage/<?php echo $row_page_items['item_image']; ?>" width="<?php echo $row_page_items['img_size']; ?>" /> 
<?php } ?>
...
echo '<tr>'.'<td colspan="3" class="italic">'."Information".'</td>'.'</tr>'; 
{
	...
	$path_info_att = pathinfo($row_att['up_pdf_name']); 
   	if ($row_att['up_type'] == 'Attachment'):
		...
		// pdf viewer link
    echo '<td style="width:20%"><a href="../pdfjs/web/viewer.html?file=../../Front_page_upload/' . urlencode($row_att['up_pdf_name']) . '" title=View><span class="glyphicon glyphicon-eye-open"></span></a></td>';	
  	}
		else echo '<td nowrap="nowrap" style="width:60%">'.$row_att['up_desc'].'</td>'.'<td style="width:20%">&nbsp;</td>' ;
    // download link
    echo '<td style="width:20%"><a href="../Front_page_upload/' . urlencode($row_att["up_pdf_name"]) . '" download="../Front_page_upload/' . urlencode($row_att["up_pdf_name"]) . '" title=Download><span class="glyphicon glyphicon-download"></a></td>';
    echo '</tr>';
    //echo '<td style="width:20%"><a href="../pdfjs/web/viewer.html?file=../../Front_page_upload/' 
    //    . rawurlencode($row_att['up_pdf_name']) 
    //    . '" title="View"><span class="glyphicon glyphicon-eye-open"></span></a></td>';
		echo '</tr>';
	endif;	
...
echo '<tr>'.'<td colspan="3" class="italic">'."Information".'</td>'.'</tr>'; 
{
	while($row_event = mysql_fetch_array($result_event)):	   
   	if ($row_event['type'] == 'Uploaded Attachment'):
		...
		// pdf viewer link
    echo '<td style="width:20%"><a href="../pdfjs/web/viewer.html?file=../../calendar/cal_upload/' . urlencode($row_event['Attachment']) . '" title=View><span class="glyphicon glyphicon-eye-open"></span></a></td>'; 
		...
		// download link
		echo '<td style="width:20%"><a href=../calendar/cal_upload/' . $row_event['Attachment'] . ' download="../calendar/cal_upload/' . urlencode($row_event['Attachment']) . '"><span class="glyphicon glyphicon-download"></a></td>';
		echo '</tr>';
  	endif;		
