...
$query_RP_weekly = "Select ranknum, previous_rank, memb_id, FirstName, LastName, total_weekly_rp, date_format(last_update,'%D %b %Y') AS last_update FROM rank_S_open_weekly LEFT JOIN members ON MemberID = memb_id WHERE total_weekly_rp>0 ORDER BY ranknum";
...
<table class="table">
  <tr>
    ...
    <?php do { 
      $query_Previous = "Select previous_rank, memb_id, total_rp, date_format(last_update,'%D %b %Y') AS last_update FROM temp_weekly_snooker_ranking Where memb_id = " . $row_RP_weekly['memb_id'];
      $Previous = mysql_query($query_Previous, $connvbsa) or die(mysql_error());
      $row_Previous = mysql_fetch_assoc($Previous);
      if($row_RP_weekly['ranknum'] < $row_Previous['previous_rank'])
      {
        $arrow = "&nbsp;<font color='green'>&#8593;</font>";
      }
      else if($row_RP_weekly['ranknum'] > $row_Previous['previous_rank'])
      {
      ...
      ?>
  <tr>
    <td class="text-center"><?php echo ($row_RP_weekly['ranknum']); ?><?php echo($arrow); ?></td>
    <!--<td align="center" nowrap="nowrap"><?php echo $row_RP_weekly['ranknum']; ?></td>-->
    <td align="center" nowrap="nowrap"><?php echo $row_Previous['previous_rank']; ?></td>
