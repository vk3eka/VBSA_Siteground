...
$query_RPall = "Select 
  ...
  where Junior != 'na' and Gender != 'Female' ORDER BY (weekly_percent+total_tourn) DESC";
...
<div id="Wrapper"><!-- Wrapper contains content to a max width of 1200px--> 
  ...
  <div class="Page_heading_container">
 		<div class="page_title">Victorian Rankings - Snooker (Junior)</div>
  </div>  	
...
<div class="table-responsive center-block" style="max-width:250px" > 
  ...
  <tr>
    <td>
      <form method="get" action="rankings_vic_snooker_name_srch_res.php">
        <label class="glyphicon glyphicon-search" style="margin-right:3px"></label>
        <input type='hidden' name='cohort' id='cohort' value='junior'>
...
<div class="table-responsive center-block" style="max-width:500px; padding-left:3px">
   ...
   <table class="table">
      ...
      if(($row_RPall['weekly_percent']+$row_RPall['total_tourn']) == 0) 
      { 
      ...
      else
      {
        ...
        do { 
          ...
          if(($row_RPall['weekly_percent']+$row_RPall['total_tourn']) > 0)
          {
            $query_Previous = "Select previous_rank, memb_id, total_rp, date_format(last_update,'%D %b %Y') AS last_update FROM temp_junior_snooker_ranking Where memb_id = " . $row_RPall['memb_id'];
            $Previous = mysql_query($query_Previous, $connvbsa) or die(mysql_error());
            $row_Previous = mysql_fetch_assoc($Previous);
            if($rank_number < $row_Previous['previous_rank'])
            {
              $arrow = "&nbsp;<font color='green'>&#8593;</font>";
            }
            else if($rank_number > $row_Previous['previous_rank'])
            {
          ...
          ?>
        <tr>
          <td class="text-center"><?php echo ($rank_number); ?><?php echo($arrow); ?></td>
          ...
          <td class="text-center"><?php echo ($row_RPall['weekly_percent']+$row_RPall['total_tourn']); ?></td>
          <!--<td class="text-center"><?php echo $row_Previous['total_rp']; ?></td>-->
          <td class="text-center" nowrap="nowrap"><a href="rankings_vic_snooker_detail.php?rank=<?php echo $row_RPall['memb_id']; ?>&cohort=junior">How awarded</a></td>
        </tr>
        <?php } 
