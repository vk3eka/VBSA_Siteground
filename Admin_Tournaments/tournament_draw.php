...
function UploadDraw() {
    document.tournament_draw.TournamentID.value = '<?= $_GET['tourn_id'] ?>';
    //document.tournament_draw.action = "import_mapping.php?tourn_id=<?= $_GET['tourn_id'] ?>";
...
<form name='tournament_draw' id='tournament_draw' method="post" action='tournament_draw.php'>
...
if(isset($_GET['tourn_id']))
{
  ...
  // check if players already saved in the scoring table
  $query_import = 'Select * FROM vbsa3364_vbsa2.tournament_players where tourn_id = ' . $tournament_id;
  ...
  //echo("Query Scores " . $query_scores . "<br>");;
  ...
  //echo("Total Scores " . $total_scores . "<br>");;
  $total_players = 128;
  while($build_scores = $result_scores->fetch_assoc())
  {
    ...
    // addded 4/5/2025
    $best_of_128 = ($build_scores['best_of_128']);
    $best_of_64 = ($build_scores['best_of_64']);
    $best_of_32 = ($build_scores['best_of_32']);
    $best_of_16 = ($build_scores['best_of_16']);
    $best_of_8 = ($build_scores['best_of_8']);
    $best_of_4 = ($build_scores['best_of_4']);
    $best_of_2 = ($build_scores['best_of_2']);
  }
  ...
  //echo("console.log('SQL " . $query_score_table . "');");
  ...
  while($build_scores_table = $result_score_table->fetch_assoc())
  {
    if($total_players == 128)
    {
      if(($build_scores_table['match_index'] <= 128))
      {
        ...
        echo("document.getElementById('R" . $rounds . "_match_" . ($i+1) . "').value = '" . $build_scores_table['match_index'] . "';");
        $i++;
      }
      if(($build_scores_table['match_index'] > 128) && ($build_scores_table['match_index'] < (128+64+1)))
      {
        ...
        echo("document.getElementById('R" . $rounds . "_match_" . ($j+1) . "').value = '" . $build_scores_table['match_index'] . "';");
        $j++;
      }
      if(($build_scores_table['match_index'] > (128+64)) && ($build_scores_table['match_index'] < (128+64+32+1)))
      {
        ...
        echo("document.getElementById('R" . $rounds . "_match_" . ($k+1) . "').value = '" . $build_scores_table['match_index'] . "';");
        $k++;
      }
      if(($build_scores_table['match_index'] > (128+64+32)) && ($build_scores_table['match_index'] < (128+64+32+16+1)))
      {
        ...
        echo("document.getElementById('R" . $rounds . "_match_" . ($l+1) . "').value = '" . $build_scores_table['match_index'] . "';");
        $l++;
      }
      if(($build_scores_table['match_index'] > (128+64+32+16)) && ($build_scores_table['match_index'] < (128+64+32+16+8+1)))
      {
        ...
        echo("document.getElementById('R" . $rounds . "_match_" . ($m+1) . "').value = '" . $build_scores_table['match_index'] . "';");
        $m++;
      }
      if(($build_scores_table['match_index'] > (128+64+32+16+8)) && ($build_scores_table['match_index'] < (128+64+32+16+8+4+1)))
      {
        ...
        echo("document.getElementById('R" . $rounds . "_match_" . ($n+1) . "').value = '" . $build_scores_table['match_index'] . "';");
        $n++;
      }
      if(($build_scores_table['match_index'] > (128+64+32+16+8+4)) && ($build_scores_table['match_index'] < (128+64+32+16+8+4+2+1)))
      {
        ...
        echo("document.getElementById('R" . $rounds . "_match_" . ($o+1) . "').value = '" . $build_scores_table['match_index'] . "';");
        $o++;
      }
    }
  ...
  mysql_data_seek($result_scores, 0);

  while($build_scores = $result_scores->fetch_assoc())
  {
    if(($build_scores['member_id'] == 1))
    {
        $member_name = "Bye";
    }
    else
    ...
    //echo("Member Name " . GetMemberName($build_scores['member_id']) . "<br>");
    if($build_scores['r_128_position'] != 0)
    {
      //echo("console.log('" . GetMemberName($build_scores['member_id']) . "');");
      echo("console.log('" . $build_scores['member_id'] . "');");
      $R128_Pos = $build_scores['r_128_position'];
      //echo("console.log('" . $R128_Pos . "');");
  ...
  echo("<br>");
  echo('<div align="center" class="greenbg"><button id="import_draw" onclick="ImportDraw()" style="width: 300px;">Import Tournament Draw Data</button></div>');
  echo("<br>");
...
?>
  ...
  if($result_tourn_players_count->num_rows == 0)
  {
    if($tourn_type == 'Billiards')
    {
      ...
      //$query_tourn_players = 'Select * FROM tournament_players';
  ...
  {
    $query_tourn_players = 'Select * FROM tournament_scores LEFT JOIN members on member_id=MemberID LEFT JOIN tournaments on tournament_scores.tourn_id=tournaments.tourn_id where tournament_scores.tourn_id = ' . $tournament_id . ' Order by r_128_position';
  }
  ...
  while($build_tourn = $result_tourn_players->fetch_assoc())
  {
    ...
    //echo("Name " . "R" . $total_players . "_name_" . ($x+1) . "<br>");

    echo("<div class='team-item' style='width:200px'><input type='text' id='R" . $total_players . "_seed_" . ($x+1) . "' value='" . $build_tourn['ranknum'] . "' style='width:35px'><input type='text' value='" . $fullname . "' id='R" . $total_players . "_name_" . ($x+1) . "' style='width:120px'><input type='text' id='R" . $total_players . "_score_" . ($x+1) . "' style='width:20px' readonly value=''>");
    echo("<input type='hidden' value=" . $build_tourn['member_id'] . " id='R" . $total_players . "_id_" . ($x+1) . "'><input type='hidden' id='R" . $total_players . "_match_" . ($x+1) . "' value=''><br><img src='$default_user_image' id='R" . $total_players . "_change_" . ($i+1) . "' style='width:32px; height:32x'><time><input type='text' id='R" . $total_players . "_day_" . ($x+1) . "' value='" . $build_tourn['r_128_day'] . "' style='width:50px'><input type='text' id='R" . $total_players . "_time_" . ($x+1) . "' value='' style='width:50px'></time></div>");
    $x++;
  }
  echo("</div>");
  ...
  if($total_players > 64)
  {
    ...
    for($i = 0; $i < 64; $i++)
    {
      ...
      echo("<input type='hidden' value='' id='R64_id_" . ($i+1) . "'><input type='hidden' id='R64_match_" . ($i+1) . "' value=''><br><img src='$default_user_image' id='R64_change_" . ($i+1) . "' style='width:32px; height:32x'><time><input type='text' id='R64_day_" . ($i+1) . "' value='' style='width:50px'><input type='text' id='R64_time_" . ($i+1) . "' value='' style='width:50px'></time></div>");
    }
    echo("</div>");
  }
  ...
  if($total_players > 32)
  {
    ...
    for($i = 0; $i < 32; $i++)
    {
      ...
      echo("<input type='hidden' value='' id='R32_id_" . ($i+1) . "'><input type='hidden' id='R32_match_" . ($i+1) . "' value=''><br><img src='$default_user_image' id='R32_change_" . ($i+1) . "' style='width:32px; height:32x'><time><input type='text' id='R32_day_" . ($i+1) . "' value='' style='width:50px'><input type='text' id='R32_time_" . ($i+1) . "' value='' style='width:50px'></time></div>");
    }
    echo("</div>");
  }
  ...
  if($total_players > 16)
  {
    ...
    for($i = 0; $i < 16; $i++)
    {
      ...
      echo("<input type='hidden' value='' id='R16_id_" . ($i+1) . "'><input type='hidden' id='R16_match_" . ($i+1) . "' value=''><br><img src='$default_user_image' id='R16_change_" . ($i+1) . "' style='width:32px; height:32x'><time><input type='text' id='R16_day_" . ($i+1) . "' value='' style='width:50px'><input type='text' class='time' id='R16_time_" . ($i+1) . "' value='' style='width:50px'></time></div>");
    }
    echo("</div>");
  }
  ...
  if($total_players > 4)
  {
    ...
    for($i = 0; $i < 4; $i++)
    {
      echo("<div class='team-item' style='width:200px'><input type='text' id='R4_name_" . ($i+1) . "' style='width:120px' value=''><input type='text' id='R4_score_" . ($i+1) . "' value='' style='width:20px'><input type='hidden' value='' id='R4_id_" . ($i+1) . "'><input type='hidden' id='R4_match_" . ($i+1) . "' value=''><br><img src='$default_user_image' id='R4_change_" . ($i+1) . "' style='width:32px; height:32x'><time><input type='text' id='R4_day_" . ($i+1) . "' value='' style='width:50px'><input type='text' id='R4_time_" . ($i+1) . "' value='' style='width:50px'></time></div>");
    }
    echo("</div>");
  }
  ...
  if($total_players > 1)
  {
    ...
    echo("<div class='team-item' style='width:200px'><input type='text' id='R1_name_1' style='width:120px' value=''><input type='hidden' value='' id='R1_id_1'><input type='hidden' id='R1_match_1' value=''><br></div>");
    echo("</div>");
  }
...
$(document).ready(function()
{
  ...
  $.fn.get_match = function (str) {
    ...
    var second = (str.split(subStr, 2).join(subStr).length);
    var element_no = parseInt(str.substring(second+1));
    return $('#R' + col_no + '_match_' + element_no).val();
  }
  ...
  for(i = 0; i < 128; i++)
  {
    $('#R128_day_' + (i+1)).click(function() {
        ...
        $('#match_index').val($.fn.get_match(id));
        $('#element_id').html(id);
    });
  ...
  $('#save_time_button').click(function()
  {
    ...
    var element_id = $('#element_id').html();
    var match_index = $('#match_index').val();
    var subStr = '_';
    var first = (element_id.split(subStr, 1).join(subStr).length);
    var col = element_id.substring(0, first);
    var col_no = col.substring(1, col.length);
    ...
    //console.log("Time/Date " + col_no + ", " + element_no);
    $.ajax({
      url:"save_time_data.php?tourn_id=" + tourn_id + "&player_id=" + player + "&new_day=" + new_day + "&new_time=" + new_time + "&round_no=" + col_no + "&match_index=" + match_index,
      method: 'GET',
      success:function(data)
      {
...
<!--
          <tr>
            <td colspan='8' align='center'>Best of 128 - <?= $best_of_128 ?></td>
          </tr>
          <tr>
            <td colspan='8' align='center'>Total Rounds - <?= $total_tourn ?></td>
          </tr>
-->
          <tr>
    ...
    <table class='table table-striped table-bordered' style='width:300px;'>
      ...
      <tr>
          ...
          <td colspan='3' ><select id="referee">
          <?php
