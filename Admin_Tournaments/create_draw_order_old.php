...
//    $tournament_id = $_POST['tournament'];
    ...
    //$query_tourn_orig = 'Select * FROM tournament_players Where tourn_id = 202462 order by round_no, draw_pos';
    ...
    $total_players = $result_tourn_orig->num_rows;
    //echo("Total Players in team_entry " . $total_players . "<br>");
    $total_players = 128;
    $original = [];
    $zero = [];
    ...
    */
    while($build_players_orig = $result_tourn_orig->fetch_assoc())
    {
        ...
        //$original[$original_index] = "Round: " . $build_players_orig['round_no'] . ", MemberID: " . $build_players_orig['memb_id'] . ", Rank: " . $build_players_orig['ranknum'];
...
*/
    ...
    {
        $bye[] = "Round: 1, Index: " . ($x+200) . ", MemberID: " . ($bye_index+10000+$x) . ", Rank: 0";
    }
/*
    $even_r2 = [];
    $odd_r2 = [];
...
*/    
    ...
    $newArray = array_merge($original, $bye);
...
*/
        ...
        //$total_matches = ($total_players/2);
        //echo("No of Matches Day 1 - " . $build_dates['matches_day_1'] . "<br>");
        //echo("Index - " . $index . "<br>");
...
/*
        ...
        $query_draw_table = 'Select * FROM tournament_players where tourn_id = ' . $tournament_id . ' Order by id';
        ...
        if($total_draw_table > 0)
        {
            ...
            {
                $sql_draw = 'Update tournament_scores SET r_' . floor($total_players/2) . '_position = ' . $build_draw_table['id'] . ' where member_id = ' . $build_draw_table['memb_id'] . ' and tourn_id = ' . $tournament_id;
                echo($sql_draw . "<br>");
...
*/
    ...
    if($total_draw_table > 0)
    {
        $sql_draw_pos = 'Update tournament_scores SET r_128_position = 0, r_64_position = 0, r_32_position = 0, r_16_position = 0, r_8_position = 0, r_4_position = 0, r_2_position = 0 where tourn_id = ' . $tournament_id;
        ...
        while($build_draw_table = $result_draw_table->fetch_assoc())
        {
            ...
            {
                $sql_draw = 'Update tournament_scores SET r_' . floor($total_players) . '_position = ' . $build_draw_table['draw_pos'] . ' where member_id = ' . $build_draw_table['memb_id'] . ' and tourn_id = ' . $tournament_id;
                //echo("(1) " . $sql_draw . "<br>");
                $update = mysql_query($sql_draw, $connvbsa) or die(mysql_error());  
            }
            else if($build_draw_table['round_no'] == 2) 
            {
                $sql_draw_2 = 'Update tournament_scores SET r_' . floor($total_players/2) . '_position = ' . $build_draw_table['draw_pos'] . ' where member_id = ' . $build_draw_table['memb_id'] . ' and tourn_id = ' . $tournament_id;
                //echo("(2) " . $sql_draw_2 . "<br>");
                $update = mysql_query($sql_draw_2, $connvbsa) or die(mysql_error());   
            }
            else if($build_draw_table['round_no'] == 3) 
            {
                $sql_draw_3 = 'Update tournament_scores SET r_' . floor($total_players/4) . '_position = ' . $build_draw_table['draw_pos'] . ' where member_id = ' . $build_draw_table['memb_id'] . ' and tourn_id = ' . $tournament_id;
    ...
    //header("Location: ../Admin_Tournaments/tournament_draw.php?tourn_id=" . $tournament_id);
