...
function ImportExcel($filename, $current_year, $season, $tourn_id) 
{
    ...
    for ($row_2 = 0; $row_2 < $highestRow; $row_2++) {
        ...
        {
            if($rowData_2[$row_2][2] == 'Bye')
            {
                $memberID = (10000+$row);
            }
            else
            {
                $memberID = (int)GetMemberID($rowData_2[$row_2][2]);
            }
    ...
    $rowData_3 = $sheet->rangeToArray("G2:I" . $highestRow, NULL, TRUE, FALSE);
    for ($row_3 = 0; $row_3 < $highestRow; $row_3++) {
        //  Read a row of data into an array
        //$unix_date = ($rowData[$row][0] - 25569) * 86400;
        if(isset($rowData_3[$row_3][0])) // stop before the last entry
        {   
            if($rowData_3[$row_3][2] == 'Bye')
            {
                $memberID = (10000+$row);
            }
            else
            {
                $memberID = (int)GetMemberID($rowData_3[$row_3][2]);
            }
            // no Byes carried forward.
            $sql_3 = "Insert INTO tournament_players (draw_pos, round_no, tourn_id, ranknum, memb_id, fullname) Values (" . ($row_3+1) . ", 3, " . $tourn_id . ", " . $rowData_3[$row_3][0] . ", " . (int)GetMemberID($rowData_3[$row_3][2]) . ", '" . $rowData_3[$row_3][2] . "')";
            echo($sql_3 . "<br>");
