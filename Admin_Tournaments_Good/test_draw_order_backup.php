...
/*    
    // prepare snooker and billiards ranking table
    //Rank_Billiards_Draw Billiards Rankings
    $querytoexecute = "Truncate TABLE `Rank_Billiards_Draw`";
    $result=mysql_query($querytoexecute, $connvbsa) or die(mysql_error());

    //Rank_Snooker_Draw Snooker Rankings
    $querytoexecute = "Truncate TABLE `Rank_Snooker_Draw`";
    $result=mysql_query($querytoexecute, $connvbsa) or die(mysql_error());
    //Rank_Snooker_Draw Snooker Tournament Rankings
    $querytoexecute = "Insert INTO `Rank_Snooker_Draw`
    SELECT 
    0,
    ...
    $querytoexecute = "Insert INTO `Rank_Billiards_Draw`
    SELECT 
    0,
...
if($tourn_type == 'Billiards')
{
$query_tourn_ranked_test = 'Select * FROM vbsa3364_vbsa2.tourn_entry LEFT JOIN members on tourn_memb_id=MemberID LEFT JOIN rank_Billiards on memb_id=MemberID LEFT JOIN tournaments on tourn_id=tournament_number where tournament_number = ' . $tournament_id . ' and MemberID != "" Order by ranknum = 0, ranknum ASC';
}
else if($tourn_type == 'Snooker')
{
$query_tourn_ranked_test = 'Select * FROM vbsa3364_vbsa2.tourn_entry LEFT JOIN members on tourn_memb_id=MemberID LEFT JOIN rank_S_open_tourn on memb_id=MemberID LEFT JOIN tournaments on tourn_id=tournament_number where tournament_number = ' . $tournament_id . ' and MemberID != "" Order by ranknum = 0, ranknum ASC';
}

echo("Winner " . $previous_winner . "<br>");
$result_tourn_ranked_test = mysql_query($query_tourn_ranked_test, $connvbsa) or die(mysql_error());
//$total_ranked_test = $result_tourn_ranked_test->num_rows;
$ranked_index_test = 0;
while($build_players = $result_tourn_ranked_test->fetch_assoc())
{
    ...
    //echo("MemberID: " . $build_players['MemberID'] . ", Name: " . $build_players['FirstName'] . " " . $build_players['LastName'] . ", Rank: " . $rank_num . "<br>");
    $ranked_test[$ranked_index_test] = "Round: 1, Rank: " . $rank_num . ", Index: " . ($ranked_index_test+1) . ", MemberID: " . $build_players['MemberID'];
    $ranked_index_test++;
}
sort($ranked_test);
echo("List of Ranked and Non Ranked (index)<br>");
echo("<pre>");
echo(var_dump($ranked_test));
echo("</pre>"); 
*/
