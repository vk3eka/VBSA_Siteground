...
if(isset($_POST['ButtonName']) && ($_POST['ButtonName'] == 'SaveDraw'))
{
  ...
  //echo($total_players_score . "<br>");
  $packedscoredata_1 = json_decode(stripslashes($_POST['ScoreData1']), true);
//echo("<pre>");
//echo(var_dump($packedscoredata_1));
//echo("</pre>");

  //$packedscoredata_2 = json_decode(stripslashes($_POST['ScoreData2']), true);
  //$packedscoredata_3 = json_decode(stripslashes($_POST['ScoreData3']), true);
  //echo("Data 1 " . $packedscoredata_1[0] . "<br>");
  //echo("Data 2 " . stripslashes($_POST['ScoreData']) . "<br>");
  //echo("Size 1 " . sizeof($packedscoredata_1) . "<br>");
  //echo("Size 2 " . sizeof($packedscoredata_2) . "<br>");
  //echo("Size 3 " . sizeof($packedscoredata_3) . "<br>");

  for($i = 0; $i < sizeof($packedscoredata_1); $i++)
  {
    //echo("Size " . sizeof($packedscoredata) . "<br>");
    $score = explode(",", $packedscoredata_1[$i]);
    //echo("Size 2 " . sizeof($score) . "<br>");
...
/*
// loop until total_player_scores = 1

  $total_players_score = ($total_players_score/2);

  for($i = 0; $i < sizeof($packedscoredata_2); $i++)
  {
    //echo("Size " . sizeof($packedscoredata) . "<br>");
    $score = explode(",", $packedscoredata_2[$i]);
    for($y = 0; $y < sizeof($score); $y++)
    {
      $score_row = explode(", ", $score[$y]);
      if($score_row[0] != '')
  ...
  for($i = 0; $i < sizeof($packedscoredata_3); $i++)
  {
    //echo("Size " . sizeof($packedscoredata) . "<br>");
    $score = explode(",", $packedscoredata_3[$i]);
    for($y = 0; $y < sizeof($score); $y++)
    {
      $score_row = explode(", ", $score[$y]);
      if($score_row[0] != '')
...
<script type='text/javascript'>
  ...
  function SaveAll(total_players, tourn_id) {
    ...
    //var scoredata_player_2 = [];
    //var scoredata_player_3 = [];
    //var scoredata_player_total = [];
    ...
    //console.log(scoredata_player);
    /*
    total_players = (total_players/2);

    for(x = 0; x < total_players; x++)
    {
      if((document.getElementById("R" + total_players + "_name_" + x).value) != '')
      {
        scoredata_player_2[x] = document.getElementById("R" + total_players + "_name_" + x).value + ", " + document.getElementById("R" + total_players + "_score_" + x).value;
      }
    }
    scoredata_player_2 = JSON.stringify(scoredata_player_2);
    //console.log(scoredata_player_1);
    ...
    for(x = 0; x < total_players; x++)
    {
      if((document.getElementById("R" + total_players + "_name_" + x).value) != '')
      {
        scoredata_player_3[x] = document.getElementById("R" + total_players + "_name_" + x).value + ", " + document.getElementById("R" + total_players + "_score_" + x).value;
      }
    }
    scoredata_player_3 = JSON.stringify(scoredata_player_3);
    //console.log(scoredata_player_2);

    //scoredata_player_total = JSON.stringify(scoredata_player + ", " + scoredata_player_1 + ", " + scoredata_player_2);
    //console.log(scoredata_player_total);
    */
    ...
    //document.tournament_draw.ScoreData2.value = scoredata_player_2;
    //document.tournament_draw.ScoreData3.value = scoredata_player_3;
...
<!--<input type='hidden' name='ScoreData2'>
<input type='hidden' name='ScoreData3'>-->
...
?>
  ...
  while($build_tourn = $result_tourn_players->fetch_assoc())
  {
    ...
    echo("<div class='team-item' style='width:200px'><input type='text' value='" . $build_tourn['FirstName'] . " " . $build_tourn['LastName'] . "' id='R" . $total_players . "_name_" . $x . "' style='width:120px'><input type='text' id='R" . $total_players . "_score_" . $x . "' style='width:20px' readonly></div>");
    ...
    for(i = 0; i < 7; i++)
    {
      ...
      //$('#score' + (i+1) + '_1').change(function () { 
        if(score_data_1[i] > score_data_2[i])
        {
          game_score_1++;
          //game_score_2--;
          $('#game_score_1').html(game_score_1);
        }
        else if(score_data_1[i] < score_data_2[i])
        {
          //game_score_1--;
          game_score_2++;
          $('#game_score_2').html(game_score_2);
        }
      //});
    }
...
/*
    $.ajax({
      url:"<?= $url ?>/save_draw_score.php?playername=" + team_name + "&memberid=" + firstname + "&element_id=" + surname + "&score_data=" + team_grade + "&break_data=" + round,
      method: 'POST',
      ...
      <div class="modal-body">
        ...
        <table class='table table-striped table-bordered' style='width:250px;'>
          ...
          <!--<tr>
            ...
            <td align='center'><input type='text' id='game_score_1' value=0 style='width:40px; height:20px'></td>-->
          </tr>
        </table>
      </td>
      <td align='center'>
        <table class='table table-striped table-bordered'  style='width:250px;'>
          ...
          <!--<tr>
            <td colspan='2' align='center'>Game Score</td>
            <td align='center'><input type='text' id='game_score_2' value=0 style='width:40px; height:20px'></td>-->
          </tr>
        </table>
      </td>
    </tr>
