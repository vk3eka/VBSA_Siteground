...
if ((isset($_POST["MM_insert"])) && ($_POST["MM_insert"] == "form1")) {
  ...
  /*
  $updateCalSQL = sprintf("Update calendar SET tourn_id=%s, event=%s, venue=%s, state=%s, aust_rank=%s, ranking_type=%s, startdate=%s, finishdate=%s, closedate=%s, about=%s, tourn=%s, visible=%s, footer1=%s, footer2=%s, footer3=%s, footer4=%s WHERE event_id=%s",
                       GetSQLValueString($_POST['tourn_id'], "text"),
                       GetSQLValueString($_POST['event'], "text"),
                       GetSQLValueString($_POST['venue'], "text"),
                       GetSQLValueString($_POST['state'], "text"),
                       GetSQLValueString($_POST['aust_rank'], "text"),
  ...
  <table width="700" style='border: 1px solid black; border-collapse: collapse' align="center" class="page">
    ...
    <tr valign="baseline">
      ...
      <td><select name="tourn_director">
        ...
        echo("<option value='" .  $row_Cal_edit['tourn_director'] . "'>" . $row_Cal_edit['tourn_director'] . "</option>");
        //echo("<option value=''>&nbsp;</option>");
    ...
    <tr valign="baseline">
      ...
      <td><select name="referee_early">
        ...
        $query_member = 'Select MemberID, FirstName, LastName FROM members Where MemberID != 1 Order by LastName';
        $result_member = mysql_query($query_member, $connvbsa) or die(mysql_error());
        echo("<option value='" .  $row_Cal_edit['referee_early'] . "'>" . $row_Cal_edit['referee_early'] . "</option>");
        while($build_member = $result_member->fetch_assoc())
        {
           echo("<option value='" . $build_member['FirstName'] . " " . $build_member['LastName'] . "'>" . $build_member['FirstName'] . " " . $build_member['LastName'] . "</option>");
        }
    ...
    <tr valign="baseline">
      ...
      <td><select name="referee_later">
        ...
        $query_member = 'Select MemberID, FirstName, LastName FROM members Where MemberID != 1 Order by LastName';
        $result_member = mysql_query($query_member, $connvbsa) or die(mysql_error());
        echo("<option value='" .  $row_Cal_edit['referee_later'] . "'>" . $row_Cal_edit['referee_later'] . "</option>");
        while($build_member = $result_member->fetch_assoc())
        {
           echo("<option value='" . $build_member['FirstName'] . " " . $build_member['LastName'] . "'>" . $build_member['FirstName'] . " " . $build_member['LastName'] . "</option>");
        }
