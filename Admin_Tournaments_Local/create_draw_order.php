...
//    $tournament_id = $_POST['tournament'];
    ...
    //$query_tourn_orig = 'Select * FROM tournament_players';
    ...
    if($total_draw_table > 0)
    {
        ...
        while($build_draw_table = $result_draw_table->fetch_assoc())
        {
            if($build_draw_table['round_no'] == 1)
            {
                ...
                if($build_draw_table['memb_id'] == 1)
                {    
                    $sql_draw = 'Update tournament_scores SET r_' . floor($total_players) . '_position = ' . $build_draw_table['draw_pos'] . ', fullname = "Bye" where member_id = ' . $build_draw_table['memb_id'] . ' and tourn_id = ' . $tournament_id;
                    echo("Bye " . $sql_draw . "<br>");
                    $update = mysql_query($sql_draw, $connvbsa) or die(mysql_error()); 
                }  
                else
                {
                    ...
                    echo("Not Bye " . $sql_draw . "<br>");
                    $update = mysql_query($sql_draw, $connvbsa) or die(mysql_error());  
                }
            }
            else if ($build_draw_table['round_no'] == 2) 
            {
                $sql_draw_2 = 'Update tournament_scores SET r_' . floor($total_players/2) . '_position = ' . ($build_draw_table['draw_pos']/2) . ' where member_id = ' . $build_draw_table['memb_id'] . ' and tourn_id = ' . $tournament_id;
                echo("R2 " . $sql_draw . "<br>");
