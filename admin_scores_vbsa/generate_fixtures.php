...
//echo("Grade Start " . $grade_start . "<br>");
...
{   
    ...
    $sql_update = "Update Team_entries set home_games = 0, fix_sort = 0 WHERE team_season ='" . $saeson . "' AND team_cal_year = " . $year;
...
{   
    ...
    //header("Location:'" . $_SERVER['PHP_SELF'] . "'");
    header("generate_fixtures.php?DayPlayed=Wed&season=S2");
}
...
{   
    ...
    //echo($_SERVER['PHP_SELF'] . "?DayPlayed=" . $dayplayed ."&season=" .  $season . "<br>");
    //header("Location:'" . $_SERVER['PHP_SELF'] . "?DayPlayed=" . $dayplayed ."&season=" .  $season . "'");
    header("Location:'" . $_SERVER['PHP_SELF'] . "'");
}
...
function CreateDateArray($form_no, $teams, $team_grade, $year, $season) 
{
    for($y = 0; $y < (($teams*2)-2); $y++)
    {
        ...
        while($row = $result_playing_dates->fetch_assoc()) 
        {
            ...
            {
                //echo("console.log('New Date " . $row_date . ", Database Date " . $row_dates['DTSTART'] . "');");
...
function SetDate(form_no, teams)
{
    ...
    for(i = 0; i < teams; i++)
    {
        //document.getElementById('A_' + form_no + '_date_' + (i+1)).value = addDays(document.getElementById('A_' + form_no + '_date_' + i).value, 7).value;
...
function GetHomeGames($team_grades, $round, $year, $season, $dayplayed)
{
    ...
    foreach($team_grade as $grade)
    {
        ...
        if($grade != '')
        {
            ...
            $fixArray_round = $build_home_games['fix1home'] . ', ' . $build_home_games['fix2home'] . ', ' . $build_home_games['fix3home'] . ', ' . $build_home_games['fix4home'] . ', ' . $build_home_games['fix5home'] . ', ' . $build_home_games['fix6home'];
            $fixArray = $fixArray . ", " . $fixArray_round;
        }
    }
    //echo("<pre>");
...
function CountHomeGames($team_grades, $year, $season, $dayplayed)
{
    ...
    while($build_home_games = $result_home_games->fetch_assoc())
    {
        ...
        //echo("Count " . $count['Box Hill Test'] . "<br>");
        $numOfTrue = $count[$team_name];
        //echo("Count " . $numOfTrue . "<br>");
...
//echo("Create Fixtures SQL " . $sql_count_home_games . "<br>");
$result_count_home_games = mysql_query($sql_count_home_games, $connvbsa) or die(mysql_error());
//$build_count_home_games = $result_count_home_games->fetch_assoc();
$count_home_games_rows = $result_count_home_games->num_rows;
//echo("Create Fixtures Count " . $count_home_games_rows . "<br>");
if($count_home_games_rows > 0)
{
    //echo("Counting Home Games<br>");
...
?>
<form name='reset_fixture' method='post' action='generate_fixtures.php?DayPlayed=Wed&season=S2'>
...
<table width="900" align="center">
  ...
  <tr>
    ...
    <td align="center" nowrap="nowrap" class="greenbg" style='width:350px'><a href="AA_scores_index_select_season.php">Return to opening page</a></td>
  </tr>
  <tr>
    <td>&nbsp;</td>
    <td>&nbsp;</td>
...
$sql_grades_menu = 'Select team_grade, count(team_grade) as count from Team_entries Join clubs where clubs.ClubNumber = Team_entries.team_club_id and team_cal_year = ' . $year . ' and day_played = "' . $dayplayed . '" group by team_grade';
$result_grades_menu = mysql_query($sql_grades_menu, $connvbsa) or die(mysql_error());
$i = 0;
while($build_data_menu = $result_grades_menu->fetch_assoc())
{
