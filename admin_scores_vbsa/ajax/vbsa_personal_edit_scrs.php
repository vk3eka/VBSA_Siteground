...
$editFormAction = "ajax/vbsa_personal_edit_scrs.php";
if (isset($_SERVER['QUERY_STRING'])) {
  $editFormAction .= "?" . htmlentities($_SERVER['QUERY_STRING']);
}

if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE members SET LastName=%s, FirstName=%s, HomeAddress=%s, HomeSuburb=%s, HomeState=%s, HomePostcode=%s, HomePhone=%s, WorkPhone=%s, MobilePhone=%s, ReceiveSMS=%s, Email=%s, ReceiveEmail=%s, BoardMemb=%s, LifeMember=%s, Deceased=%s, Junior=%s, Female=%s, Overseas=%s, Referee=%s, Ref_Class=%s, LastUpdated=%s, UpdateBy=%s, AffiliateMemb=%s, board_position=%s, paid_memb=%s, paid_how=%s, paid_date=%s, dob_day=%s, dob_mnth=%s, dob_year=%s, prospective_ref=%s WHERE MemberID=%s",
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_membedit = "SELECT members.MemberID, LastName, FirstName, HomeAddress, HomeSuburb, HomeState, HomePostcode, HomePhone, WorkPhone,  MobilePhone, ReceiveSMS,  Email, ReceiveEmail, BoardMemb, LifeMember, Deceased,  Female, Overseas, Referee,  Ref_Class, members.LastUpdated, members.UpdateBy, AffiliateMemb, entered_on, board_position, dob_day, dob_mnth, dob_year, prospective_ref, members.paid_memb, members.paid_how, members.paid_date FROM members WHERE members.MemberID='$mID'";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_played = "SELECT FirstName, LastName, lifemember, team_grade, count_played, game_type, scr_season, current_year_scrs FROM scrs  LEFT JOIN members ON members.MemberID=scrs.MemberID WHERE scrs.MemberID='$mID' AND (current_year_scrs = YEAR(CURDATE( )) OR fin_year_scrs = YEAR(CURDATE( ))) ORDER BY current_year_scrs DESC";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_fin = "SELECT MemberID, paid_memb, paid_how, paid_date, lifemember FROM members WHERE paid_memb is not null AND YEAR(paid_date)=YEAR( CURDATE( ) ) AND MemberID='$membedit'";
...
<table width="1000" align="center">
  ...
  <tr>
    <td colspan="5" class="red_bold">Playing History, Current year and Financial Year - Editable by the Score Registrar only</td>
    </tr>
  <tr>
  ...
  <tr>
      ...
      <td align="left"><?php echo $row_played['game_type']; ?></td>
      </tr>
  <?php } while ($row_played = mysql_fetch_assoc($played)); ?>
  
  <tr>
    <td colspan="5" align="left">Member Summary - Life member<?php echo $row_fin['lifemember']; ?></td>
    </tr>
</table>
    </td><!--endnested table -->
    <td><!--nested table -->
