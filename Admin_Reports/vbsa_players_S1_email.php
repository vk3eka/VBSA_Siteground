...
$query_S1players = "SELECT scrsID, scrs.MemberID, members.FirstName, members.LastName, members.Email, members.MobilePhone, SUM(scrs.played_bill_S1) AS S1bill, SUM(scrs.played_S1) AS S1snook FROM scrs, members, Team_entries WHERE members.MemberID=scrs.MemberID   AND scrs.team_id=Team_entries.team_id  AND scrs.S1 is not null  AND scrs.MemberID<>'1'   AND scrs.MemberID<>'500'  AND scrs.MemberID<>'1000'   AND scrs.current_year_scrs = YEAR( CURDATE( ) )  AND scrs.S1 =1 AND email is not null GROUP BY scrs.MemberID HAVING (SUM(scrs.played_bill_S1)>1 OR SUM(scrs.played_S1)>1) ORDER BY FirstName, LastName";
$S1players = mysql_query($query_S1players, $connvbsa) or die(mysql_error());
$row_S1players = mysql_fetch_assoc($S1players);
$totalRows_S1players = mysql_num_rows($S1players);mysql_select_db($database_connvbsa, $connvbsa);
$query_S1players = "SELECT scrsID, scrs.MemberID, members.FirstName, members.LastName, members.Email, members.MobilePhone, SUM(scrs.played_bill_S1) AS S1bill, SUM(scrs.played_S1) AS S1snook FROM scrs, members, Team_entries WHERE members.MemberID=scrs.MemberID   AND scrs.team_id=Team_entries.team_id  AND scrs.S1 =1  AND scrs.MemberID<>'1'   AND scrs.MemberID<>'500'  AND scrs.MemberID<>'1000'   AND scrs.current_year_scrs = YEAR( CURDATE( ) )  AND scrs.S1 =1 AND email is not null GROUP BY scrs.MemberID HAVING (SUM(scrs.played_bill_S1)>1 OR SUM(scrs.played_S1)>1) ORDER BY FirstName, LastName";
$S1players = mysql_query($query_S1players, $connvbsa) or die(mysql_error());
$row_S1players = mysql_fetch_assoc($S1players);
...
<table align="center">
      ...
      <tr>
        <td colspan="2" class="red_bold">All players currently playing Snooker or Billiards in season S1 and have played more than 1 match, scroll to bottom of page to send email</td>
      </tr>
      <tr>
  ...
  <table border="1" align="center" cellpadding="2" cellspacing="2">
    <tr class="page">
      ...
      <td align="center" nowrap="nowrap">Played Billiards</td>
      <td>Email</td>
      <td>Mobile</td>
      <td>&nbsp;</td>
      <td>&nbsp;</td>
    ...
    <?php do { ?>
      <tr>
        ...
        <td align="center"><?php echo $row_S1players['S1bill']; ?></td>
