...
function EditTeam($edit_team) 
{
  ...
  document.edit_scoresheet.Year.value = <?php echo($current_year); ?>; 
  ...
  $.fn.getexistingarray = function (type, team, home_away) {
    ...
    $.ajax({
      ...
      success : function(data){
        ...
        //console.log("Existing Team " + "<?= $_SESSION['clubname'] ?>");
        //console.log("Home Team " + home_team);
        //console.log("Team " + team);
        if($.trim(currForm) == $.trim(obj))
        {
          if(team != '<?= $_SESSION['clubname'] ?>') // if current team is same as approval team no need to get password
          {
            $.ajax({
              ...
              error: function (request, error) {
                alert("Error, cannot get data (capt approval)"); 
              }
            });
          ...
          {
            $("input[id='" + home_away + "_ok']").prop("checked", true);
            team = '<?= $_SESSION['home'] ?>'; 
            opposition = '<?= $_SESSION['away'] ?>'; 
            $.fn.save_approval(team, opposition, home_away);
          }
        }
        else
        {
      ...
      error: function (request, error) {
          alert("Error, cannot get data (data changed)"); 
      }
    });
  }
  ...
  $.fn.getscorematchups = function (type) {
    ...
    $.ajax({
      ...
      success : function(data){
        ...
        var obj = jQuery.parseJSON(data);
        obj_approval = obj[5].split(","); // change from 4 to 5 - 7/8/2025
        ...
        {
          obj_test = obj[x].split(",");
          //console.log("obj_test x " + x + ", " + obj_test);

          if(type == 'Billiards')
          {
            //console.log("Billiards here");
            if($('#A_player_0').html().substr(0,12) == 'Team Forfeit')
            {
              $("input[id='A_loss_0']").prop("checked", true); 
              $("input[id='A_loss_1']").prop("checked", true); 
          ...
          {
            // changed first team member to three 'trues', all others two 'trues' 10/08/2024
            // reverted 10/08/2024

            //console.log("Snooker here");
            if($('#A_player_0').html().substr(0,12) == 'Team Forfeit')
            {
              $("input[id='B_win_0_0']").prop("checked", true); 
              $("input[id='B_win_0_1']").prop("checked", true); 
              ...
              $("input[id='B_win_1_1']").prop("checked", true); 
              $("input[id='B_win_1_2']").prop("checked", false); 
              $("input[id='B_win_1_3']").prop("checked", false); 

              $("input[id='B_win_2_0']").prop("checked", true); 
              $("input[id='B_win_2_1']").prop("checked", true); 
              $("input[id='B_win_2_2']").prop("checked", false); 
              $("input[id='B_win_2_3']").prop("checked", false); 

              $("input[id='B_win_3_0']").prop("checked", true); 
              $("input[id='B_win_3_1']").prop("checked", true); 
              $("input[id='B_win_3_2']").prop("checked", false); 
              $("input[id='B_win_3_3']").prop("checked", false); 

              // added to remove wins from forfeit 12/08/2024

              $("input[id='A_win_1_0']").prop("checked", false); 
              $("input[id='A_win_1_1']").prop("checked", false); 
              $("input[id='A_win_1_2']").prop("checked", false); 
              $("input[id='A_win_1_3']").prop("checked", false); 

              $("input[id='A_win_2_0']").prop("checked", false); 
              $("input[id='A_win_2_1']").prop("checked", false); 
              $("input[id='A_win_2_2']").prop("checked", false); 
              $("input[id='A_win_2_3']").prop("checked", false); 

              $("input[id='A_win_3_0']").prop("checked", false); 
              $("input[id='A_win_3_1']").prop("checked", false); 
              $("input[id='A_win_3_2']").prop("checked", false); 
              $("input[id='A_win_3_3']").prop("checked", false); 
          ...
          if(x == 4)
          {
            if(type == 'Snooker')
            {
                //console.log("Wins 1 " + obj[3].split(","));
                //obj_test = obj[3].split(",");
                //console.log("Wins " + obj_test[1]);
      ...
      error: function (request, error) {
          alert("Error, cannot get data (matchups)"); 
      }
    });
  }
...
// get hometeam
  $sql_wins = "Select * from tbl_club_results where club = '" . $session_home . "' AND round = " . $round . " AND season = '" . $season . "' AND team_grade = '" . $team_grade . "' AND year = " . $current_year;
  ...
  $sql = "Select * from tbl_scoresheet where team = '" . $session_home . "' AND round = " . $round . " AND season = '" . $season . "' AND year = " . $current_year . "  AND team_grade = '" . $team_grade . "' Order By playing_position";
  ...
  while($build_data = $result->fetch_assoc()) 
  {
    ...
    if($type == 'Billiards')
    {
      ...
      echo("document.getElementById('A_tier_" . $j . "').innerHTML = '" .  addslashes($build_data['tier']) . "';");
    }
    else
    {
      echo("document.getElementById('A_player_" . $j . "').innerHTML = '" . addslashes($build_data['players_name']) . "';");
  ...
  // get awayteam
  $sql_wins_op = "Select * from tbl_club_results where club = '" . $session_away . "' AND round = " . $round . " AND season = '" . $season . "' AND team_grade = '" . $team_grade . "' AND year = " . $current_year;
  ...
  $sql_op = "Select * from tbl_scoresheet where team = '" . $session_away . "' AND round = " . $round . " AND season = '" . $season . "' AND year = " . $current_year . " AND team_grade = '" . $team_grade . "' Order By playing_position";
  ...
  while ($build_data_op = $result_op->fetch_assoc()) 
  {
    ...
    if($type == 'Billiards')
    {
      ...
      echo("document.getElementById('B_tier_" . $j . "').innerHTML = '" . addslashes($build_data_op['tier']) . "';");
    }
    else
    {
      echo("document.getElementById('B_player_" . $j . "').innerHTML = '" . addslashes($build_data_op['players_name']) . "';");
...
*/
    ...
    $.ajax({
      ...
      success : function(data){
        ...
        for (j = 0; j < 4; j++) 
        {
          ...
          //console.log("Obj_test home (page refresh) " + obj_test);
          if(obj_test[1] == 1)
          {
            $("input[id='home_ok']").prop("checked", true); 
          }
          ...
          {
            new_array +=  $('#A_player_' + j).val(obj_test[0]) + "" +
                          $('#A_tier_' + j).html($.trim(obj_test[10])) + "" +
        ...
        for (j = 0; j < 4; j++) 
        {
          ...
          //console.log("Obj_test away (page refresh) " + obj_test);
          if(obj_test[1] == 1)
          {
            $("input[id='away_ok']").prop("checked", true); 
          }
          ...
          {
              new_array +=  $('#B_player_' + j).val(obj_test[0]) + "" +
                            $('#B_tier_' + j).html(obj_test[10]) + "" +
                            $('#B_score_' + j).val($.trim(obj_test[2])) + "" +
                            $('#B_breaks_' + j).val($.trim(obj_test[6])) + "";
          }
        }
        if(type == 'Snooker')
        {
  ...
  $.fn.save_approval = function (team, opposition, venue, event) 
  {
    ...
    for(var j = 0; j < no_of_players; j++)
    {
      ...
      for (var i = 0; i < no_of_games; i++) 
      {
        ...
        else if(type == 'Billiards')
        {
          ...
          if((home_forfeit === true) || (away_forfeit === true))
          {
            scoredata_tier_calc[i] = 0 + "," + 0;
          }
          else
          {
    ...
    finalsdata = Awin + ", " + Bwin;

    forfeitdata = home_forfeit + ", " + away_forfeit;
...
<table class='table table-striped table-bordered dt-responsive nowrap display'>
  <tr>
   <td colspan=4 align="center"><b>Pennant Score Sheet Summary - <?= $current_year ?>. Season - <?= $season ?></b></td>
  </tr>
  <tr> 
...
$sql_home = "Select * from tbl_club_results where club = '" . $session_home . "' AND round = " . $round . " AND season = '" . $season . "' AND team_grade = '" . $team_grade . "' AND year = " . $current_year;
...
$sql_away = "Select * from tbl_club_results where club = '" . $session_away . "' AND round = " . $round . " AND season = '" . $season . "' AND team_grade = '" . $team_grade . "' AND year = " . $current_year;
...
{
?>
<table class='table table-striped table-bordered dt-responsive nowrap display' width='50px'>
<tr> 
  <td colspan=8 align=center><b>Multiplier Table</b></td> 
</tr> 
<tr> 
  <td class='text-center'><b>Tier Difference</b></td>
