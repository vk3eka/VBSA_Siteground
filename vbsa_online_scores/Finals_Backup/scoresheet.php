      ...
      <div class="x_panel">
        ...
        for($i = 0; $i < $no_of_players; $i++) // no of players
        {
          ...
          if(($i == 0) and ($type == 'Snooker')) // add 4th frame for finals if required
          {
            ...
            echo("<td align=center style='background-color: gray; color: white;'><input type='text' id='score_" . $i . "_3' name='score_" . $i . "_3' style='width:50px; height:25px'  class='form-control' " . $readonly . " OnChange='CheckInt();'></td>");
  ...
  $('#submit_approval').click(function(){
      ...
      url:"<?= $url ?>/check_two_teams.php?home=" + home_team + "&away=" + away_team + "&year=" + year + "&grade=" + team_grade + "&season=" + season + "&round=" + round,
       success : function(data){
        ...
        {
          $.ajax({
            url:"<?= $url ?>/check_scoresheet_change.php?home=" + home_team + "&away=" + away_team + "&year=" + year + "&grade=" + $('#team_grade').val() + "&season=" + season + "&round=" + $('#round').val(),
            method: 'GET',
            success : function(data){
              var obj = jQuery.parseJSON(data);
  ...
  $('#go_to_opposition').click(function(){
    ...
    $.ajax({
      ...
      success : function(data){
        ...
        {
          $.ajax({
            url:"<?= $url ?>/check_scoresheet_change.php?home=" + team_to_edit + "&away=" + opposition + "&year=" + year + "&grade=" + $('#team_grade').val() + "&season=" + season + "&round=" + $('#round').val(),
            method: 'GET',
            success : function(data){
              var obj = jQuery.parseJSON(data);
