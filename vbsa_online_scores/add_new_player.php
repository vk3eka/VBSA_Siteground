...
{
	// get next memberid
	$sql_em_player = "Insert into members (FirstName, LastName, Email, MobilePhone, entered_on, Gender) VALUES ('" . ucfirst($firstname) . "', '" . ucfirst($surname) . "', '" . $email . "', '" . $mobile . "', '" . date('Y-m-d') . "', '" . $gender . "')"; 
...
*/
	...
	{
		$sql_emergency = "Insert into tbl_emergency (MemberID, FirstName, LastName, Email, MobilePhone, entered_on, Team, AddedBy, Gender) VALUES (" . $memberID . ", '" . ucfirst($firstname) . "', '" . ucfirst($surname) . "', '" . $email . "', '" . $mobile . "', '" . date('Y-m-d') . "', '" . $clubname . "', '" . $user . "', '" . $gender . "')"; 
		...
		$message .= "<p>Played Previously: - " . $previous_text . ".</p>";
...
{
	// send email to scores director
	$subject = 'VBSA New Player';  
	$message = '<html><body>';
	$message .= "<p>Members Registrar</p>";
	$message .= "<p>&nbsp;</p>";
	$message .= "<p>A new player has been added to the team.</p>";
	$message .= "<p>Date Added: " . date("F j, Y") . ".</p>";
	$message .= "<p>Name of player: -  " . $firstname . " " . stripslashes($surname) . "</p>";
	$message .= "<p>Playing for: - " . $clubname . ".</p>";
	$message .= "<p>Team Grade: - " . $team_grade . ".</p>";
...
if($num_rows_check == 0)
{
	// find allocated_rp to add to scrs table.
	$sql_rp = "Select grade, RP from Team_grade where grade = '" . $team_grade . "' and current = 'Yes' and fix_cal_year = " . $current_year;
	//echo("Sql RP - " . $sql_rp . "<br>");
	$result_rp = $dbcnx_client->query($sql_rp) or die("Couldn't execute team query. " . mysqli_error($dbcnx_client));
	$build_rp = $result_rp->fetch_assoc();
	$rp = $build_rp['RP'];

	$sql = "Insert into scrs (team_id, MemberID, team_grade, allocated_rp, game_type, scr_season, current_year_scrs, count_played, pts_won, percent_won, total_RP, maxpts, average_position, captain_scrs, fin_year_scrs) 
Values (" . $teamID . ", " . $memberID . ", '" . $team_grade . "', '" . $rp . "', '" . $type . "', '" . $season. "', " . $current_year . ", 0, 0, 0.00, 0, 3, 0.00, 0, " . $current_year . ")";
}
else
{
