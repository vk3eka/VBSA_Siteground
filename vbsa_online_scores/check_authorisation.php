...
$sql_authorise = "Select * from tbl_authorise left join Team_entries on team_name = Team_1 where PlayerNo = " . $player_id;
//echo("SQL " . $sql_authorise . "<br>");
...
if($num_rows_authorise > 0) 
{
  ...
  if($row['Password'] != '')
  {
    ...
    $caption = $row['Name'] . ' already has a login for ' . $teams . '.\n' . $team . ' will be added to their login.';
    echo($caption);
    return;
  }
  else
