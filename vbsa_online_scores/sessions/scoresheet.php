...
while ($build_data = $result_scoresheet->fetch_assoc()) 
{
  ...
  {   //number_format(2.10, 1)
    ...
    {
      echo("document.getElementById('score_" . $j . "_" . $i . "').value = '" . number_format(-1.0, 1) . "';");
    }
    else
    {
  ...
  <div class="">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        ...
        $sql_approve = "Select * from tbl_scoresheet where team = '" . $team . "' AND round = " . $round . " AND season = '" . $season . "' AND year = " . $current_year . " and team_grade = '" . $grade . "' Order By playing_position";
        ...
        $sql_approve_away = "Select * from tbl_scoresheet where team = '" . $opposition . "' AND round = " . $round . " AND season = '" . $season . "' AND year = " . $current_year . " and team_grade = '" . $grade . "' Order By playing_position";
...
$(document).ready(function()
{
  ...
  $('#savebutton').click(function()
  {
    ...
    for(var j = 0; j < no_of_players; j++)
    {
      ...
      {
        //alert($('#lastname_' + j).val());
        //alert($('#member_id_' + j).val());
        /*if($('#player_' + j).val().indexOf("'") >= 0)
        {
          //alert($('#player_' + j).val());
          //alert($('#firstname_' + j).val());
          //alert($('#lastname_' + j).val());
          var player_name = $('#player_' + j).val().replace("'", "''");
          var name_array = $('#player_' + j).val().split(" ");
          var firstname = name_array[0];
          var lastname = name_array[1];
          //alert(namearray[1]);
        }
