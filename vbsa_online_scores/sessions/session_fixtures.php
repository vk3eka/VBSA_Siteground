...
<table class='table table-striped dt-responsive nowrap display' width='100%'>
  ...
  <tr>
    <td>&nbsp;</td>
    <td align="center">Select Fixture for <?php echo($season); ?> in <?php echo($year); ?></td>
    <td>&nbsp;</td>
  </tr>
  <tr>
    <td>&nbsp;</td>
...
if (isset($_POST['ButtonName']) && ($_POST['ButtonName'] == 'clear_results'))
{
    ...
    $sql_breaks = "Delete From breaks where recvd  = '" . $date . "' AND season = '" . $current_season . "' and grade = '" . $team_grade . "'";             
...
if($num_rows > 0)
{
    ...
    while ($build_data = $result_fixture->fetch_assoc()) 
    {
      ...
      echo("document.getElementById('round" . ($i+1) . "_date').innerHTML = '" . ($build_data['date']) . "';");
...
echo("<center>");
echo("<form name='fixture_select' method='post' action='session_fixtures.php'>");
