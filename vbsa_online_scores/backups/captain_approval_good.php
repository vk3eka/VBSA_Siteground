  ...
  while($build_data = $result->fetch_assoc()) 
  {
    ...
    {   
      for($i = 0; $i < $no_of_games; $i++)
      {
        ...
        $row_arr = explode(" ", $build_data['break_' . ($i+1)]);
  ...
  while ($build_data_op = $result_op->fetch_assoc()) 
  {
    ...
    {   
      if($type == 'Snooker')
      {
        ...
        $row_arr = explode(" ", $build_data_op['break_' . ($i+1)]);
        $row_size = sizeof($row_arr);
        echo("document.getElementById('B_breaks_" . $j . "_" . $i . "').rows = " . $row_size . ";");
...
$(document).ready(function()
{
  $.fn.pagerefresh = function (type) {
    ...
    var team_grade = '<?= $team_grade ?>';
    var type = $('#type').val();
    var new_array_home = new Array;
    var new_array_away = new Array;
    var obj_test;
    new_array_home = '';
    new_array_away = '';
    $.ajax({
      ...
      success : function(data){
        ...
        for (j = 0; j < 4; j++) 
        {
          var obj_test = obj[j].split(",")
          //console.log('Obj ' + obj);
          //console.log('Obj_test ' + obj_test[0] + ', ' + obj_test[1] + ', ' + obj_test[2]);
          if(type == 'Snooker')
          {
            new_array_home +=  $('#A_player_' + j).val(obj_test[0]) + "";
            for (i = 0; i < 3; i++) 
            {
              new_array_home +=  $('#A_score_' + j + '_' + i).val(obj_test[i+1]) + "" +
                            $.trim($('#A_breaks_' + j + '_' + i).val(obj_test[i+4]));
            }
            new_array_home += ",";
          }
          ...
          {
            new_array_home +=  $('#A_player_' + j).val(obj_test[0]) + "" +
                          $('#A_score_' + j).val(obj_test[1]) + "" +
                          $.trim($('#A_breaks_' + j).val(obj_test[2])) + "";
          }
          //console.log('Home ' + new_array_home);
        }
        if(type == "Billiards")
        {
          new_array_home += ",";
        ...
        for (j = 0; j < 4; j++) 
        {
          if(type == 'Snooker')
          {
            new_array_away +=  $('#B_player_' + j).html() + "";
            for (i = 0; i < 3; i++) 
            {
              new_array_away +=  $('#B_score_' + j + '_' + i).val() + "" +
        ...
        return new_array_home + "" + new_array_away;
      },
      error: function (data) {
        //alert(data);
...
<div> 
  <div class='text-center'>
    <td colspan=8 align='center'><a class='btn btn-primary btn-xs' id='test_reload' style='width:300px'>Temp Reload</a>
  </div>
</div>--> 
