...
/*
$.fn.edit_team = function (team) {
  if(team == '<?php echo($_SESSION['home']); ?>')
  {
    team = '<?php echo($_SESSION['home']); ?>';
    opposition = '<?php echo($_SESSION['away']); ?>';
  }
  else if(team == '<?php echo($_SESSION['away']); ?>')
  {
    team = '<?php echo($_SESSION['away']); ?>';
    opposition = '<?php echo($_SESSION['home']); ?>';
  }
  var Year = <?php echo($current_season); ?>; 
  var Grade = '<?php echo($_POST['Grade']); ?>'; 
  var FixtureDate = '<?php echo($_POST['DatePlayed']); ?>'; 
  var Season = '<?= $season ?>'; 
  var RoundNo = <?= $round ?>; 
  var TeamGrade = '<?= $team_grade ?>'; 
  var TeamScoring = '<?php echo($_POST['TeamScoring']); ?>'; 
  var Type = '<?= $type ?>'; 
  var DatePlayed = '<?= $_POST['DatePlayed'] ?>'; 
  var TeamtoEdit = team; 
  var OppositiontoEdit = opposition; 
  var Test = 'Test'; 

  $.ajax({
    url:"<?= $url ?>/get_approval_status.php?Round=" + RoundNo + "&Year=" + Year + "&Season=" + Season + "&DatePlayed=" + DatePlayed + "&TeamGrade=" + TeamGrade + "&Type=" + Type + "&Home=" + team + "&Away=" + opposition,
    method: 'POST',
    success:function(approval)
    {
      ...
      {
        alert('Both Approvals now recorded.\nNo further editing possible.\nContact the Scores Registrar if changes need to be made');
...
*/
  ...
  //if(($('#B_player_0').html().substr(0,12) == 'Team Forfeit') || ($('#B_player_0').html().substr(0,12) == 'Team Forfeit'))

    //alert($('#B_player_0').html());
  ...
  $.fn.getcurrentarray = function (type) {
    ...
    if(($('#B_player_0').html().substr(0,12) == 'Team Forfeit') || ($('#B_player_0').html().substr(0,12) == 'Team Forfeit'))
    {
      new_array = new_array.substring(0, new_array.length-3);
    }
    else
    {
    ...
    $.ajax({
      ...
      success : function(data){
        ...
        if(type == 'Snooker')
        {
          ...
          {
            console.log("Snooker Score Matchups Split " + x + " " + obj[x].split(","));
        ...
        for(x = 0; x < 5; x++) // 4 matches and 1 results = 5
        {
          console.log("Score Matchups Split (Billiards) " + x + " " + obj[x].split(","));
          obj_test = obj[x].split(",");
          for(i = 0; i < obj_test.length; i++)
          {
            console.log("Score Test Split " + i + " " + obj_test[i]);
            //console.log("Obj Test 0 " + obj_test[0]);
            ...
            {
              console.log("Obj Test 0 " + i + " " + obj_test[0]);
              console.log("Obj Test 1 " + i + " " + obj_test[1]);
              if(obj_test[1] == 1)
              {
  ...
  while($build_data = $result->fetch_assoc()) 
  {
    ...
    {   
      for($i = 0; $i < $no_of_games; $i++)
      {
        ...
        echo("document.getElementById('A_breaks_" . $j . "_" . $i . "').style='width:" . ($row_size*30) . "px; height:20px';");
        /*
        if($build_data['players_name'] == "Player Forfeit")
        {
           echo("document.getElementById('A_score_" . $j . "_" . $i . "').value = '0.0';");
           echo("document.getElementById('B_score_" . $j . "_" . $i . "').value = '1.0';");
        }
        */
      }
    }
  ...
  while ($build_data_op = $result_op->fetch_assoc()) 
  {
    ...
    {   
      for($i = 0; $i < $no_of_games; $i++)
      {
        ...
        echo("document.getElementById('B_breaks_" . $j . "_" . $i . "').style='width:" . ($row_size*30) . "px; height:20px';");
        /*
        if($build_data_op['players_name'] == "Player Forfeit")
        {
           echo("document.getElementById('B_score_" . $j . "_" . $i . "').value = '0.0';");
           echo("document.getElementById('A_score_" . $j . "_" . $i . "').value = '1.0';");
        }
        */
      }
    }
...
$(document).ready(function()
{
  ...
  $.fn.save_approval = function (team, opposition, venue) 
  {
    ...
    opposition = '<?= $session_away ?>';
    $.ajax({
      url:"<?= $url ?>/save_approvals.php?Home_Approve=" + home_ok + "&Away_Approve=" + away_ok + "&Round=" + Round + "&Year=" + Year + "&Season=" + Season + "&DatePlayed=" + DatePlayed + "&TeamGrade=" + TeamGrade + "&Type=" + Type + "&Home=" + team + "&Away=" + opposition + "&GamesWonHome=" + GamesWonHome + "&GamesWonAway=" + GamesWonAway + "&GamesDrawHome=" + GamesDrawHome + "&GamesDrawAway=" + GamesDrawAway + "&Venue=" + venue+ "&PackedData=" + PackedData + "&RoundTitle=" + RoundTitle + "&Finals=" + Finals,
      method: 'POST',
      success:function(response)
      {
        $.ajax({
...
elseif($type == 'Billiards')
{
  ...
  for($i = 0; $i < $no_of_players; $i++)
  {
    ...
    echo("<td class='A_Team_" . $i . "' align=center><input type='text' id='A_score_" . $i . "' style='width:50px; 
      height:20px; font-size:12px;' readonly><br><br>");
    echo("<textarea id='A_breaks_" . $i . "' rows=6 style='width:50px; font-size:12px;' readonly></textarea></td>");
    ...
    echo("<textarea id='B_breaks_" . $i . "' rows=6 style='width:50px; height:20px; font-size:12px;' readonly></textarea></td>");
    echo("</tr>");
  }
}
