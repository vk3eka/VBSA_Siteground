...
// added 7/9/2023... do i need it???
/*
f($num_rows_club_count == 0) //  changed from < 1
{
  $sql_club = "Insert into tbl_club_results (
  club, 
  team_grade,
...
// player forfeit save score in oppositions table
if($player == 'Player Forfeit')
{
  // check if opposition already saved.
  $sql_forfeit = "Select * from tbl_scoresheet where team = '" . $opposition . "' AND round = " . $round . " AND season = '" . $season . "' AND team_grade = '" . $team_grade . "' AND year = " . $current_year . " Order By playing_position";
  $result_count_forfeit = $dbcnx_client->query($sql_forfeit); 
  $num_rows_forfeit_count = $result_count_forfeit->num_rows;

  if($num_rows_forfeit_count > 0)
  {
    while($build_forfeit = $result_count_forfeit->fetch_assoc())
    {
      $opp_player_4 = $build_forfeit['id']; // get player id for player in same position as player forfeit
    }
    if($type == 'Billiards')
    {
      $sql_player_forfeit = "Update tbl_scoresheet Set 
      score_1 = 250
      where id = " . $opp_player_4;  
    }
    elseif($type == 'Snooker')
    {
      $sql_player_forfeit = "Update tbl_scoresheet Set 
    ...
    $update = $dbcnx_client->query($sql_player_forfeit);
  }
}

// player forfeit save score in oppositions table
if($player == 'Team Forfeit')
{
  // check if opposition already saved.
  $sql_forfeit = "Select * from tbl_scoresheet where team = '" . $opposition . "' AND round = " . $round . " AND season = '" . $season . "' AND team_grade = '" . $team_grade . "' AND year = " . $current_year . " Order By playing_position";
  $result_count_forfeit = $dbcnx_client->query($sql_forfeit); 
