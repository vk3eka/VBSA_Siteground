...
//echo("Select * from tbl_scoresheet where team = '" . $team . "' AND round = " . $round . " AND season = '" . $season . "' AND year = " . $current_year . " AND team_grade = '" . $team_grade . "'  Order By playing_position");
...
$(document).ready(function()
{
  ...
  $.fn.getcurrentarray = function (type) {
    ...
    //new_array = new_array.substring(1, new_array.length);
    if(type == 'Snooker')
    {
      ...
      <div class="x_panel">
        ...
        for($i = 0; $i < $no_of_players; $i++) // no of players
        {
          ...
          for($j = 0; $j < $no_of_games; $j++) // no of games
          {
            ...
            echo("<td align=center><input type='text' id='score_" . $i . "_" . $j . "' name='score_" . $i . "_" . $j . "' style='width:50px' class='form-control input-sm' " . $readonly . " OnChange='CheckInt();' tabindex=" . $no_of_games . "></td>");
            echo("<td align=center><textarea id='breaks_" . $i . "_" . $j . "' name='breaks_" . $i . "_" . $j . "' rows=6  style='width:40px' class='form-control input-sm break' " . $readonly . " tabindex=" . ($no_of_games+3) . "></textarea></td>");
            //echo("<td align=center><input type='checkbox' id='forfeit_" . $i . "_" . $j . "'style='width:30px'></td>");
            echo("</tr>");
            echo("<tr>");
          }
          if(($i == 0) and ($type == 'Snooker')) // add 4th frame for finals if required
          {
            ...
            echo("<td align=center style='background-color: gray; color: white;'><input type='text' id='score_" . $i . "_3' name='score_" . $i . "_3' style='width:50px' class='form-control input-sm' " . $readonly . " OnChange='CheckInt();'></td>");
            echo("<td align=center style='background-color: gray; color: white;'><input type='text' id='breaks_" . $i . "_3' name='breaks_" . $i . "_3' style='width:50px' class='form-control input-sm break' " . $readonly . "></td>");
            //echo("<td align=center><input type='checkbox' id='forfeit_" . $i . "_" . $j . "'style='width:30px'></td>");
            echo("</tr>");
          }
          else
...
<div> 
  <div class='text-center'>
    <td colspan=8 align='center'><a class='btn btn-primary btn-xs' id='test_reload' style='width:250px'>Test Reload</a>
  </div>
</div> 
...
// add selected player to the database
  $('#newplayer').click(function(event){
    ...
    else
    {
      ...
      var type = $('#type').val();
      $.ajax({
        url:"<?= $url ?>/add_new_player.php?clubname=" + team_name + "&firstname=" + firstname + "&surname=" + surname + "&team_grade=" + team_grade + "&type=" + type + "&year=" + year + "&season=" + season + "&fullname=" + new_player,
        method: 'POST',
        success:function(response)
        {
          $('#tags').val(''); 
...
*/
      ...
      */
      ...
      success : function(data)
      {
        ...
        if((data == 'true') || ('<?= $_SESSION['login_rights'] ?>' == 'Administrator'))
        {
          ...
          {
            if(confirm("You have unsaved data, do you wish to continue to the Approval Page?") == true)
            {
              $("#approval").submit();
            }
            else
  ...
  $('#savebutton').click(function()
  {
    ...
    var existingForm = $.fn.checkscores($('#type').val());
    /*
    $.ajax({
      url:"<?= $url ?>/check_scoresheet_change.php?home=" + team + "&away=" + opposition + "&year=" + year + "&grade=" + $('#team_grade').val() + "&season=" + season + "&round=" + $('#round').val(),
      method: 'GET',
      success : function(data){
...
*/
  ...
  $.fn.checkscores = function (type) {
    ...
    $.ajax({
      url:"<?= $url ?>/get_scoresheet.php?home=" + team + "&away=" + opposition + "&year=" + year + "&grade=" + team_grade + "&season=" + season + "&round=" + round,
      //dataType: 'json',
      success : function(data){
        var obj = jQuery.parseJSON(data);
        for (j = 0; j < 4; j++) 
        {
          ...
          if(type == 'Snooker')
          {
            ...
            for (i = 0; i < 3; i++) 
            {
              $('#score_' + j + '_' + i).val(obj_test[i+1]);
              $.trim($('#breaks_' + j + '_' + i).val(obj_test[i+4]));
            }
          }
          else if(type == 'Billiards')
          {
            $('#player_' + j).val(obj_test[0]);
            $('#score_' + j).val(obj_test[1]);
            $('#breaks_' + j).val(obj_test[2]);
          }
        }
      },
      error: function (data) {
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        <table class='table table-striped table-bordered dt-responsive nowrap display' width='100%'>
          <tr>
            <td>Firstname:</td><td><input type='text' id='em_first' style='width:200px; text-transform: capitalize;'></td>
          <tr>
          </tr>
