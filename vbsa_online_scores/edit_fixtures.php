...
function SaveFixtures() 
{
    ...
    //alert(team_grade.substring(2, 3));
...
function PopulateCalendar() 
{
    ...
    for(i = 0; i < no_of_rounds; i++)
    {
        myCalendar = new dhtmlXCalendarObject("round" + (i+1) + "_date");
...
if ($_POST['ButtonName'] == "SaveFixtures") 
{
    ...
    for ($i = 0; $i < count($scoredata); $i++) 
    {
        ...
        elseif($k == 5)
        {
            ...
            $update = $dbcnx_client->query($sql_update_5);
        }
        /*elseif($k == 6)
        {
...
if($num_rows > 0)
{
    ...
    while ($build_fixture_data = $result_fixture->fetch_assoc()) 
    {
      ...
      {
        if($build_fixture_data["fix" . ($j+1) . "home"] == '')
        {
            $home_round = 'NA';
        }
        else
        {
            $home_round = $build_fixture_data["fix" . ($j+1) . "home"];
        }
        if($build_fixture_data["fix" . ($j+1) . "away"] == '')
        {
            $away_round = 'NA';
        }
        else
        {
            $away_round = $build_fixture_data["fix" . ($j+1) . "away"];
        }
        echo("document.getElementById('round" . ($i+1) . "_fix" . ($j+1) . "_home').value = '" . $home_round . "';");
        echo("document.getElementById('round" . ($i+1) . "_fix" . ($j+1) . "_away').value = '" . $away_round . "';");
      }
      $i++;
    }
    echo("}");
