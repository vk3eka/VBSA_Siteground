...
$sql_grades = "Select * From Team_grade Where grade = '" . $team_grade . "' AND season = '" . $season . "' and current = 'Yes' and fix_cal_year = " . $current_year;
//echo("Players SQL " . $sql_grades . "<br>");
...
//echo("Players DT " . $no_of_players . "<br>");
...
while ($build_data = $result_scoresheet->fetch_assoc()) 
{
  ...
  echo("document.getElementById('tier_" . $j . "').value = '" . $build_data['tier'] . "';");
  
  if($build_data['type'] == 'Billiards')
  {
    echo("document.getElementById('billiard_stick_" . $j . "').value = '" . $build_data['billiard_stick'] . "';");
  }
  elseif($build_data['type'] == 'Snooker')
  {
    for ($i = 0; $i < $no_of_games; $i++) 
    {   
      if($build_data['score_' . ($i+1)] > 0)
      {
        echo("document.getElementById('score_" . $j . "_" . $i . "').value = '" . intval($build_data['score_' . ($i+1)]) . "';");
        //echo("document.getElementById('score_" . $j . "_" . $i . "').value = '" . number_format($build_data['score_' . ($i+1)], 2) . "';");
      }
      else
      {
        echo("document.getElementById('score_" . $j . "_" . $i . "').value = 'Test';");
  ...
  {   
    echo("document.getElementById('score_0_3').value = '" . $build_data['score_4'] . "';");
...
function CheckInt()
{
  ...
  {
    if((score_input[i].type == "text") && ((score_input[i].id.substring(0,5) == 'score')))
    {
      ...
      else if(score_input[i].value == '')
      {
        score_input[i].value = '';
      }
      else
      {
        current_val = parseInt(score_input[i].value);
...
function CheckTier()
{
  ...
  for (var i = 0; i < score_input.length; i++) 
  {
    if((score_input[i].type == "text") && ((score_input[i].id.substring(0,4) == 'tier')))
    {
      if((score_input[i].value) > 6)
      {
        alert("Tier needs to be 6 or less");
        score_input[i].value = 0;
        return;
      }
    }
...
function CheckTier()
{
  ...
  for (var i = 0; i < score_input.length; i++) 
  {
    if((score_input[i].type == "text") && ((score_input[i].id.substring(0,4) == 'tier')))
    {
      if((score_input[i].value) > 12)
      {
        alert("Tier needs to be between -2 and 12");
        //alert("Tier needs to be between 0 and 12");
        score_input[i].value = 0;
        return;
      }
    }
  ...
  <div class="">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        ...
        //echo("Home " . $num_home . "<br>");
        //echo("Away " . $num_away . "<br>");
        ...
        <table class='table table-striped table-bordered dt-responsive nowrap display'>
          <tr>
            <td class='text-left' colspan='11'><a class='btn btn-primary btn-xs' id='add_player' <?php echo($disabled); ?><?php echo($saved_already); ?> >Add Players to this scoresheet</a></td>
          </tr>
          <?php
            if($type == 'Billiards')
            {
              $enter_caption = "Enter Scores & Breaks and Check Tiers (override if necessary) for ";
            }
            else
            {
              $enter_caption = "Enter Scores & Breaks for ";
            }
          ?>
          <tr> 
            <td colspan="4"><?= $enter_caption ?><?php echo($team); ?></td> <!-- change from 5 -->
          </tr>
          <tr> 
            ...
            if($type == 'Billiards')
            {
              ...
              echo("<td align='center'>Raw Score</td>");
            }
            else
            {
        ...
        for($i = 0; $i < $no_of_players; $i++) // no of players
        {
          ...
          if($type == 'Billiards') // tier
          {
            echo("<td style='width: 30px; valign='center'><input type='text' name='tier_" . $i . "' id='tier_" . $i . "' style='width:50px' onchange='CheckTier();' onfocusout='this.value = parseInt(this.value);' tabindex=" . $no_of_games . " value='0'></td>
              ");
            //echo("<td style='width: 30px; valign='center'><input type='text' name='tier_" . $i . "' id='tier_" . $i . "' style='width:50px' onFocus='this.setSelectionRange(0, this.value.length)' onchange='this.value = parseInt(this.value);' tabindex=" . $no_of_games . " value='0'></td>");
          }
          else
          {
            echo("<input type='hidden' id='tier_" . $i . "' name='tier_" . $i . "' value='0'>");
          }
          ...
          for($j = 0; $j < $no_of_games; $j++) // no of games
          {
            if($type == "Snooker")
            {
              ...
              echo("<td align=center><input type='text' id='score_" . $i . "_" . $j . "' name='score_" . $i . "_" . $j . "' style='width:50px' " . $readonly . " OnChange='CheckInt();' tabindex=" . $no_of_games . "></td>");
            }
            elseif($type == 'Billiards')
            {
              echo("<input type='hidden' id='result_" . $i . "_" . $j . "' name='result_" . $i . "_" . $j . "'>");
              echo("<td align=center><input type='text' id='billiard_stick_" . $i . "' name='billiard_stick_" . $i . "' style='width:50px' " . $readonly . " tabindex=" . $no_of_games . "  value='' onchange='this.value = parseInt(this.value);'></td>");
            } // changed value to '' from 0 for blank score entry
            echo("<td align=center><textarea id='breaks_" . $i . "_" . $j . "' name='breaks_" . $i . "_" . $j . "'  class='break' " . $readonly . " tabindex=" . ($no_of_games+3) . " rows='1' cols='5'></textarea></td>"); // added rows and cols 22/02/2024
            echo("</tr>");
            echo("<tr>");

          }
          // add 4th frame for finals player 1
...
<input type="hidden" name="TeamGrade" id="team_grade" value="<?php echo($_POST['TeamGrade']); ?>" />
<td class='text-center'>
   <a class='btn btn-primary btn-xs' id='submit_approval' style='width:150px; height:100px; padding: 35px 0; border: 2px solid black;' ><b>Go to Approval Page</b></a>
</td>
  </tr>
</table>
</form>
...
{
  /*
  // get minimum break for the tier
  $.fn.get_min_breaks = function (tier, grade) {
    //event.preventDefault();
    var min_break = 0;
      $.ajax({
        url:"<?= $url ?>/get_min_breaks.php",
        method: 'GET',
        data: {'grade':grade, 'tier':tier},
        success:function(data)
        {
          min_break = data;
          alert("Inside func " + min_break);
        },
        error: function() {
            alert('Error occured');
        }
  ...
  */
  ...
  $.fn.pagerefresh = function (type) {
    ...
    var title = '<?= $title ?>';
    $.ajax({
      url:"<?= $url ?>/get_scoresheet.php?home=" + team + "&away=" + opposition + "&year=" + year + "&grade=" + team_grade + "&season=" + season + "&round=" + round + "&title=" + title + "&type=" + type,
      success : function(data){
        ...
        for (j = 0; j < 4; j++) 
        {
          ...
          else if(type == 'Billiards')
          {
            ...
            for (i = 0; i < 3; i++) 
            {
              //console.log("Obj Data " + obj_test[i+4]);
              row_arr = obj_test[i+4].split(" ");
              row_size = row_arr.length;
              if(row_size  == 0)
              {
              ...
              $('#player_' + j).val(obj_test[0]);
              $('#billiard_stick_' + j).val($.trim(obj_test[i+1]));
              $('#breaks_' + j + '_' + i).attr('rows', row_size);
              $('#breaks_' + j + '_' + i).val($.trim(obj_test[i+4]));
  ...
  $.fn.displayplayers = function () {
    ...
    $.ajax({
      ...
      success : function(data){
        ...
        output += ("<th colspan='" + PlayerCount + "' style='text-align: center;'>Playing Position</th>");
        output += ("</tr>");
        output += ("<tr>");
        ...
        for(k = 0; k < obj.length; k++)
        {
          ...
          memberID = fullobj[2];
          team_id = fullobj[3];
          count_played = fullobj[4];
          final_sub = fullobj[5];
          tier  = fullobj[6];
          byes  = fullobj[7];
          total_rounds  = fullobj[8];
          //console.log("Obj " + total_rounds + ", " + byes + ", " + count_played + ", " + (total_rounds-byes));
          if(fullobj[2] != 1) // remove Bye
          {
            output += ("<tr>");
            //if(($row_individual['count_played']) >= ($total_rounds_available-$no_of_byes)*0.5)
            if(((count_played) < ((total_rounds-byes)*0.5)) && (fullobj[1] != 'Forfeit'))
            //if(((count_played-byes) < (total_rounds*0.5)) && (fullobj[1] != 'Forfeit'))
            //if((count_played < 4) && (fullobj[1] != 'Forfeit')) // or fullname is Player Forfeit or team forfeit..................
            {
              if((final_sub == 'Yes') || (final_sub == 1))
              {
                output += ("<td align='left' style='width:250px' id='fullname" + k + "' name='fullname" + k + "'>" + fullname + "</td>");
              }
              else
              {
                 output += ("<td align='left' style='width:250px; color:red;' id='fullname" + k + "' name='fullname" + k + "'>" + fullname + "</td>");
              }
            }
            else
            {
            ...
            {
              output += ("<input type='hidden' id='firstname" + k + "' name='firstname" + k + "' id='firstname" + k + "' value='" + fullobj[0] + "'>");
              ...
              output += ("<input type='hidden' id='tier" + k + "' name='tier" + k + "' value='" + fullobj[6] + "'>");

              output += ("<td align='center'><input type='radio' data-col='" + (i+1) + "' data-row='" + k + "' id='position_r" + k + "_c" + (i+1) + "' name='position" + k + "'></td>");
              //alert("Tier " + fullobj[6] + ", name " + fullobj[0] + " " + fullobj[1]);
            }
            output += ("</tr>");
          }
          var name = fullname;
        ...
        $("input[type='radio']").click(function(){
          ...
          if(($("input[id='position_r" + row + "_c" + col + "']").prop("checked", true)) &&  ($('#fullname' + row).css('color') == 'rgb(255, 0, 0)'))
          {
            if($('#round').val() >= 13)
            {
              alert("This is an Alert message only.\n\nPlayer " + $('#fullname' + row).html() + " is not qualified to play finals.\n\n If finals have commenced, contact the Scores Registrar to obtain permission to play.");
              //$("input[data-col='" + col + "']").prop("checked", false);
            }
            return;
          }
  ...
  $.fn.add_emergency = function () {
    ...
    var previous = $('#em_previous').is(":checked");
    ...
    {
      $.ajax({
        url:"<?= $url ?>/add_new_player.php?clubname=" + team_name + "&firstname=" + firstname + "&surname=" + surname + "&team_grade=" + team_grade + "&type=" + type + "&year=" + year + "&season=" + season + "&email=" + email + "&mobile=" + mobile + "&previous=" + previous + "&user=" + user + "&round=" + round + "&gender=" + gender,
        method: 'POST',
  ...
  $('#newplayer').click(function(event){
    ...
    $.ajax({
      ...
      success : function(data)
      {
        ...
        //console.log('Membership ID ' + player_obj[2]);
        if(player_obj[2] == '')
        {
/*
          // check if email or mobile exist
          alert("Membership ID does not exist, please try email address or mobile number.");

          $('#em_check_first').val(player_obj[0]);
          $('#em_check_last').val(player_obj[1]);
          $('#EmPlayerCheck').modal('show');
          $('#check_player').click(function (event) {
            event.preventDefault();
            email = $('#em_check_email').val();
            mobile = $('#em_check_mobile').val();
            $.ajax({
              url:"<?= $url ?>/check_player_email_mobile.php?fullname=" + new_player + "&email=" + email + "&mobile=" + mobile,
              success : function(result)
              {
                if(result == 'Not Existing')
                {
                  alert(new_player " is not a player, they will be added to the list");
                }
              }
            });
...
*/
        ...
        else
        {
          ...
          gender = player_obj[19];
          //alert("Gender " + player_obj[19]);
          var year = '<?php echo($_SESSION['year']); ?>';
          var season = '<?php echo($_SESSION['season']); ?>';
          var team_grade = $('#team_grade').val();
          var type = $('#type').val();
          var round = <?= $_POST['RoundNo'] ?>;
          $.ajax({
            url:"<?= $url ?>/add_new_player.php?clubname=" + team_name + "&firstname=" + firstname + "&surname=" + surname + "&team_grade=" + team_grade + "&type=" + type + "&year=" + year + "&season=" + season + "&round=" + round + "&gender=" + gender,
            method: 'POST',
            success:function(response)
            {
              $('#tags').val(''); 
...
for(let i = 1; i <= 8; i++)
{
  $("#brk" + i).focusout(function()
  {
    let val = $(this).val();
    if(type == 'Snooker')
    {
      if(($.isNumeric(val)) && (val >= min_break) && (val <= max_break))
      {
        if(val > 99)
        {
          alert("Are you sure the break is correct?");
        }
        total_breaks = val + " " + $('#all_breaks').val();
        $('#all_breaks').val(total_breaks);
      }
      else
      {
        if(val != "")
        {
          alert("Only numbers between " + min_break + " and " + max_break + " are allowed!");
          $(this).val('');
        }
      }
    }
    ...
    {
      if(($.isNumeric(val)) && ($("#brk" + i).val() >= min_break))
      {
        if(val > 99)
        {
          alert("Are you sure the break is correct?");
      ...
      else
      {
        if(val != "")
        {
          alert("Only numbers between " + min_break + " or greater are accepted!");
          $(this).val('');
        }
      }
    }
  ...
  {
    if(($.isNumeric($("#brk1").val())) && ($("#brk1").val() >= min_break) && ($("#brk1").val() <= max_break))
    {
      if($('#brk1').val() > 99)
      {
        alert("Are you sure the break is correct?");
  ...
  {
    if(($.isNumeric($("#brk2").val())) && ($("#brk2").val() >= min_break) && ($("#brk2").val() <= max_break))
    {
      if($('#brk2').val() > 99)
      {
        alert("Are you sure the break is correct?");
  ...
  {
    if(($.isNumeric($("#brk3").val())) && ($("#brk3").val() >= min_break) && ($("#brk3").val() <= max_break))
    {
      if($('#brk3').val() > 99)
      {
        alert("Are you sure the break is correct?");
  ...
  {
    if(($.isNumeric($("#brk4").val())) && ($("#brk4").val() >= min_break) && ($("#brk4").val() <= max_break))
    {
      if($('#brk4').val() > 99)
      {
        alert("Are you sure the break is correct?");
  ...
  {
    if(($.isNumeric($("#brk5").val())) && ($("#brk5").val() >= min_break) && ($("#brk5").val() <= max_break))
    {
      if($('#brk5').val() > 99)
      {
        alert("Are you sure the break is correct?");
  ...
  {
    if(($.isNumeric($("#brk6").val())) && ($("#brk6").val() >= min_break) && ($("#brk6").val() <= max_break))
    {
      if($('#brk6').val() > 99)
      {
        alert("Are you sure the break is correct?");
  ...
  $("#brk7").focusout(function()
  {
    if(($.isNumeric($("#brk7").val())) && ($("#brk7").val() >= min_break) && ($("#brk7").val() <= max_break))
    {
      if($('#brk7').val() > 99)
      {
        alert("Are you sure the break is correct?");
      }
      total_breaks = $('#brk7').val() + " " + $('#all_breaks').val();
      $('#all_breaks').val(total_breaks);
    }
    else
    {
  ...
  $("#brk8").focusout(function()
  {
    if(($.isNumeric($("#brk8").val())) && ($("#brk8").val() >= min_break) && ($("#brk8").val() <= max_break))
    {
      if($('#brk8').val() > 99)
      {
        alert("Are you sure the break is correct?");
      }
      total_breaks = $('#brk8').val() + " " + $('#all_breaks').val();
      $('#all_breaks').val(total_breaks);
    }
    else
    {
  ...
  */
  ...
  $('#new_break').click(function(event)
  {
    ...
    $('.break').each(function()
    {
      ...
      {
        $(this).val(box_value);
        total_breaks = $('#brk1').val() + " " + $('#brk2').val() + " " + $('#brk3').val() + " " + $('#brk4').val() + " " + $('#brk5').val() + " " + $('#brk6').val() + " " + $('#brk7').val() + " " + $('#brk8').val();
  ...
  $.fn.clearplayerscores = function () {
    ...
    for(j = 0; j < no_of_players; j++)
    {
      ...
      if(type == "Snooker")
      {
         // changed score/billiard_stick value to '' from 0 for blank score entry
        for(i = 0; i < 7; i++)
        {
          $("textarea[name='player_" + i + "']").val('');
          $("input[id='win_" + j + "_" + i  + "']").prop("checked", false);
          $("input[id='score_" + j + "_" + i  + "']").val('');
      ...
      else if(type == 'Billiards')
      {
        ...
        $("input[id='tier_" + j + "_0']").val('');
        $("input[id='billiard_stick_" + j + "_0']").val('');
      ...
      success : function(data){
        ...
        else
        {
          $.ajax({
            ...
            {
              alert(response);
              // disable "Add Players to Scoresheet" button if both teams entered.
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header ui-front">
                ...
                <h4 class="modal-title">Select from the Playing List of <?php echo($team); ?></h4>
            </div>
...
<!--
            <center>
              <table class='table'>
                <tr>
                  <td align='left'><b>Enter name in this field to add players:</b></td>
                  <td align='center'><input id='tags'></td>
                </tr>
                <div id='add_extra_params'></div>
                <tr>
                  <td align='left'><b>Enter Mobile No.:</b></td>
                  <td align='center'><input id='mobile'></td>
                </tr>
                <tr>
                  <td align='left'><b>Enter Email Address:</b></td>
                  <td align='center'><input id='email'></td>
                </tr>
              </table>
            </center>
-->           
              ...
              <br>
              <div><a class='btn btn-primary btn-xs' id='newplayer'>Add to Playing List</a>
              <br>
              <div></div>
              </div>
              ...
              <div id='add'></div>
              <br>
              <div class='text-center'><a class='btn btn-primary btn-xs' id='populate_positions'>Populate Players in Scoresheet</a>
              </div>
              <br>
              <div class='text-center'><i>This action will clear any existing scores.</i></div>
              <br>
              <br>
              <div class='text-center' style="color: red"><i><b>Players shown in RED are unqualified for finals.</b></i></div>
              <br>
              <div class='text-center' style="color: red"><i><b>You MUST contact the Scores Registrar to gain approval to play unqualified players.</b></i></div>
              <br>
            </div>
          ...
          <div class='text-center ui-widget'>
            ...
            <div>
              ...
              <table class='table table-striped table-bordered'  style='width:350px;'>
                ...
                <tr>
                  <td align='center'><input type='text' id='brk1' style='width:40px; height:20px'></td>
                  <td align='center'><input type='text' id='brk2' style='width:40px; height:20px'></td>
                  <td align='center'><input type='text' id='brk3' style='width:40px; height:20px'></td>
                  <td align='center'><input type='text' id='brk4' style='width:40px; height:20px'></td>
                </tr>
                <tr>
                ...
                <tr>
                  <td align='center'><input type='text' id='brk5' style='width:40px; height:20px'></td>
                  <td align='center'><input type='text' id='brk6' style='width:40px; height:20px'></td>
                  <td align='center'><input type='text' id='brk7' style='width:40px; height:20px'></td>
                  <td align='center'><input type='text' id='brk8' style='width:40px; height:20px'></td>
                </tr>
              </table>
            </center>
            </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        ...
        if($type == 'Billiards')
        {
          echo('<h4 class="modal-title">Player to Player Matchups (tiers must be saved first)</h4>');
        }
        else
        {
          echo('<h4 class="modal-title">Player to Player Matchups</h4>');
        }
        ?>
        <button type="button" class="close" data-dismiss="modal">&times;</button>
      </div>
  ...
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-body">
        ...
        <br>
        <div class='text-center'>You must now lodge an online VBSA Membership form for this new player.  See <a href='https://vbsa.org.au/Admin_DB_VBSA/membership_application_online.php' target="_blank">https://vbsa.org.au/Admin_DB_VBSA/membership_application.php.</a></div>
        <br>
        <br>
...
<!-- Non Current Player Check Modal -->
<div class="modal fade" id="EmPlayerCheck" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
      ...
      <div class="modal-body">
        <table class='table table-striped table-bordered dt-responsive nowrap display' width='100%'>
          <tr>
            <td>Firstname:</td><td><input type='text' id='em_check_first' style='width:200px; text-transform: capitalize;' placeholder="Firstname"></td>
          <tr>
          </tr>
            <td>Lastname:</td><td><input type='text' id='em_check_last' style='width:200px; text-transform: capitalize;'></td>
          <tr>
          </tr>
            <td>Email Address:</td><td><input type='text' id='em_check_email' style='width:200px;'></td>
          </tr>
          </tr>
            <td>Mobile No.:</td><td><input type='text' id='em_check_mobile' style='width:200px;'></td>
          </tr>
        </table>
        <br>
        <div class='text-center'><a class='btn btn-primary btn-xs' id='check_player'>Check Player Details</a></div>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
      </div>
