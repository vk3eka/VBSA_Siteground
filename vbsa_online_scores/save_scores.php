...
elseif($_GET['Type'] == "Billiards")
{
  for($i = 0; $i < sizeof($packedscoredata); $i++)
  {
    ...
    // get allocated ranking points for tier
    // check if data already entered
    
    $sql_tier = "Select * from Team_grade where grade = '" . $team_grade . "' AND season = '" . $season . "' AND fix_cal_year = " . $current_year;
    $result_tier = $dbcnx_client->query($sql_tier) or die("Couldn't execute query. " . mysqli_error($dbcnx_client));
    $build_data = $result_tier->fetch_assoc();
    if($score1[18] == -1)
    {
      $tier_rp = $build_data['tier_1_rp'];
    }
    else if($score1[18] == -2)
    {
      $tier_rp = $build_data['tier_2_rp'];
    }
    else
    {
      $tier_rp = $build_data['tier' . $score1[18] . '_rp'];
    }
