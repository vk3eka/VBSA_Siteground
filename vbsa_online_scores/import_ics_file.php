...
$icalfeed = file_get_contents("Victorian-public-holiday-dates.ics");
...
function ImportExcel($filename, $current_year, $season) 
{
    ...
    for ($row = 1; $row < $highestRow; $row++) {
        ...
        if($rowData[$row][1] != '') // stop before the last entry
        {
            ...
            $sql = "Insert INTO tbl_fixtures (date, type, grade, round, fix1home, fix1away, fix2home, fix2away, fix3home, fix3away, fix4home, fix4away, fix5home, fix5away, fix6home, fix6away, year, season, team_grade, dayplayed, title) Values ('" . date('Y-m-d', $unix_date) . "', '" . $rowData[$row][1] ."', '" . $rowData[$row][2] . "', " . $rowData[$row][3] .", '" . $rowData[$row][4] . "', '" . $rowData[$row][5] . "', '" . $rowData[$row][6] . "', '" . $rowData[$row][7] . "', '" . $rowData[$row][8] . "', '" . $rowData[$row][9] . "', '" . $rowData[$row][10] . "', '" . $rowData[$row][11] . "', '" . $rowData[$row][12] . "', '" . $rowData[$row][13] . "', '" . $rowData[$row][14] . "', '" . $rowData[$row][15] . "', " . $rowData[$row][16] . ", '" . $rowData[$row][17] . "', '" . $rowData[$row][18] . "', '" . $rowData[$row][19] . "', '" . $rowData[$row][20] . "')";
            //echo($sql . "<br>");
...
{
    echo("<form action='create_fixture_upload.php' method='post' enctype='multipart/form-data'>");
    echo("<center>");
    echo("<table border='0' align='center' cellpadding='0' cellspacing='0'>");
    echo("<tr>");
    echo("<td align=center><h2>Upload iCal File</h2></td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td align=center>&nbsp;</td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td align=center>Select the .ics File to upload:</td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td align=center>&nbsp;</td>");
    echo("</tr>");
    ...
    echo("</tr>");
    echo("<tr>");
    echo("<td align=center><b><font color=red>Please note: any existing dates data for season " . $season . ", in " . $current_year . " will be deleted.</b></font></td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td align=center>&nbsp;</td>");
    echo("</tr>");
