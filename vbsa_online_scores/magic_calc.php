...
Step 1
1 Average position calculate  
*/
$sql = "Update `vbsa3364_vbsa2`.`scrs` SET `total_position` =(IFNULL(scrs.r01pos,0)) +(IFNULL(scrs.r02pos,0)) +(IFNULL(scrs.r03pos,0)) +(IFNULL(scrs.r04pos,0)) +(IFNULL(scrs.r05pos,0)) +(IFNULL(scrs.r06pos,0)) +(IFNULL(scrs.r07pos,0)) +(IFNULL(scrs.r08pos,0)) +(IFNULL(scrs.r09pos,0)) +(IFNULL(scrs.r10pos,0)) +(IFNULL(scrs.r11pos,0)) +(IFNULL(scrs.r12pos,0)) +(IFNULL(scrs.r13pos,0)) +(IFNULL(scrs.r14pos,0)) +(IFNULL(scrs.r15pos,0)) +(IFNULL(scrs.r16pos,0)) +(IFNULL(scrs.r17pos,0)) +(IFNULL(scrs.r18pos,0)) WHERE current_year_scrs = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND scr_season = '$season'";
$update = $dbcnx_client->query($sql);
if(!$update)
{
  die("Could not update average position calculate 1: " . mysqli_error($dbcnx_client));
} 
/*
2 Average position calculate  
*/
$sql = "Update `vbsa3364_vbsa2`.`scrs` SET `count_position` =(SELECT COUNT(`r01pos`)+COUNT(`r02pos`)+COUNT(`r03pos`)+COUNT(`r04pos`)+COUNT(`r05pos`)+COUNT(`r06pos`)+COUNT(`r07pos`)+COUNT(`r08pos`)+COUNT(`r09pos`) +COUNT(`r10pos`)+COUNT(`r11pos`)+COUNT(`r12pos`)+COUNT(`r13pos`)+COUNT(`r14pos`)+COUNT(`r15pos`)+COUNT(`r16pos`)+COUNT(`r17pos`)+COUNT(`r18pos`)) WHERE current_year_scrs = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND scr_season = '$season'";
$update = $dbcnx_client->query($sql);
if(!$update)
{
  die("Could not update average position calculate 2: " . mysqli_error($dbcnx_client));
} 
/*
3 Average position calculate  
*/
$sql = "Update `vbsa3364_vbsa2`.`scrs` SET average_position=total_position/count_position WHERE current_year_scrs = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND scr_season = '$season'"; /// divide by zero error
//echo("SQL = " . $sql . "<br>");
$update = $dbcnx_client->query($sql);
if(!$update)
{
  die("Could not update average position calculate 3: " . mysqli_error($dbcnx_client));
}
...
*/
//$sql = "Update `vbsa3364_vbsa2`.`scrs` SET count_played =(SELECT COUNT(r01s)+COUNT(r02s)+COUNT(r03s)+COUNT(r04s)+COUNT(r05s)+COUNT(r06s)+COUNT(r07s)+COUNT(r08s)+COUNT(r09s) +COUNT(r10s)+COUNT(r11s)+COUNT(r12s)+COUNT(r13s)+COUNT(r14s)+COUNT(r15s)+COUNT(r16s)+COUNT(r17s)+COUNT(r18s)) WHERE current_year_scrs = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND scr_season = '$season'";

$sql = "Update `vbsa3364_vbsa2`.`scrs` SET count_played =(SELECT COUNT(r01s)+COUNT(r02s)+COUNT(r03s)+COUNT(r04s)+COUNT(r05s)) WHERE current_year_scrs = 2023 AND scr_season = 'S1'";
echo("1 " . $sql . "<br>");
$update = $dbcnx_client->query($sql);
if(!$update)
{
...
$sql = "Update `vbsa3364_vbsa2`.`scrs` SET count_played =(SELECT COUNT(r06s)+COUNT(r07s)+COUNT(r08s)+COUNT(r09s) +COUNT(r10s)+COUNT(r11s)+COUNT(r12s)+COUNT(r13s)+COUNT(r14s)+COUNT(r15s)+COUNT(r16s)+COUNT(r17s)+COUNT(r18s)) WHERE current_year_scrs = 2022 AND scr_season = 'S2'";
echo("2 " . $sql . "<br>");
$update = $dbcnx_client->query($sql);
if(!$update)
{
...
*/
$sql = "Update `vbsa3364_vbsa2`.`scrs` SET pts_won=0 WHERE (MemberID=1 OR MemberID=100 OR MemberID=1000) AND current_year_scrs = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND scr_season = '$season'";
$update = $dbcnx_client->query($sql);
if(!$update)
{
...
*/
$sql = "Update `vbsa3364_vbsa2`.`scrs` SET total_RP=allocated_rp*pts_won WHERE current_year_scrs = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND scr_season = '$season'"; // total_RP cannot be null
$update = $dbcnx_client->query($sql);
if(!$update)
{
  die("Could not update Set points won = 0 for Bye: " . mysqli_error($dbcnx_client));
...
*/
$sql = "Update vbsa3364_vbsa2.scrs SET avail_pts = (SELECT CASE WHEN game_type = 'Snooker' THEN count_played*3 WHEN game_type = 'Billiards' THEN count_played*2 ELSE 0 END) WHERE current_year_scrs = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND scr_season = '$season'";
$update = $dbcnx_client->query($sql);
if(!$update)
{
...
9 Player Percentage 
*/
$sql = "Update `vbsa3364_vbsa2`.`scrs` SET percent_won = pts_won/avail_pts*100 WHERE current_year_scrs = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND scr_season = '$season'"; // divide by zero
$update = $dbcnx_client->query($sql);
if(!$update)
{
...
*/
$sql = "Update vbsa3364_vbsa2.Team_entries T1 INNER JOIN ( SELECT team_id, SUM(scrs.r01s) as total FROM scrs GROUP BY team_id ) T2 ON T1.team_id = T2.team_id SET T1.T01 = T2.total WHERE team_cal_year = YEAR(CURDATE( )) AND team_grade = '$teamgrade' AND team_season = '$season'"; // for 18 rounds
$update = $dbcnx_client->query($sql);
if(!$update)
{
...
//update members totplayed_curr with count_playeed values from scrs

/*
Snooker Matches Played 
*/
$sql = "Update vbsa3364_vbsa2.members, (SELECT SUM(count_played) AS curr, scrs.MemberID FROM scrs WHERE current_year_scrs = YEAR(NOW( ) ) AND game_type='Snooker' GROUP BY scrs.MemberID) AS played SET members.totplayed_curr = played.curr WHERE members.MemberID = played.MemberID";
$update = $dbcnx_client->query($sql);
if(!$update)
{
...
*/
$sql = "Update vbsa3364_vbsa2.members, (SELECT SUM(count_played) AS currbill, scrs.MemberID FROM scrs WHERE current_year_scrs = YEAR(NOW( ) ) AND game_type='Billiards' GROUP BY scrs.MemberID) AS played SET members.totplaybill_curr = played.currbill WHERE members.MemberID = played.MemberID";
$update = $dbcnx_client->query($sql);
if(!$update)
{
