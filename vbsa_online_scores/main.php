...
//echo($season . "<br>");
//echo($date . "<br>");
//$sql_fix_date = "Select date, season, round FROM tbl_fixtures WHERE Year = " . $year . " and season = '" . $season . "' order by date desc Limit 1";

$sql_fix_round = "Select max(round) as round, date FROM tbl_fixtures WHERE Year = " . $year . " and season = 'S1'";
//echo($sql_fix_round . "<br>");

$result_fix_round = $dbcnx_client->query($sql_fix_round);
$row_fix_round = $result_fix_round->fetch_assoc();
//echo($row_fix_round['round'] . "<br>");

$sql_fix_date = "Select date, season, round FROM tbl_fixtures WHERE Year = " . $year . " and round = '" . $row_fix_round['round'] . "' Limit 1";
$result_fix_date = $dbcnx_client->query($sql_fix_date);
$row_fix_date = $result_fix_date->fetch_assoc();

//echo($row_fix_date['date'] . "<br>");
$fix_date = date('Y-m-d', strtotime($row_fix_date['date'] . ' + 1 days'));
if(($fix_date < $date)) 
{
  $season = 'S2';
}
...
if((($password) <> "") && (($username) <> "")) 
{
  ...
  { 
    if(($row["Password"] <> null) && (password_verify($password, trim($row["Password"]))))
    {
      ...
      $caption = "Please make a selection from the top menu.<br><br><br><i>Current year  " . $year . ", season " . $season;
...
<div class="">
    ...
    <div class="col-md-12 col-sm-12 col-xs-12">
      <div class="x_panel">
        ...
        <div class="x_content"> 
			    <div>
			      <div align="center" colspan="2"><?= $logincaption ?><br><br></div>
            <div align="center" colspan="2"><?= $administercaption ?><br><br><?= $caption ?></div>
			    </div>
			  </div>
     </div>
    </div>
