...
$sql_grades = "Select * From Team_grade Where grade = '" . $team_grade . "' AND fix_cal_year = " . $current_year . " AND season = '" . $current_season . "' AND current = 'Yes'";
...
//echo("Season " . $season . ", Year " . $current_year . "<br>");
//$season = 'S2';
//$current_year = 2023;
...
foreach($fixture_array as $fixture_no)
{
    ...
    if($build_fix[$fixture_no] != '')
    {
        ...
        if($build_fix[$fixture_no] == 'Bye')
        {
            ...
            // get home team data
            $sql_home_team = "Select team_id, team_club_id, team_club, day_played, comptype from Team_entries where team_name = '" . $home_team . "' and team_grade = '" . $teamgrade . "' and team_cal_year = " . $current_year . " and team_season = '" . $season . "' Order By team_id DESC LIMIT 1";
            ...
            // check if 'Bye' exists, if not add it...........................
            $sql_away_team = "Select team_id, team_club_id, team_club from Team_entries where team_name = '" . $away_team . "' and team_grade = '" . $teamgrade . "' and team_season = '" . $season . "' AND team_cal_year = " . $current_year . " Order By team_id DESC LIMIT 1";
            ...
            $sql_check_home = "Select * from scrs where memberID = 1 and team_grade = '" . $teamgrade . "' and team_id = " . $home_team_id . " and current_year_scrs = " . $current_year . " and scr_season = '" . $season . "'";
            ...
            if($num_rows_home == 0)
            {
                ...
                //echo("Sql Insert SCRS (Home) " . $sql_home_scrs . "<br>");
                $update = $dbcnx_client->query($sql_home_scrs);
            }
            
            // check if away team already entered....// added 6/2/2024

            $insertSQL = "Insert INTO Team_entries (team_id, team_name, team_grade, team_season, day_played, players, Final5, include_draw, audited, team_cal_year, comptype) VALUES (" . $away_team_id . ",'Bye', '" . $teamgrade . "', '" . $season . "', '" . $build_home_team['day_played'] . "', 4, 4, 'Yes', 'No', " . $current_year . ", '" . $build_home_team['comptype'] . "'";
            //echo("Bye Team Insert " . $insertSQL . "<br>");
            
            if($away_team_id == '')
            {
                echo("No Team Bye exists!<br>");
                $num_rows_away = 0;
            }
            else
            {
                $sql_check_away = "Select * from scrs where memberID = 1 and team_grade = '" . $teamgrade . "' and team_id = " . $away_team_id . " and current_year_scrs = " . $current_year . " and scr_season = '" . $season . "'";
            ...
            if($num_rows_away == 0)
            {
                ...
                //echo("Sql Insert SCRS (Away) " . $sql_away_scrs . "<br>");
            ...
            $sql_scrs = "Select * FROM scrs where current_year_scrs = " . $current_year. " and team_grade = '" . $teamgrade . "' and MemberID = 1 and team_id = " . $away_team_id  . " and scr_season = '" . $season . "'";
            ...
            //echo("Home Totals " . $build_home_total['team_total'] . ", Team " . $home_team . "<br>");

            // added ($y+1) vis $y 14/10/2025 to correct result position for a home bye.
            ...
            HB = '',
            audited = '" . $audited . "'
            where team_id = " . $home_team_id . " and team_grade='" . $teamgrade . "' and team_season = '" . $season . "' AND team_cal_year = " . $current_year;  
            ...
            $sql_away_total = "Select SUM(IFNULL(scrs.r01s,0)) + SUM(IFNULL(scrs.r02s,0)) + SUM(IFNULL(scrs.r03s,0)) + SUM(IFNULL(scrs.r04s,0)) + SUM(IFNULL(scrs.r05s,0)) + SUM(IFNULL(scrs.r06s,0)) + SUM(IFNULL(scrs.r07s,0)) + SUM(IFNULL(scrs.r08s,0)) + SUM(IFNULL(scrs.r09s,0)) + SUM(IFNULL(scrs.r10s,0)) + SUM(IFNULL(scrs.r11s,0)) + SUM(IFNULL(scrs.r12s,0)) + SUM(IFNULL(scrs.r13s,0)) + SUM(IFNULL(scrs.r14s,0)) + SUM(IFNULL(scrs.r15s,0)) + SUM(IFNULL(scrs.r16s,0)) + SUM(IFNULL(scrs.r17s,0)) + SUM(IFNULL(scrs.r18s,0)) AS team_total FROM scrs WHERE team_id = " . $away_team_id . "  AND scr_season = '" . $season . "' AND current_year_scrs = " . $current_year;

             //  . "'  AND season = '" . $season . "' AND year = " . $current_year;

            //echo("Away Totals " . $sql_away_total . "<br>");
            ...
            //echo("Away Totals " . $build_away_total['team_total'] . ", Team " . $away_team . "<br>");
            ...
            HB = '',
            audited = '" . $audited . "'
            where team_id = " . $away_team_id . " and team_grade='" . $teamgrade . "' and team_season = '" . $season . "' AND team_cal_year = " . $current_year;  
...
// added for home team Bye.............//

        //echo("Team to Process " . $build_fix[$fixture_no] . "<br>");
        //echo("Home Team (Bye) " . $home_team . "<br>");
        //echo("Away Team (Bye) " . $away_team . "<br>");
        // do not update Bye Home Team (already done)
        if($home_team == 'Bye')
        {
            $team_to_miss = $away_team;
        }
        else if($away_team == 'Bye')
        {
        ...
        if(($build_fix[$fixture_no] != $team_to_miss) && ($build_fix[$fixture_no] != 'Bye'))
...
// .........................  //
        ...
        {
            // get team data
            $sql_team = "Select team_id, team_club_id, team_club from Team_entries where team_name = '" . $build_fix[$fixture_no] . "' and team_grade = '" . $teamgrade . "' AND team_season = '" . $season . "' AND team_cal_year = " . $current_year . " Order By team_id DESC LIMIT 1";
            ...
            // added 8/10/2024
            // code to remove any previous finals scores from current season in scrs table.
            //$querytoexecute = "Update scrs set EF1 = null, EF2 = null, SF1 = null, SF2 = null, GF = null, EF1_pos = null, EF2_pos = null, SF1_pos = null, SF2_pos = null, GF_pos = null where game_type = 'Billiards' and current_year_scrs = " . $current_year . " and scr_season = '" . $season . "' AND team_grade = '" . $teamgrade . "'";
            //$update = $dbcnx_client->query($querytoexecute);
            //echo("Finals Scrs (remove existing finals) Update - " . $querytoexecute . "<br>");

            // get result position from fixture list
            $result_position = ($y+1);
            //echo("Round No " . $rnd_no . ", Position No " . $result_position . ", Team " . $team_club . "<br>");
            ...
            while($build_data = $result_count_players->fetch_assoc())
            {
                ...
                {
                    // added 8/09/2024
                    if($title == 'Elimination Final')
                    {
                        if($type == 'Billiards')
                        {
                            /*
                             // good for 3/6 4/5
                            if(($result_position == 1) OR ($result_position == 4))
                            {
                                $semi_no = 1;
                            }
                            elseif(($result_position == 2) OR ($result_position == 3))
                            {
                                $semi_no = 2;
                            }
                            */
                            ...
                            // good for 3/5 4/6
                            if(($result_position == 1) OR ($result_position == 2))
                            {
                                $semi_no = 1;
                            }
                            elseif(($result_position == 3) OR ($result_position == 4))
                            {
                                $semi_no = 2;
                            }
                            // elimination final
                            $sql_finals_scrs_update_bill = "Update scrs Set EF" . $semi_no . " = " . (($build_data['win_1']*2)+($build_data['draw_1']*2)) . ", EF" . $semi_no . "_pos = " . $build_data['playing_position'] . " WHERE scrsID = " . $build_scrs['scrsID'];
                            //echo("Finals Scrs (EF Billiards) Update - " . $sql_finals_scrs_update_bill . ", Team ID " . $team_id . "<br>");
                    ...
                    {              
                        ...
                        elseif($type == 'Billiards')
                        {
                            // add tier and ranking point to scrs table
                            $RP = $build_data['tier_rp'];
                            if($build_data['draw_1'] == 1)
                            {
                                $RP = ($build_data['tier_rp']/2);
                            }
                            else if($build_data['win_1'] == 1)
                            {
                                $RP = ($build_data['tier_rp']*($build_data['win_1']));
                            }
                            else
                            {
                                $RP = 0;
                            ...
                            //echo($build_data['tier_rp'] . "<br>");
                            //echo($build_data['win_1'] . "<br>");

                            $sql_scores = "Update scrs Set 
                            r" . $rnd_no . "s = " . (($build_data['win_1']*2)+($build_data['draw_1'])) . ", 
                            r" . $rnd_no . "pos = " . $build_data['playing_position'] . ", tier =  " . $build_data['tier'] . ", tier_rp =  " . $build_data['tier_rp'] . ", tier_r" . $rnd_no . "_rp =  " . $RP . " where scrsID = " . $build_scrs['scrsID'];
                            
/*
                            $sql_scores = "Update scrs Set 
                            r" . $rnd_no . "s = " . (($build_data['win_1']*2)+($build_data['draw_1'])) . ", 
                            r" . $rnd_no . "pos = " . $build_data['playing_position'] . ", tier =  " . $build_data['tier'] . ", tier_rp =  " . $build_data['tier_rp'] . ", tier_r" . $rnd_no . " =  " . $build_data['tier'] . ", tier_r" . $rnd_no . "_rp =  " . $build_data['tier_rp'] . "
                            where scrsID = " . $build_scrs['scrsID'];
*/                            
                        }
                        //echo("Title " . $title . "<br>");
            ...
            //echo("Break Title " . $title . "<br>");
            if(($title == 'Elimination Final') || ($title == 'Semi Final') || ($title == 'Grand Final'))
            {
                $finalsBrks = 'Yes';
            }
            else
            ...
            while($build_breaks_data = $result_breaks->fetch_assoc())
            {
                if($teamID > 0)
                {
                    ...
                    */
                    // add multiple breaks to snooker 7/9/2024

                    if($build_breaks_data['type'] == 'Billiards')
                    {
                        $no_of_frames = 1;
                    }
                    else if($build_breaks_data['type'] == 'Snooker')
                    {
                        $no_of_frames = 3;
                    }
                    for($k = 0; $k < $no_of_frames; $k++) // number of frames in snooker/billiards
            ...
            if(($num_rows_team_data == 1) && ($build_team_data['round'] == $round))
            {
                ...
                $sql_update_total = "Select SUM(IFNULL(scrs.r01s,0)) + SUM(IFNULL(scrs.r02s,0)) + SUM(IFNULL(scrs.r03s,0)) + SUM(IFNULL(scrs.r04s,0)) + SUM(IFNULL(scrs.r05s,0)) + SUM(IFNULL(scrs.r06s,0)) + SUM(IFNULL(scrs.r07s,0)) + SUM(IFNULL(scrs.r08s,0)) + SUM(IFNULL(scrs.r09s,0)) + SUM(IFNULL(scrs.r10s,0)) + SUM(IFNULL(scrs.r11s,0)) + SUM(IFNULL(scrs.r12s,0)) + SUM(IFNULL(scrs.r13s,0)) + SUM(IFNULL(scrs.r14s,0)) + SUM(IFNULL(scrs.r15s,0)) + SUM(IFNULL(scrs.r16s,0)) + SUM(IFNULL(scrs.r17s,0)) + SUM(IFNULL(scrs.r18s,0)) AS team_total FROM scrs WHERE team_id = " . $team_id . " AND scr_season = '" . $season . "' AND current_year_scrs = " . $current_year;
                //echo("Scores Sum Select " . $sql_team_data ."<br>");
                ...
                */
                ...
                if($title == 'Elimination Final')
                {
                    ...
                    */
                    ...
                    $sql_finals_update_EF = "Update Team_entries Set
                    EF" . $semi_no . "_pts = " . $totals . ", 
                    EF" . $semi_no . "tot = " . $build_team_data['games_won'] . ",
                    audited = '" . $audited . "'
                    where team_grade = '" . $teamgrade . "' and team_name = '" . $build_fix[$fixture_no] . "' and team_season = '" . $season . "' and team_cal_year = " . $current_year;
                    //echo("Finals Update (EF) - " . $sql_finals_update_EF . ", Res Pos " . $result_position . "<br>");
                    $update = $dbcnx_client->query($sql_finals_update_EF);
                    if(!$update )
                    {
                        die("Could not update finals data: " . mysqli_error($dbcnx_client));
                    } 
                }
                else if($title == 'Semi Final')
                {
                    // semi final
                    $totals = 0;
                    if($build_team_data['finals_win'] == 1)
                ...
                else if((substr($title, 0, 5) == 'Round'))
                {   
                    //echo("Current Y " . $y . "<br>");
...
// get settings from grade settings table
$sql_grades = "Select * From Team_grade Where grade = '" . $teamgrade . "' AND fix_cal_year = " . $current_year . " AND season = '" . $season . "' AND current = 'Yes'";
...
foreach($fixture_array as $fixture_no)
{
    ...
    if($build_fix[$fixture_no] != '')
    {
        ...
        //echo("Get Scoresheet Home Data (Forfeit) " . $sql_forfeit . "<br>");
        ...
        while($build_forfeit = $result_forfeit->fetch_assoc())
        {
            ...
            // changed -4 to +8 10/08/2024
            // reverted to -4 12/08/2024
...
/*
            //get opposition forfeit team id for billing purposes
            $sql_billing = "Select * from tbl_scoresheet Left Join Team_entries on Team_entries.team_id = tbl_scoresheet.team_id where round = " . ($i+1) . " AND season = '" . $season . "' AND year = " . $current_year . " AND tbl_scoresheet.team_id = " . $team_id;
            $result_billing = $dbcnx_client->query($sql_billing) or die("Couldn't execute players query. " . mysqli_error($dbcnx_client));
            $build_billing = $result_billing->fetch_assoc();
            //echo("Billing " . $sql_billing . "<br>");
            //echo("Opposition " . $build_billing['opposition'] . "<br>");

            // get team id from team name
            $sql_bill_team = "Select team_id, billing_forfeits FROM vbsa3364_vbsa2.Team_entries where team_cal_year = " . $current_year . " AND team_season = '" . $season . "' and team_grade = '" . $teamgrade . "' and team_name = '" . $build_billing['opposition'] . "'";
            $result_bill_team = $dbcnx_client->query($sql_bill_team) or die("Couldn't execute players query. " . mysqli_error($dbcnx_client));
            $build_bill_team = $result_bill_team->fetch_assoc();

            $sql_bill_update = "Update Team_entries Set billing_forfeits = " . ($build_bill_team['billing_forfeits'] + 1) . " where team_name = '" . $build_billing['opposition'] . "' AND team_season = '" . $season . "' AND team_cal_year = " . $current_year . "  and team_id = " . $build_bill_team['team_id'];
            echo($sql_bill_update . "<br>");
*/
        }
    }
}
...
foreach($fixture_array as $fixture_no)
{
    //echo("Team ID from forfeit " . $team_id . "<br>");
    ...
    $adjustment_total = 0; // Previous Adjustment ???? //
    ...
    //{
        ...
        $sql_forfeit = "Select * from tbl_scoresheet where team = '" . $build_fix[$fixture_no] . "' and team_grade = '" . $teamgrade . "'  AND round = " . $round . " AND season = '" . $season . "' AND year = " . $current_year . " AND FirstName = 'Team' AND LastName = 'Forfeit'";
        //echo("Adjustment Select " . $sql_forfeit."<br>");

        $result_forfeit = $dbcnx_client->query($sql_forfeit) or die("Couldn't execute players query. " . mysqli_error($dbcnx_client));

        $num_rows_forfeit = $result_forfeit->num_rows;
        //echo("Rows = " . $num_rows_forfeit . "<br>");
        while($build_forfeit = $result_forfeit->fetch_assoc())
        {
            $team_id = $build_forfeit['team_id']; // added 20/02/2024
            if($type == "Snooker")
            {
                $max_points_available = ($no_of_players*$no_of_games); //12
                //echo("Players " . $no_of_players . ", Games " . $no_of_games . ", Max Points " . $max_points_available . "<br>");
                $home_points_available = ($no_of_players*2); //8
                //echo("Home Points " . ($no_of_players*2) . "<br>");
                // changed to 8 from -4 12/08/2024
                // reverted 13/08/2024
                $away_points_available = -4;
                $adjustment_score = ($max_points_available+$away_points_available); // 8
                //echo("Adjustment " . ($adjustment_score) . "<br>");
            }
            elseif($type == "Billiards")
            {
            ...
            HB=NULL, 
            adj_comment='Team Forfeit " . $build_fix[$fixture_no] . "'  
            WHERE team_id=" . $team_id . " and team_grade='" . $teamgrade . "'";  
            //echo("Team Entries adjustment Update " . $sql_team_forfeit .", Games Won " . $games_won . "<br>");
...
// end adjustment
//echo("End of Adjustment (Round " . $round . ")<br>");
