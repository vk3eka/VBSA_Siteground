...
$packedcalcdata = json_decode(stripslashes($_GET['PackedDataCalc']), true);
...
//echo("Calc Data " . $packedcalcdata[0] . "<br>");
...
else if($type == "Billiards")
{
    ...
    //echo($ForfeitData[1] . "<br>");
    for($i = 0; $i < sizeof($packedscoredata); $i++)
    {
        $score1 = explode(", ", $packedscoredata[$i]);
        $calc_scores = explode(",", $packedcalcdata[$i]);

        //echo($calc_scores[$i] . "<br>");

        $sql_players_home = "Update tbl_scoresheet Set 
        score_1 = " . $calc_scores[0] . ", 
        win_1 = " . $score1[0] . ",
        draw_1 = ".$score1[4] . ",
        capt_home = " . $home_approve . "
        where team = '" . $team . "'
