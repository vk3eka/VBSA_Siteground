...
$query_page_items = "SELECT webpage_items.ID, webpage_items.Header, webpage_items.`Comment`, webpage_items.`By`, webpage_items.created_on, webpage_items.blocked, webpage_items.img_orientation, webpage_items.item_image,  webpage_items.event_id,  webpage_items.page_help, webpage_items.OrderPolProc, webpage_items.img_size FROM webpage_items WHERE webpage_items.blocked='No' AND webpage_items.page_polproc='Y' ORDER BY OrderPolProc, created_on DESC";
//echo($query_page_items . "<br>");
...
<title>VBSA Policies &amp; Procedures</title>
...
<?php include '../includes/buttons_left.php';?>   
  ...
  <div ID="News_container">
    ...
    <div class="Page_heading_container">
 		<div class="index_title"><span class="h4">VBSA Policies &amp; Procedures</span></div>
		...
		
		{
			...
			$path_info_att = pathinfo($row_att['up_pdf_name']); 
			   	...
			   	if ($row_att['up_type'] == 'Attachment'):
					...
					echo '<td class="text-center"><a href="../pdfjs/web/viewer.html?file=' . $url . '/Front_page_upload/' . $row_att['up_pdf_name'] .'" title="View"><span class="glyphicon glyphicon-eye-open"></span></a></td>';
					}
					else echo '<td nowrap="nowrap" style="width:60%">'.$row_att['up_desc'].'</td>'.'<td style="width:20%">&nbsp;</td>' ;
					
					// download link
					echo '<td class="text-center"><a href="../pdfjs/web/viewer.html?file=' . $url . '/Front_page_upload/' . $row_att['up_pdf_name'] . '" title="Download"><span class="glyphicon glyphicon-download"></span></a></td>';
		...
		
		{
			...
			while($row_event = mysql_fetch_array($result_event)):	   
			   	...
			   	if ($row_event['type'] == 'Uploaded Attachment'):
					...
					// pdf viewer link
					echo '<td class="text-center"><a href="../pdfjs/web/viewer.html?file=' . $url . '/calendar/cal_upload/' . $row_event['Attachment'] .'" title="View"><span class="glyphicon glyphicon-eye-open"></span></a></td>';
					...
					// download link
					echo '<td class="text-center"><a href="../pdfjs/web/viewer.html?file=' . $url . '/calendar/cal_upload/' . $row_event['Attachment'] . '" title="Download"><span class="glyphicon glyphicon-download"></span></a></td>';
