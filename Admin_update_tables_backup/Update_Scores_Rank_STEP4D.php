...
if(isset($_POST["submit"]))
{
  ...
  $query_audience = "Select Email FROM members LEFT JOIN scrs ON scrs.MemberID=members.MemberID WHERE captain_scrs=1 AND authoriser_scrs=1 AND current_year_scrs = YEAR( CURDATE( ) ) AND scr_season='S1' AND (ReceiveEmail = 1 AND Email != '')";
  ...
  {  
    if($i % 30 == 0)
    {
      $query_audience_1 = "Select * FROM members LEFT JOIN scrs ON scrs.MemberID=members.MemberID WHERE captain_scrs=1 AND authoriser_scrs=1 AND current_year_scrs = YEAR( CURDATE( ) ) AND scr_season='S1' AND (ReceiveEmail = 1 AND Email != '') LIMIT 30 OFFSET " . $offset;
  ...
  $query_audience = "Select Email FROM members LEFT JOIN scrs ON scrs.MemberID=members.MemberID WHERE captain_scrs=1 AND authoriser_scrs=1 AND current_year_scrs = YEAR( CURDATE( ) ) AND scr_season='S2' AND (ReceiveEmail = 1 AND Email != '')";
  $result = mysql_query($query_audience, $connvbsa) or die(mysql_error());
  $no_of_records = $result->num_rows;
  $offset = 0; // offset required for mysql batches
  $y = 0; // number of sessions
  ...
  {  
    if($i % 30 == 0)
    {
      $query_audience_1 = "Select * FROM members LEFT JOIN scrs ON scrs.MemberID=members.MemberID WHERE captain_scrs=1 AND authoriser_scrs=1 AND current_year_scrs = YEAR( CURDATE( ) ) AND scr_season='S2' AND (ReceiveEmail = 1 AND Email != '') LIMIT 30 OFFSET " . $offset;
      $result_1 = mysql_query($query_audience_1, $connvbsa) or die(mysql_error());
      while($row_audience_1 = $result_1->fetch_assoc())
      {
        $email = $row_audience_1['Email'];
