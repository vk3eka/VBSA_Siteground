...
if(isset($_SERVER['SCRIPT_FILENAME'])){
	$WA_DOCUMENT_ROOT = str_replace( '\\', '/', substr($_SERVER['SCRIPT_FILENAME'], 0, 0-strlen($_SERVER['PHP_SELF'])));
}
if(!isset($WA_DOCUMENT_ROOT)){
	if(isset($_SERVER['PATH_TRANSLATED'])){
		$WA_DOCUMENT_ROOT = str_replace( '\\', '/', substr(str_replace('\\\\', '\\', $_SERVER['PATH_TRANSLATED']), 0,	0-strlen($_SERVER['PHP_SELF'])));
	}
}
if(!isset($WA_DOCUMENT_ROOT)){
...
if (!isset($absolutepath)) { 
     $docRoot = preg_replace("/\\$|\/$/", "", realpath($WA_DOCUMENT_ROOT));
