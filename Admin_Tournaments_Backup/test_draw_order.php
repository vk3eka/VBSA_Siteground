...
<br>
<form name='tournament_draw' method="post" action='test_draw_order.php'>
...
<div align='center'>Please select a tournament
  <select name="tournament" id="tournament" onchange="GetTournament(this)">
    <option value=""></option>
    <option value="202283">202283</option>
    <option value="202281">202281</option>
    <option value="202294">202294</option>
    <option value="202280">202280</option>
    <option value="202293">202293</option>
    <option value="202251">202251</option>
    <option value="202290">202290</option>
    <option value="202291">202291</option>
    <option value="202266">202266</option>
    <option value="202269">202269</option>
    <option value="202284">202284</option>
    <option value="202287">202287</option>
    <option value="202274">202274</option>
    <option value="202257">202257</option>
    <option value="202279">202279</option>
    <option value="202271">202271</option>
    <option value="202273">202273</option>
    <option value="202270">202270</option>
    <option value="202272">202272</option>
    <option value="202288">202288</option>
    <option value="202263">202263</option>
  </select>
</div>
</form>-->
<?php
...
*/
    ...
    //{
        ...
        $exist[$exist_index] = "Round: 2, Index: " . $i . ", MemberID: " . $build_score_table['MemberID'] . ", Rank: " . $rank_num . ", Time: " . $build_score_table['time_' . $total_players];
        Echo("Round: 2, Index: " . $i . ", MemberID: " . $build_score_table['MemberID'] . ", Rank: " . $rank_num . ", Time: " . $build_score_table['time_' . $total_players] . "<br>");
        $score_table_index++;
        $i++;
        */
    //}

    //echo("Existing " . $total_score_table . "<br>");
    if($total_exist == 0)
    {
        $i = 0;
...
*/
        ...
        foreach($period as $date)
        {
            //echo("Day (" . ($day+1) . ") " . $date->format("l") . "<br>");
            if($day == 1)
            {
                $day_1 = $date->format("l");
            }
        ...
        //echo("R " . $r . "<br>");
        $starttime = date($build_dates['time_' . $total_players]);
        for($r = ($build_dates['matches_day_1']/2); $r < (($build_dates['matches_day_2']+$build_dates['matches_day_1'])/2); $r++)
        {
            $sql_insert = "Insert into tournament_draw_dates (
              tourn_id, 
              matches_day_2,
        ...
        //echo("S " . $s . "<br>");
        $starttime = date($build_dates['time_' . $total_players]);
        for($r = (($build_dates['matches_day_2']+$build_dates['matches_day_1'])/2); $r < (($build_dates['matches_day_2']+$build_dates['matches_day_1']+$build_dates['matches_day_3'])/2); $r++)
        {
            $sql_insert = "Insert into tournament_draw_dates (
              tourn_id, 
              matches_day_3,
...
/*
        ...
        $day = 0;
        //echo("Tournament Size - " . $total_players . "<br>");
        $tourn_size = $total_players;

        foreach($period as $date)
        {
            //echo("Day " . $date . "<br>");
            //if($date->format("l") == 'Friday') // day one on comp
            //{  
            //    $day_count = 1; 
            //}
            //$no_of_sessions = ($total_players/$build_tables['ClubTables']);
            $max_matches = $build_dates['matches_day_' . ($day+1)];
            $no_of_sessions = ceil($max_matches/$build_tables['ClubTables']);
            //echo("Max Sessions " . $no_of_sessions . "<br>");
            //echo("Tourn Size " . $tourn_size . "<br>");
            //echo("Max Matches Day " . ($day+1) . " " . $max_matches . "<br>");
            //if($date->format("l") == 'Friday') // day one on comp
            //{   
            $i = 0;
            $starttime = date($build_dates['time_' . $total_players]);
            for($y = 0; $y < $build_tables['ClubTables']; $y++)
            {
                for($r = 0; $r < ($no_of_sessions*2); $r++)// double session to match each player in a match
                {
                    
                    if(($i+1) < ($total_matches-(($no_of_sessions)*$build_tables['ClubTables'])))
                    {
                        //echo("Index " . ($i+1) . "<br>");
                    }
                    
                    $sql_add_date_64 = "Update tournament_draw_dates SET r_" . $total_players . "_time = '" . $starttime . "', r_" . $total_players . "_day = '" . $date->format('l') . "' where r_" . $tourn_size . "_match_no = " . ($i+1) . " and tourn_id = " . $tournament_id;
                    //echo($sql_add_date_64 . "<br>");
                    $result_dates_64 = mysql_query($sql_add_date_64, $connvbsa) or die(mysql_error());
*/                    
...
*/                
            ...
            //$i = 0;
            $no_of_sessions = (($total_players/2)/$build_tables['ClubTables']);
            if($date->format("l") == 'Saturday') // day two on comp
            {   
                $starttime = date($build_dates['time_' . ($total_players/2)]);
                for($y = 0; $y < $build_tables['ClubTables']; $y++)
                {
                    for($r = 0; $r < ($no_of_sessions); $r++)// double session to match each player in a match
                    {
                        $sql_add_date_32 = "Update tournament_draw_dates SET r_" . ($total_players/2) . "_time = '" . $starttime . "', r_" . ($total_players/2) . "_day = '" . $date->format('l') . "' where r_" . $total_players . "_match_no = " . ($x+1) . " and tourn_id = " . $tournament_id;
                        //echo($sql_add_date_32 . "<br>");
                        $result_dates_32 = mysql_query($sql_add_date_32, $connvbsa) or die(mysql_error());
                        //$starttime = date('H:i:s',strtotime('+2 hour', strtotime($starttime)));
                        $x++;
                    }
                    $starttime = date('H:i:s',strtotime('+2 hour', strtotime($starttime)));
...
//      }
            ...
            //echo("2nd Part<br>");
            // 2nd Half
            $sql_date_time_string_2 = '';
            $rounds = $total_players;
            //echo("Start Time 1 " . $starttime . "<br>");
            $starttime_2 = $starttime;
            do{
                $sql_date_time_string_2 = " r_" . floor($rounds) . "_time = '" . $starttime_2 . "', r_" . floor($rounds) . "_day = '" . date('l', strtotime($build_dates['startdate'])) . "', " . $sql_date_time_string_2; 
                //echo($sql_date_time_string_2 . "<br>");
                $rounds = ($rounds/2);
                $starttime_2 = date('H:i:s',strtotime('+2 hour',strtotime($starttime_2)));
            } while($rounds > 1);
            //echo("<br>");
            $query_exist_2 = 'Select * FROM tournament_scores where tourn_id = ' . $tournament_id . " Limit 8 OFFSET 9";
            //echo("Exist Query " . $query_exist . "<br>");
            $result_exist_2 = mysql_query($query_exist_2, $connvbsa) or die(mysql_error());
            while($build_score_table_2 = $result_exist_2->fetch_assoc())
            {
                $sql_add_date_2 = 'Update tournament_scores SET ' . $sql_date_time_string_2 . 'tourn_id = ' . $tournament_id . ' where member_id = ' . $build_score_table_2['member_id'] . ' and tourn_id = ' . $tournament_id;
                echo($sql_add_date_2 . "<br>");
                //$result_dates_2 = mysql_query($sql_add_date_2, $connvbsa) or die(mysql_error());
            }
            //echo("Start Time 2 " . $starttime . "<br>");
        ...
        echo("<script>");
        echo("window.location.href = '../Admin_Tournaments/tournament_draw.php?tourn_id=" . $tournament_id . "';");
        echo("</script>");
    }
    else
    {
