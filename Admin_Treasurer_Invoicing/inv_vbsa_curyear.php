...
$page = "inv_vbsa_curyear.php?inv_type=$inv_type";
$_SESSION['page'] = $page;

$inv_page = "../inv_vbsa_curyear.php?inv_type=$inv_type";
...
$query_all_invoices = "SELECT inv_id, club_id,  inv_busname, inv_to, inv_email, inv_phone, inv_date, inv_type,  inv_paid_amount, inv_total_all, inv_status, inv_bad_debt FROM inv_to WHERE ( inv_date is null OR YEAR(inv_date) = YEAR( CURDATE( ) )) AND inv_type ='$inv_type' ORDER BY inv_status DESC,  inv_busname";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_create_inv = "SELECT ClubNumber, ClubTitle, FirstName, LastName, COUNT(cont_type) AS tot_contact FROM clubs  LEFT JOIN clubs_contact ON club_id=ClubNumber AND cont_type='Invoice to' LEFT JOIN members ON members.MemberID=cont_memb_id WHERE VBSAteam=1 GROUP BY club_id ORDER BY ClubTitle";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_all = "SELECT SUM(total_less_disc) AS allinv FROM inv_to WHERE inv_type = '$inv_type'  AND (inv_date is null OR YEAR(inv_date) = YEAR( CURDATE( ) ))";
$all = mysql_query($query_all, $connvbsa) or die(mysql_error());
$row_all = mysql_fetch_assoc($all);
$totalRows_all = mysql_num_rows($all);

mysql_select_db($database_connvbsa, $connvbsa);
$query_paid = "SELECT SUM(inv_paid_amount) AS paid FROM inv_to WHERE inv_type = '$inv_type'  AND (inv_date is null OR YEAR(inv_date) = YEAR( CURDATE( ) ))";
$paid = mysql_query($query_paid, $connvbsa) or die(mysql_error());
$row_paid = mysql_fetch_assoc($paid);
$totalRows_paid = mysql_num_rows($paid);

mysql_select_db($database_connvbsa, $connvbsa);
$query_unsent = "SELECT SUM(total_less_disc) AS unsent FROM inv_to  WHERE inv_type = '$inv_type'  AND inv_status = 'Not Sent'  AND (inv_date is null OR YEAR(inv_date) = YEAR( CURDATE( ) ))";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_sent = "SELECT SUM(total_less_disc) AS sent FROM inv_to WHERE inv_type = '$inv_type'  AND inv_status = 'Sent'  AND (inv_date is null OR YEAR(inv_date) = YEAR( CURDATE( ) ))";
...
<table align="center">
  ...
  <tr>
    <td colspan="4" align="center">All invoices issued for VBSA <?php echo $inv_type; ?> in <?php echo date("Y")?></td>
  </tr>
  <tr>
    <td colspan="4" align="center" class="red_bold">&nbsp;</td>
  </tr>
  <tr>
    <td colspan="4" align="center" valign="middle" class="red_text">Scroll down for a full list of <?php echo date("Y")?> participating Clubs, or to create a new <?php echo date("Y")?>, <?php echo $inv_type; ?> invoice</td>
  </tr>
  <tr>
...
<table align="center">
  ...
  <tr>
    <td align="center">List contains all VBSA Clubs that are participating in <?php echo date("Y")?> - Total Clubs : <?php echo $totalRows_create_inv ?></td>
  </tr>
  <tr>
  ...
  <tr>
    <td align="left"><span class="red_text">&quot; Invoice to&quot; names are from the Clubs table, there may be more than 1 contact, if so you will be asked which contact you wish to create an invoice for.</span></td>
  </tr>
</table>
...
<table border="1" align="center">
  ...
  <tr>
    ...
    <td align="left">
	<?php if($row_create_inv['tot_contact']>1) echo '<span class=red_text>'."More than 1 contact!".'</span>'; else echo $row_create_inv['FirstName']." ".$row_create_inv['LastName']; ?></td>
    <td nowrap="nowrap">
      <?php // if 1 only "Invoice to" contact go to create invoice
	if($row_create_inv['tot_contact'] ==1) { ?>
      <a href="user_files/inv_insert_from_club_id.php?club_id=<?php echo $row_create_inv['ClubNumber']; ?>&inv_type=<?php echo $inv_type; ?>"><img src="../Admin_Images/new_doc.fw.png"  height="20" title="Create a new invoice for this Club" /></a>
      <?php  // if more than 1 "Invoice to" contact go to page to select which one 
	 } else { ?>
      <a href="inv_to_multiple.php?club_id=<?php echo $row_create_inv['ClubNumber']; ?> & inv_type=<?php echo $inv_type; ?>" ><img src="../Admin_Images/new_doc.fw.png"  height="20" alt="new document" title="Multiple Invoice to contacts" /></a>
      <?php } ?>
    </td>
