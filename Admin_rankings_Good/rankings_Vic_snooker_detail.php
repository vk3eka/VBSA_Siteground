...
mysql_select_db($database_connvbsa, $connvbsa);
$query_rp_curr = "SELECT tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND tournaments.ranking_type='Vic rank' AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( )) GROUP BY tourn_id";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_rp_1yr = "SELECT tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND tournaments.ranking_type='Vic rank' AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( ))-1 GROUP BY tourn_id";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_rp_2yr = "SELECT tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND tournaments.ranking_type='Vic rank' AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( ))-2 GROUP BY tourn_id";
...
<table align="center">
    <tr>
      <td>&nbsp;</td>
        ...
        <tr>
          <td align="left">
            <strong><?php echo date("Y"); ?> Victorian Snooker Ranking for: <?php echo $row_RPall['FirstName']; ?> <?php echo $row_RPall['LastName']; ?></strong></td>
  </tr>
        <tr>
