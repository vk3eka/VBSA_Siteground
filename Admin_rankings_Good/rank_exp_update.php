...
$targetDir = "../Rankings/files/";
if (!is_dir($targetDir)) {
    mkdir($targetDir, 0777, true); // Create the directory if it doesn't exist
}

date_default_timezone_set('Australia/Melbourne');
$update_date =  date("l, F jS, Y \: g ia",time()); 

if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {

   // check if filename already uploaded.
  mysql_select_db($database_connvbsa, $connvbsa);
  /*$sql_filename = "Select item_image FROM webpage_items where ID = " . $_POST['ID'];
  $existing_file = mysql_query($sql_filename, $connvbsa) or die(mysql_error());
  $existing_filename = mysql_fetch_assoc($existing_file);
  $existing = $existing_filename['item_image'];*/
  $existing = $_POST['rank_exp_title'];
  //echo("Existing " . $existing . "<br>");

  $imageFile = $_FILES['rank_exp_pdf'];
...
if (isset($_GET['comp_type'])) {
  $rank_exp_type = $_GET['comp_type'];
}
//echo($rank_exp_type . "<br>");

mysql_select_db($database_connvbsa, $connvbsa);
$query_exp_rank = "Select * FROM rank_info WHERE rank_exp_type = '" . $rank_exp_type . "' ORDER BY rank_info.rank_exp_type";
//echo($query_exp_rank . "<br>");
