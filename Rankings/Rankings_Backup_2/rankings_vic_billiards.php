...
if(isset($_GET['cohort']))
{
  switch ($_GET['cohort'])
  {
      ...
      break;
      case 'woman':
        $query_RPall = "Select ranknum, memb_id, FirstName, LastName, Female, Junior, total_rp, date_format(last_update,'%D %b %Y') AS last_update FROM rank_Billiards LEFT JOIN members ON MemberID = memb_id where memberID IN (SELECT tourn_memb_id FROM vbsa3364_vbsa2.tourn_entry where tournament_number IN (SELECT tourn_id FROM vbsa3364_vbsa2.tournaments where ranking_type = 'Womens Rank')) AND Female = 1 ORDER BY total_rp DESC";
...
else
{
  ...
  //$query_RPall = "Select ranknum, rank_Billiards.memb_id, FirstName, LastName, Female, Junior, rank_Billiards.total_rp, date_format(rank_Billiards.last_update,'%D %b %Y') AS last_update FROM rank_Billiards LEFT JOIN members ON MemberID = rank_Billiards.memb_id LEFT JOIN rank_a_billiards_master ON rank_a_billiards_master.memb_id = members.MemberID ORDER BY total_rp DESC";
  $query_RPall = "Select 
  tourn_2, 
  tourn_1, 
  tourn_curr, 
