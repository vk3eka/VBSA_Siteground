...
$query_rp_curr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id=$rank AND " . $cohort_select . " AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( )) GROUP BY tourn_id order by rank_pts DESC";
...
$query_rp_1yr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND (tournaments.ranking_type='Vic Rank' OR tournaments.ranking_type='Womens Rank' OR tournaments.ranking_type='Junior Rank') AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( ))-1 GROUP BY tourn_id order by rank_pts DESC";
...
$query_rp_2yr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND (tournaments.ranking_type='Vic Rank' OR tournaments.ranking_type='Womens Rank' OR tournaments.ranking_type='Junior Rank') AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( ))-2 GROUP BY tourn_id order by rank_pts DESC";
...
<table class="table">
    <tr>
    	<td class="text-right"><span class="italic">Total Tournament Ranking Points</span> &nbsp;&nbsp;&nbsp; <strong><?php echo ($row_RPall['tourn_total']-$row_RPall['weekly_percent']); ?></strong></td>
    </tr>
</table>
</div>
