...
echo("function GetHomeGameCountDataAll() {");
...
foreach($team_grade as $grade)
{
    ...
    if($grade != '')
    {
        ...
        for($i = 0; $i < $rounds; $i++)
        {
            //echo("fixArray[round] = 'Round " . ($i+1) . "';");
            for($j = 0; $j < ($fixtures); $j++)
            {
                echo("fixArray = fixArray + ', ' + document.getElementById('" . $grade . "_home_" . ($i+1) . "_" . ($j+1) . "').value;");
            }
            //echo("round++;");
        }
    }
}
echo("console.log('Test Count Array (All) ' + fixArray);");
echo("return fixArray;");
echo("}");
echo("GetHomeGameCountDataAll();");
echo("</script>");

echo("<script type='text/javascript'>");
echo("function GetHomeGameCountDataRound() {");
$team_grade = explode(", ", $team_grades);
foreach($team_grade as $grade)
{
    echo("var round = 0;");
    ...
    if($grade != '')
    {
        ...
        for($i = 0; $i < $rounds; $i++)
        {
            ...
            for($j = 0; $j < ($fixtures); $j++)
            {
                echo("fixArray[round] = fixArray[round] + ', ' + document.getElementById('" . $grade . "_home_" . ($i+1) . "_" . ($j+1) . "').value;");
            }
            echo("round++;");
        }
    }
}
echo("console.log('Test Count Array (Round) ' + fixArray);");
echo("return fixArray;");
echo("}");
echo("GetHomeGameCountDataRound();");
echo("</script>");


echo("<script type='text/javascript'>");
echo("function GetHomeGameCountDataGrade() {");
$team_grade = explode(", ", $team_grades);
foreach($team_grade as $grade)
{
    echo("var grade = '';");
    ...
    if($grade != '')
    {
        ...
        for($i = 0; $i < $rounds; $i++)
        {
            echo("fixArray[grade] = 'Grade " . $grade . "';");
            for($j = 0; $j < ($fixtures); $j++)
            {
                echo("fixArray[grade] = fixArray[grade] + ', ' + document.getElementById('" . $grade . "_home_" . ($i+1) . "_" . ($j+1) . "').value;");
            }
        }
    }
}
echo("console.log('Test Count Array (Grade) ' + fixArray);");
echo("return fixArray;");
echo("}");
echo("GetHomeGameCountDataGrade();");
echo("</script>");
*/
...
echo("<script type='text/javascript'>");
echo("function GetHomeGameCountData(i) {");

$team_grade = explode(", ", $team_grades);

echo("var fixArray_grades = [];");
//echo("console.log('Round No. ' + i);");
...
foreach($team_grade as $grade)
{
    ...
    if($grade != '')
    {
        ...
        for($i = 0; $i < $rounds; $i++)
        {
            // add round number to home fixtures array
            echo("fixArray += 'Round ' + " . ($i+1) . ";");
            echo("fixArray += ', ';");

            for($j = 0; $j < ($fixtures); $j++)
            {
                echo("fixArray += document.getElementById('" . $grade . "_home_" . ($i+1) . "_" . ($j+1) . "').value;");
                echo("fixArray += ', ';");
            }
            //echo("fixArray = fixArray.substring(0, fixArray.length-1);");
            //echo("];");
            //echo("console.log(fixArray);");
            //echo("return fixArray;");
            //echo("fixArray_grades.push(fixArray[i]);");
        }
           
    }
    //$i++;
    //echo("return fixArray;");
    echo("fixArray_grades.push(fixArray);");
}
//echo("console.log(fixArray_grades[i]);");
...
$(document).ready(function()
{
    ...
    $.fn.count_clubs = function () {
    var year = 2024;
    var season = 'S2';
    var club_name = 'Yarraville Club';
    var team_array = [];
    var team_grade = 'AVS2';
    var day_played = 'Wed';
    console.log("Before");
    team_array = GetHomeGameCountData();
    console.log(team_array);
    console.log("After");
    for(var round = 0; round < 16; round++);
    {
        $.ajax({
            url:"count_clubs.php?TeamArray=" + team_array + "&Year=" + year + "&ClubName=" + club_name + "&TeamGrade=" + team_grade + "&Season=" + season + "&Round=" + round + "&DayPlayed=" + day_played,
...
echo("function CountClubsJava(team_name, HomeTeamArray)"); // need to pass in team name, club name and round from php
echo("{");
echo("        var HomeTeamArray = [];");
echo("        var HomeCountArray = [];");
echo("        HomeTeamArray = GetHomeGameCountData();");

// split HomeTeamArray into round numbers

//echo("        console.log('Array ' + HomeTeamArray + ' in round ' + i);");
echo("        numOfTrue = 0;");
echo("        HomeCountArray = HomeTeamArray.split(', ');");
//echo("        console.log('Team Name ' + team_name + ' in round ' + i);");
echo("        for(j = 0; j < HomeCountArray.length; j++)");
echo("        {");
//echo("            console.log(HomeCountArray[j]);");
echo("            if(team_name == HomeCountArray[j])");
echo("            {");
echo("                numOfTrue++;");
echo("            }");
echo("        }");
//echo("        console.log('Team Name ' + team_name + ' in round ' + i + ' count is ' + numOfTrue);");
//echo("    console.log('Count ' + numOfTrue);");
echo("}");
echo("</script>");
*/
/*
echo("<script type='text/javascript'>");
echo("function CountClubsJava(i, team_name)"); // need to pass in team name, club name and round from php
echo("{");
echo("        var HomeTeamArray = [];");
echo("        var HomeCountArray = [];");
echo("        HomeTeamArray = GetHomeGameCountData(i);");
echo("        console.log('Array ' + HomeTeamArray + ' in round ' + i);");
echo("        numOfTrue = 0;");
echo("        HomeCountArray = HomeTeamArray.split(', ');");
//echo("        console.log('Team Name ' + team_name + ' in round ' + i);");
...
function GetHomeGameData(grade, round, teams) 
{
    var fixArray_grades = [];
    var fixArray = '';
    var fixArray_home_games = '';
    var fixtures = (teams/2);
    for(j = 0; j < (fixtures); j++)
    {
        fixArray_home_games = document.getElementById(grade + '_home_' + round + '_' + (j+1)).value;
        fixArray = fixArray + ', ' + fixArray_home_games;
        //console.log("GetHomeGameData "  + grade + '_home_' + round + '_' + (j+1));
    }
    fixArray_grades = fixArray_grades + ', ' + fixArray;
    //console.log('Test Array (Home) ' + fixArray_grades + ", Round " + round);
    return fixArray_grades;
}

function GetAwayGameData(grade, round, teams) 
{
    var fixArray_grades = [];
    var fixArray = '';
    var fixArray_away_games = '';
    var fixtures = (teams/2);
    for(j = 0; j < (fixtures); j++)
    {
        //console.log("Doco " + document.getElementById(grade + '_home_' + round + '_' + (j+1)).value);
        fixArray_away_games = document.getElementById(grade + '_away_' + round + '_' + (j+1)).value;
        fixArray = fixArray + ', ' + fixArray_away_games;
        //console.log(grade + '_home_' + round + '_' + (j+1));
    }
    fixArray_grades = fixArray_grades + ', ' + fixArray;
    //console.log('Test Array (Away) ' + fixArray_grades);
    return fixArray_grades;
}

function CountHomeTeams(team, round, grade, teams) 
{
    var HomeTeamArray = GetHomeGameData(grade, round, teams).split(", ");
    var AwayTeamArray = GetAwayGameData(grade, round, teams).split(", ");
    var numOfTrue = 0;
    for(i = 0; i < HomeTeamArray.length; i++)
    {
        if(team == HomeTeamArray[i])
        {
            if(AwayTeamArray[i] != 'Bye')
            {
                numOfTrue++;
            }
        }
    }
    //console.log('Team ' + team + ", Count " + numOfTrue);
    return (numOfTrue);
}
...
window.onload = function() 
{
    ...
    //for(i = 0; i < 18; i++)
    //{
        console.log("Loop Onload " + GetHomeGameCountData(1));
    //}

    //GetHomeGameCountData();
    // setdate needs form number and number of rounds
    // needs to be dynamic...................
    SetDate(1, 10);
    SetDate(2, 10);
    SetDate(3, 18);
...
function SetDate(form_no, rounds)
{
...
function CountHomeGames($team_grades, $year, $season, $dayplayed)
{
    ...
    while($build_home_games = $result_home_games->fetch_assoc())
    {
        ...
        //echo("Count = " . $count[$team_name] . "<br>");
        
        if(($team_name == 'Bye') || ($count[$team_name] == NULL))
        {
            $numOfTrue = 0;
        }
        else
...
foreach($grades as $grade)
{
    if($grade != '')
    {
        ...
        //echo("SQL " . $sql_count_home_games . "<br>");
        ...
        //echo("Rows " . $count_home_games_rows . "<br>");
        if($count_home_games_rows > 0)
        {
            //echo("Count goes here<br>");
            CountHomeGames($team_grades, $year, $season, $dayplayed);
        }
    }
}

//echo(CountHomeTeams('Test Team Boronia') . "<br>");
...
while($build_data_grades = $result_grades->fetch_assoc())
{
    ...
    //echo("SQL " . $sql_create . "<br>");
    $result_create = mysql_query($sql_create, $connvbsa) or die(mysql_error());
    $create_data_exists = $result_create->num_rows;
    //echo("Rows " . $create_data_exists . "<br>");
...
while($row = $result_club->fetch_assoc()) 
{
    ...
    while($row_team = $result_team->fetch_assoc()) 
    {
        ...
        echo('<td align="center">&nbsp;</td>');
        if($row_team['team_grade'] == 'AVS2')
        {
            $teams = 6;
            $rounds = 10;
        }
        if($row_team['team_grade'] == 'BVS2')
        {
            $teams = 6;
            $rounds = 10;
        }
        if($row_team['team_grade'] == 'CVS2')
        {
            $teams = 10;
            $rounds = 18;
        }
        for($j = 0; $j < $rounds; $j++)
        {
            ...
            echo("<script type='text/javascript'>");
            echo("document.writeln(CountHomeTeams('" . $row_team['team_name'] . "', " . ($j+1) . ", '" . $row_team['team_grade'] . "', " . $teams . ")*2);");
            echo("</script>");
            //echo($count*2);
            echo('</td>');
        }
    ...
    //{   
        ...
        $sql_club_name = "Select team_name, team_club_id, team_club FROM Team_entries where team_club = '" . $row['team_club'] . "' and team_cal_year = 2024 and team_season = 'S2' and day_played = 'Wed'";
        //echo($sql_club_name . "<br>");
        $result_club_name = mysql_query($sql_club_name, $connvbsa) or die(mysql_error());
        while($build_club_name = $result_club_name->fetch_assoc())
        {
            //echo($build_club_name['team_name'] . "<br>");
            //echo("Round " . ($j+1) . "<br>");
            echo("<script type='text/javascript'>");
            //echo("var HomeTeamArray = GetHomeGameCountData();");
            //echo("document.writeln(CountClubsJava(" . ($j+1) . ", '" . $build_club_name['team_name'] . "'));");
            //echo("document.writeln(CountClubsJava('" . $build_club_name['team_name'] . "', HomeTeamArray));");
            echo("</script>");
        }
...
function show_fixtures($names, $team_grade, $form_no, $year, $season)
{
    ...
    for ($b = (sizeof($rounds) - 1); $b >= 0; $b--) {
        ...
        foreach ($rounds[$b] as $r) {
            $round_data = explode(" v ", $r);
            echo("<tr>");
            echo("<td align='center'><input class='float-child' type='text' id='" . $team_grade . "_home_" . ($round_counter-1) . "_" . ($y+1) . "' value='" . $round_data[1] . "' style='width:200px'></td>");
            echo("<td align='center'>v</td>");
            echo("<td align='center'><input class='float-child' type='text' id='" . $team_grade . "_away_" . ($round_counter-1) . "_" . ($y+1) . "' value='" . $round_data[0] . "' style='width:200px'></td>");
            echo("</tr>");
            $y++;
        }
    }
...
function PopulateFromDatabase($team_grade, $form_no, $year, $season, $dayplayed)
{
    ...
    //echo("<table style='background-color: #DCDBDB;' class='table table-striped table-bordered dt-responsive nowrap display float-container' width='1000px'>");
