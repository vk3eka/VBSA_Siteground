...
$page = "../admin_scores/scores_ladders_detail.php?team_id=$team_id&grade=$grade&comptype=$comptype&season=$season";
$_SESSION['page'] = $page;
...
$sql_byes = "Select 
  ...
  (fix6away = 'Bye') +
  (fix7away = 'Bye'))
  as byes FROM vbsa3364_vbsa2.tbl_fixtures Where season = '$season' and year = " . date('Y') . " and team_grade = '$grade'";
//echo("Select Byes " . $sql_byes . "<br>");
...
$sql_teams = "Select * FROM vbsa3364_vbsa2.Team_entries Where team_season = '$season' and team_cal_year = " . date('Y') . " and team_grade = '$grade' and team_name != 'Bye'";
...
$query_total_rounds = "Select no_of_rounds, grade, season, fix_cal_year, type FROM Team_grade WHERE grade = '$grade' and season = '$season' and fix_cal_year = " . date('Y') . " and current = 'Yes'";
...
//echo("Count Played " . $row_ladd_det['count_played'] . ", Total Rounds " . $total_rounds_available . ", Byes " . $no_of_byes . "<br>");
      ...
      <td><table border="1">
        ...
        <?php do { ?>
          <tr>
            ...
            <?php
            //echo(($row_ladd_det['totplayed_curr']+$row_ladd_det['totplaybill_curr']) . "<br>");
            if(
              $row_ladd_det['count_played'] > 0
              /*($row_ladd_det['paid_memb'] > 0 OR
              $row_ladd_det['LifeMember'] == 1 OR
              $row_ladd_det['ccc_player'] == 1 OR
              $row_ladd_det['playing_memb'] > 0 OR
              $row_ladd_det['memb'] > 0 OR 
...
*/            
        ...
        ?>
            ...
            <!--<td align="center"><?php if((($row_ladd_det['count_played']) >= ($total_rounds_available-$no_of_byes)*0.5)) echo "Yes"; else echo "No"; ?></td>
            <td align="center"><?php if(($row_ladd_det['count_played']) >= ($total_rounds_available-$row_ladd_det['byes_to_date'])*0.5) echo "Yes"; else echo "No"; ?></td>-->

            <?php
            if($comptype=='Snooker') { ?>
            <td align="center"><?php if($row_ladd_det['count_played'] > 6) echo "Yes"; else echo "No"; ?></td>
            <?php
            }
            else if($comptype=='Billiards') { ?>
            <td align="center"><?php if($row_ladd_det['count_played'] > 4) echo "Yes"; else echo "No"; ?>
           </td>
           <?php
            }
            ?>
            ...
            {
              echo ($row_ladd_det['count_played']);
              //echo ($row_ladd_det['count_played'] . " >= " . (($total_rounds_available-$row_ladd_det['byes_to_date'])*.5)  . "<br>");
              //echo ($row_ladd_det['count_played'] . ", " . $row_ladd_det['byes_to_date'] . ", " . $total_rounds_available . "<br>");
            }
            else 
            { ?>
  ...
  <table border="1" align="center" cellpadding="2" cellspacing="2">
    ...
    ?>
      ...
      } while ($row_team_brks = mysql_fetch_assoc($team_brks)); ?>
  </table>
</center>
</body>
</html>
