...
$query_Tot_Play_Positions = "Select SUM(players) AS TotPositions FROM Team_entries WHERE team_season='$season' AND team_name<>'Bye' AND include_draw='Yes' AND team_cal_year = YEAR( CURDATE( ) )";
//echo("Team Playing Positions " . $query_Tot_Play_Positions . "<br>");
...
$query_Tot_Inc = "Select SUM(players*9*15) AS TotInc FROM Team_entries WHERE team_season='$season' AND team_name<>'Bye' AND include_draw='Yes' AND team_cal_year = YEAR( CURDATE( ) ) AND include_draw='Yes'";
//echo("Team Playing Positions " . $query_Tot_Play_Positions . "<br>");
...
// change season to team_season or scr_season................need to change all other queries to match???????

$query_team_entries = "Select Team_entries.team_id, team_club, team_name, comptype, Team_entries.team_grade, day_played, players, include_draw, team_club_id,  COUNT(scrs.team_id) AS allplayers, team_season, SUM(scrs.captain_scrs) AS capt, SUM(scrs.authoriser_scrs) AS authoriser FROM Team_entries   LEFT JOIN scrs ON Team_entries.team_id = scrs.team_id WHERE team_season='$season'  AND team_cal_year = YEAR( CURDATE( ) ) GROUP BY Team_entries.team_id ORDER BY team_grade, team_name";
...
//mysql_select_db($database_connvbsa, $connvbsa);
//$query_team_summary = "SELECT COUNT( Team_entries.team_grade ) AS teams, team_grade, day_played, current_year_team, grade_name, fix_upload, fix_cal_year, comptype, team_season FROM Team_entries LEFT JOIN Team_grade ON Team_entries.team_grade=Team_grade.grade WHERE team_season='$season' AND `include_draw` = 'Yes' AND team_cal_year = YEAR( CURDATE( ) ) GROUP BY team_grade ORDER BY day_played, comptype DESC,  team_grade ASC";
$query_team_summary = "Select COUNT( Team_entries.team_grade ) AS teams, team_grade, day_played, current_year_team, team_grade, team_cal_year, comptype, team_season FROM Team_entries WHERE team_season='$season' AND `include_draw` = 'Yes' AND team_cal_year = YEAR( CURDATE( ) ) GROUP BY team_grade ORDER BY day_played, comptype DESC,  team_grade ASC";
//echo("Team Summary " . $query_team_summary . "<br>");
...
<table align="center">
  ...
  <tr>
    <td colspan='5' align="center">Total Playing Positions = <?php echo $row_Tot_Play_Positions['TotPositions']; ?>. Total Teams = <?php echo $row_team_count['totalteams']; ?>. Total Approx Income $<?php echo $row_Tot_Inc['TotInc']; ?> (Based on $9 per player per round over 15 rounds)</td>
  ...
  <?php do { ?>  
    <tr>
      ...
      <td align="center"><?php echo $row_team_summary['team_season']; ?></td>
  ...
  <?php do { ?>
    <tr>
      <td align="center" nowrap="nowrap"><?php echo $row_team_entries['team_id']; ?></td>
        ...
        <td align="center"><?php echo $row_team_entries['authoriser']; ?></td>
