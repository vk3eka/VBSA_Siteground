...
if($comptype == 'Snooker')
{
  $query_ladder = "SELECT team_id, team_club, team_name, team_grade, match_pts_total AS Pts, team_perc, match_won_count AS W, match_drawn_count AS D, total_score AS F,  pts_against AS A, rounds_played AS P, count_byes AS B, count_forfeits AS Forfeits, Updated, Countback, comptype, team_season, audited FROM Team_entries WHERE team_grade='$grade' AND comptype='$comptype' AND include_draw ='Yes' AND team_cal_year = YEAR( CURDATE( ) ) GROUP BY team_id ORDER BY Pts DESC, team_perc DESC, W DESC, D DESC ";
}
elseif ($comptype == 'Billiards') {
  $query_ladder = "SELECT team_id, team_club, team_name, team_grade, match_pts_total AS Pts, team_perc, match_won_count AS W, match_drawn_count AS D, total_score AS F,  pts_against AS A, rounds_played AS P, count_byes AS B, count_forfeits AS Forfeits, Updated, Countback, comptype, team_season, audited, scrs_for_finals, scrs_against_finals, scrs_percent_finals FROM Team_entries WHERE team_grade='$grade' AND comptype='$comptype' AND include_draw ='Yes' AND team_cal_year = YEAR( CURDATE( ) ) GROUP BY team_id ORDER BY F DESC, scrs_percent_finals DESC";
...
<?php include '../admin_xx_includes/db_srch.php';?>
  ...
  <table border="1" align="center" cellpadding="2" class="greenbg">
    ...
    <td align="center">Team ID</td>
      ...
      <td align="center">Gms For</td>
      <td align="center">Gms Agst</td>
      <td align="center">Played</td>
      <?php
      if($comptype == 'Billiards')
      {
        echo("<td align='center'>Pts For</td>");
        echo("<td align='enter'>Pts Agst</td>");
        echo("<td align='center'>Pts %</td>");
      }
      ?>
      <td align="center">Byes</td>
      <td align="center">Forfeits</td>
  ...
  <?php do { ?>
    <tr>
      <td align="center"><?php echo $row_ladder['team_id']; ?></td>
        ...
        <td width="15" align="center"><?php echo $row_ladder['Forfeits']; ?></td>
