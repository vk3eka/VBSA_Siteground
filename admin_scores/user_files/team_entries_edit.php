...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  ...
  if($_POST['current_grade'] != $_POST['team_grade'])
  {
    $sql_scores = 'Select scrsID from scrs where team_id = ' . $_POST['team_id'];
    //echo($sql_scores . "<br>");
    $scores_result = mysql_query($sql_scores, $connvbsa) or die(mysql_error());
    while($build_scores = mysql_fetch_assoc($scores_result))
    {
      $scores_update = 'Update scrs set team_grade = "' . $_POST['team_grade'] . '" Where scrsID = ' . $build_scores['scrsID'];
      //echo($scores_update . "<br>");
      $Update = mysql_query($scores_update, $connvbsa) or die(mysql_error());
    }
  }
...
//echo("Club Entries " . $query_team_entries . "<br>");
$current_grade = $row_team_entries['team_grade'];
//echo($current_grade . "<br>");
...
//mysql_select_db($database_connvbsa, $connvbsa);
$query_grades = "Select grade, grade_name, season, type, RP, `current` FROM Team_grade WHERE current='Yes' AND season='$season' AND fix_cal_year= year(CURDATE())";
//echo($query_grades . "<br>");
...
/*$query_grades = "SELECT distinct grade, grade_name, season, type, current, dayplayed FROM Team_grade WHERE season = '$season' AND current = 'Yes' and fix_cal_year = YEAR( CURDATE( ) ) ORDER BY type, dayplayed, grade";
...
--> 
    ...
    <tr valign="baseline">
      ...
      <td><select name="team_grade">
          ...
          <?php do {  ?>
          <option value="<?php echo $row_grades['grade']?>"<?php if (!(strcmp($row_grades['grade'], htmlentities($row_team_entries['team_grade'], ENT_COMPAT, 'utf-8')))) {echo "selected=\"selected\"";} ?>><?php echo $row_grades['grade_name']?></option>
          <?php
    ...
    <input type="hidden" name="current_grade" value="<?php echo $current_grade; ?>" />
</form>
<p>&nbsp;</p>
</center>
</body>
