...
$sqlorderby_curr_play = "LastName";
if (isset($sql_orderby)) {
  $sqlorderby_curr_play = $sql_orderby;
}
mysql_select_db($database_connvbsa, $connvbsa);

$query_curr_play = "Select scrsID, scrs.MemberID, FirstName, LastName, junior, MobilePhone, Email, team_grade, team_id, game_type, count_played, captain_scrs, scr_season, memb_by FROM scrs LEFT JOIN members ON members.MemberID = scrs.MemberID WHERE count_played>0 AND current_year_scrs = YEAR(CURDATE( )) AND (scrs.MemberID != 1 AND scrs.MemberID != 100 AND scrs.MemberID != 1000) AND scr_season='$season' ORDER BY {$sqlorderby_curr_play}";
...
mysql_select_db($database_connvbsa, $connvbsa);
/*
$query_curr_play = "SELECT scrsID, scrs.MemberID, FirstName, LastName, MobilePhone, Email, team_grade, team_id, game_type, count_played, captain_scrs, scr_season FROM scrs LEFT JOIN members ON members.MemberID = scrs.MemberID WHERE count_played>0 AND current_year_scrs = YEAR(CURDATE( )) AND (scrs.MemberID != 1 AND scrs.MemberID != 100 AND scrs.MemberID != 1000) AND scr_season='$season' ORDER BY {$sqlorderby_curr_play}";
$curr_play = mysql_query($query_curr_play, $connvbsa) or die(mysql_error());
$row_curr_play = mysql_fetch_assoc($curr_play);
$totalRows_curr_play = mysql_num_rows($curr_play);
...
<table align="center" cellpadding="2" cellspacing="2" class="page">
      <tr>
        ...
        <td align="left"><a href="<?Php echo ($tfm_orderbyURL); ?>junior">Junior</a></td>
      ...
      <?php do { ?>
        <tr>
          ...
          <?php 
          if(($row_curr_play['junior'] ==='na') OR ($row_curr_play['junior'] === 'U21'))
          {
            $junior = 'na';
          }
          else
          {
            $junior = $row_curr_play['junior'];
          }
          ?>
          <td align="left"><?php echo $junior ?></td>
