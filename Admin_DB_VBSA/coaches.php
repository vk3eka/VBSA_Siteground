...
$page = "../Admin_DB_VBSA/coaches.php";
$_SESSION['page'] = $page;
...
$query_coaches_bulk = "Select MemberID, memb_id, FirstName, LastName, Email, MobilePhone, coach_id, class, comment, URL, coach_order, active_coach FROM members, coaches_vbsa WHERE members.MemberID = coaches_vbsa.memb_id ORDER BY coach_order";
//echo("Select " . $query_coaches . "<br>");
$coaches_bulk = mysql_query($query_coaches_bulk, $connvbsa) or die(mysql_error());
$row_coaches_bulk = mysql_fetch_assoc($coaches_bulk);
$totalRows_coaches_bulk = mysql_num_rows($coaches_bulk);mysql_select_db($database_connvbsa, $connvbsa);

$query_coaches = "Select MemberID, memb_id, FirstName, LastName, Email, MobilePhone, coach_id, class, comment, URL, coach_order, active_coach, ReceiveEmail FROM members, coaches_vbsa WHERE members.MemberID = coaches_vbsa.memb_id AND (ReceiveEmail = 1 AND Email != '')  ORDER BY coach_order";
//echo("Select " . $query_coaches . "<br>");
$coaches = mysql_query($query_coaches, $connvbsa) or die(mysql_error());
$row_coaches = mysql_fetch_assoc($coaches);
$totalRows_coaches = mysql_num_rows($coaches);mysql_select_db($database_connvbsa, $connvbsa);
...
if(isset($_POST["Do_Send"]) && $_POST["Do_Send"]=="Send"){
	...
	do {
		...
		if($_POST['StartNumber']==$current_record){
			$error="";
			//Send the message
			$to=($_POST["SendMode"]!="Test")?(bind_email($_POST["To"], $row_coaches)):$_POST["To"];
			...
			if (!$valid_email){
				...
				if($dyn_subject===1){
					$subject=bind_fields($_POST["Subject"],$coaches,$row_coaches);
				}
				//Bind fields if the message contains variables
				if($dyn_message===1){
					$message=bind_fields($_POST["m_source"],$coaches,$row_coaches);
				}
				$email=$_POST["From"];
	...
	}while ($row_coaches = mysql_fetch_assoc($coaches)); 
	//Render Server Response
	header("Content-Type: text/xml");
	echo "<sent to=\"".$to."\" total=\"".$totalRows_coaches."\"><errors>".$error."</errors></sent>";
	die();
}
/*$query_coaches = "Select MemberID, memb_id, FirstName, LastName, Email, MobilePhone, coach_id, class, comment, URL, coach_order, active_coach FROM members, coaches_vbsa WHERE members.MemberID = coaches_vbsa.memb_id ORDER BY coach_order, coach_id";
$coaches = mysql_query($query_coaches, $connvbsa) or die(mysql_error());
$row_coaches = mysql_fetch_assoc($coaches);
$totalRows_coaches = mysql_num_rows($coaches);
*/
$query_coaches_count = "Select COUNT(MemberID) AS tot_coaches FROM members LEFT JOIN coaches_vbsa ON MemberID=memb_id WHERE active_coach=1";
//echo $query_coaches . "<br>";
$coaches_count = mysql_query($query_coaches_count, $connvbsa) or die(mysql_error());
$row_coaches_count = mysql_fetch_assoc($coaches_count);
$totalRows_coaches_count = mysql_num_rows($coaches_count);
...
<table align="center" cellpadding="5" cellspacing="5">
      <tr>
        <td class="red_bold">All Accredited Coaches</td>
        <td class="greenbg"><a href="user_files/coaches_insert.php">Insert a new coach</a> (ID number req.)</td>
        <td align="right" class="greenbg"><a href="A_memb_index.php">Return to Members index</a></td>
      </tr>
      <tr>
        <td class="page">After updating, please check the <a href="http://www.vbsa.org.au/VBSA/accredited_coaches.php" target="_blank">webpage</a> (refresh if you already have it open)</td>
        <td>Total Coaches: <?php echo $row_coaches_count['tot_coaches'] ?></td>
        <td align="right">&nbsp;</td>
      </tr>
</table>
<p>&nbsp;</p>
  ...
  <table width="1100" align="center" cellpadding="2" cellspacing="2">
	  ...
	  <?php do { ?>
      <tr>
        <td width="150" align="left"><?php echo $row_coaches_bulk['FirstName']; ?> <?php echo $row_coaches_bulk['LastName']; ?></td>
        <td width="200" align="left" class="page"><a href="mailto:<?php echo $row_coaches_bulk['Email']; ?>"><?php echo $row_coaches_bulk['Email']; ?></a></td>
        <td width="100" align="left" class="page"><a href="tel:<?php echo $row_coaches_bulk['MobilePhone']; ?>"><?php echo $row_coaches_bulk['MobilePhone']; ?></a></td>
        <td width="50" align="left"><?php echo $row_coaches_bulk['class']; ?></td>
        <td width="50" align="center"><?php echo $row_coaches_bulk['coach_order']; ?></td>
        <td width="50" align="center"><?php if($row_coaches_bulk['active_coach'] == 1) echo "Yes"; else echo "No"; ?></td>
        
        <td width="180" class="page">
        <?php
		if (isset ($row_coaches_bulk['URL']))
		{
		?>
        <a href="<?php echo $row_coaches_bulk['URL']; ?>" target="_blank"><?php echo $row_coaches_bulk['URL']; ?></a>
        <?php
		}
		else;
		echo '&nbsp;';
        ...
        <td width="120" align="center" nowrap="nowrap" class="greenbg"><a href="user_files/coach_edit.php?coach_edit=<?php echo $row_coaches_bulk['coach_id']; ?> ">Edit Coach Details</a></td>
        <td width="120" align="center" nowrap="nowrap" class="greenbg"><a href="../A_common/vbsa_member_edit.php?memb_id=<?php echo $row_coaches_bulk['MemberID']; ?>">Edit Personal</a></td>
        <td width="30"><a href="user_files/coach_delete.php?coach_del=<?php echo $row_coaches_bulk['memb_id']; ?>"><img src="../Admin_Images/Trash.fw.png" width="25" height="25" /></a></td>
      </tr>
      <tr>
        <td colspan="9" style="border-bottom: 1px solid #000;">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;<?php echo $row_coaches_bulk['comment']; ?></td>
        </tr>
  <?php } while ($row_coaches_bulk = mysql_fetch_assoc($coaches_bulk)); ?>    
  </table>
  ...
  <p>&nbsp;</p>
    ...
    <br />
    <?php $myRecordset=$coaches; $myTotalRecords=$totalRows_coaches; ?>
