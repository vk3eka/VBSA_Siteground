...
$page = "../Admin_DB_VBSA/vbsa_members_old.php";
$_SESSION['page'] = $page;
...
//echo("Sort Order " . $sortby . "<br>");
//$query_memb = "Select MemberID, FirstName, LastName, MobilePhone, Email, paid_memb, LifeMember, referee, coach_id, ccc_player,  totplayed_curr+totplaybill_curr AS Current, totplayed_curr AS CSnooker, totplaybill_curr AS CBilliards, memb_by FROM members LEFT JOIN coaches_vbsa ON MemberID=memb_id WHERE ((paid_memb=20 AND YEAR(paid_date)=YEAR(NOW())) OR LifeMember=1 OR totplayed_curr+totplaybill_curr>0 OR ccc_player=1 OR referee=1) AND (MemberID != 1 AND MemberID != 100 AND MemberID != 1000 AND MemberID != 1500 AND Deceased !=1) " . $sortby;

//$query_memb = "Select MemberID, FirstName, LastName, MobilePhone, Email, paid_memb, LifeMember, referee, coach_id, ccc_player,  totplayed_curr+totplaybill_curr AS Current, totplayed_curr AS CSnooker, totplaybill_curr AS CBilliards, memb_by, active_coach FROM members LEFT JOIN coaches_vbsa ON MemberID=memb_id WHERE ((paid_memb=20 AND YEAR(paid_date)=YEAR(NOW())) OR LifeMember=1 OR totplayed_curr+totplaybill_curr>0 OR ccc_player=1 OR referee=1) AND (MemberID != 1 AND MemberID != 100 AND MemberID != 1000 AND MemberID != 1500 AND Deceased !=1) OR active_coach = 1 Order By paid_memb DESC, LifeMember DESC, Referee DESC, active_coach DESC, ccc_player DESC, LastName, FirstName";

$query_memb = "Select MemberID, FirstName, LastName, MobilePhone, Email, paid_memb, LifeMember, referee, coach_id, ccc_player,  totplayed_curr+totplaybill_curr AS Current, totplayed_curr AS CSnooker, totplaybill_curr AS CBilliards, memb_by FROM members LEFT JOIN coaches_vbsa ON MemberID=memb_id WHERE ((paid_memb=20 AND YEAR(paid_date)=YEAR(NOW())) OR LifeMember=1 OR totplayed_curr+totplaybill_curr>0 OR ccc_player=1 OR referee=1) AND (MemberID != 1 AND MemberID != 100 AND MemberID != 1000 AND MemberID != 1500 AND Deceased !=1) Order By paid_memb DESC, LifeMember DESC, Referee DESC, ccc_player DESC, LastName, FirstName";
...
<form name='sort'  method="post" action='vbsa_members_old.php'>
  ...
  <?php 
	...
	?>
	    <tr>
	      ...
	      <td align="center"><?php if($row_memb['LifeMember']==1) echo "Yes"; else echo ""; ?></td>
	      <td align="center"><?php if($row_memb['referee']==1) echo "Yes"; else echo ""; ?></td>
	      <td align="center"><?php if($row_memb['active_coach'] == 1) echo "Yes"; else echo ""; ?></td>
	      <td align="center"><?php if($row_memb['ccc_player']==1) echo "Yes"; else echo ""; ?></td>
