...
$query_contact = "SELECT * FROM RSL_contact WHERE contact_current=1 ORDER BY contact_order";
$contact = mysql_query($query_contact, $connvbsa) or die(mysql_error());
$row_contact = mysql_fetch_assoc($contact);
$totalRows_contact = mysql_num_rows($contact);

mysql_select_db($database_connvbsa, $connvbsa);
$query_about = "SELECT item_title, news_content FROM RSL WHERE RSL_id =7";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_header = "SELECT item_title, news_content, RSL.pagezone_header_desc, pagezone_header FROM RSL WHERE RSL_id=8";
...
<title>
	<?php 
	if(!empty($row_contact['contact_id']) && !empty($row_about['item_title']))  echo "RSL Contact & About"; 
	if(empty($row_contact['contact_id']) && !empty($row_about['item_title']))  echo "RSL About"; 
	if(!empty($row_contact['contact_id']) && empty($row_about['item_title']))  echo "RSL Contact";
	?>
</title>
