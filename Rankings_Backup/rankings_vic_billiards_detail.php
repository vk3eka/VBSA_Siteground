...
$query_rp_curr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND tournaments.tourn_type='Billiards' AND tournaments.ranking_type='Vic Rank' AND entry_cal_year = YEAR(CURDATE( )) GROUP BY tourn_id order by rank_pts DESC";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_rp_1yr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND tournaments.tourn_type='Billiards' AND tournaments.ranking_type='Vic Rank' AND entry_cal_year = YEAR(CURDATE( ))-1 GROUP BY tourn_id order by rank_pts DESC";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_rp_2yr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND tournaments.tourn_type='Billiards' AND tournaments.ranking_type='Vic Rank' AND entry_cal_year = YEAR(CURDATE( ))-2 GROUP BY tourn_id order by rank_pts DESC";
...
$total_15_pc = Round((((ROUND($row_S1curr['total_RP']))+(ROUND($row_S2curr['total_RP']))+(ROUND($row_S1_1year['total_RP']*65/100))+(ROUND($row_S2_1year['total_RP']*65/100))+(ROUND($row_S1_2year['total_RP']*35/100))+(ROUND($row_S2_2year['total_RP']*35/100)))*0.15));
...
<div class="table-responsive center-block" style="max-width:600px" > 
	<table class="table">
        ...
        <tr>
          <td class="text-center"> Tournament Points =  <strong><?php echo $row_RPall['total_rp']; ?></strong> which includes 15% of Weekly points = <strong><?php echo $total_15_pc; ?></strong></td>
        </tr>
    </table>
</div>
<div class="table-responsive center-block" style="max-width:600px" > 
