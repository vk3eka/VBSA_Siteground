...
if(isset($_GET['cohort']))
{
  ...
  {
      case 'all':
      //$query_RPall = "Select ranknum, memb_id, FirstName, LastName, Female, Junior, total_rp, date_format(last_update,'%D %b %Y') AS last_update FROM rank_Billiards LEFT JOIN members ON MemberID = memb_id ORDER BY total_rp DESC";
      $query_tournRP = "Select scr_2yr_s1+scr_2yr_s2 as scr_2yr, scr_1yr_s1+scr_1yr_s2 as scr_1yr, scr_curr_s1+scr_curr_s2 as scr_curr, tourn_2+tourn_1+tourn_curr+rank_a_billiards_master.total_rp as total_points, ranknum, rank_Billiards.memb_id, FirstName, LastName, Female, Junior, rank_Billiards.total_rp, date_format(rank_Billiards.last_update,'%D %b %Y') AS last_update FROM rank_Billiards LEFT JOIN members ON MemberID = rank_Billiards.memb_id LEFT JOIN rank_a_billiards_master ON rank_a_billiards_master.memb_id = rank_Billiards.memb_id ORDER BY rank_Billiards.total_rp DESC";
      break;
      case 'woman':
        //$query_RPall = "Select ranknum, memb_id, FirstName, LastName, Female, Junior, total_rp, date_format(last_update,'%D %b %Y') AS last_update FROM rank_Billiards LEFT JOIN members ON MemberID = memb_id where Female = 1 ORDER BY total_rp DESC";
        ...
        break;
      case 'junior': 
        //$query_RPall = "Select ranknum, memb_id, FirstName, LastName, Female, Junior, total_rp, date_format(last_update,'%D %b %Y') AS last_update FROM rank_Billiards LEFT JOIN members ON MemberID = memb_id where Junior != 'na' ORDER BY total_rp DESC";
...
{
  //echo("Cohort Not Set<br>");
  //$query_RPall = "Select ranknum, memb_id, FirstName, LastName, Female, Junior, total_rp, date_format(last_update,'%D %b %Y') AS last_update FROM rank_Billiards LEFT JOIN members ON MemberID = memb_id LEFT JOIN rank_a_billiards_master ON memb_id = members.MemberID ORDER BY total_rp DESC";
...
<?php include '../includes/prev_page.php';?>
  ...
  <table class="table">
  <tr>
    <td><form action="rankings_vic_billiards_name_srch_res.php" method="get" enctype="<?php echo $ranknum; ?>" name="RP_sname" id="RP_sname" >
...
<div class="table-responsive center-block" style="max-width:700px; padding-left:3px"> <!-- class table-responsive -->
  <table class="table">
        ...
        <tr>
          ...
          <td nowrap="nowrap"><a href="rankings_vic_billiards_detail.php?rank=<?php echo $row_tournRP['memb_id']; ?>">how these points were awarded</a></td>
        </tr>
