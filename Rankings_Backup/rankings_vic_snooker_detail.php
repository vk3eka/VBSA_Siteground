...
$query_rp_curr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id=$rank AND tournaments.ranking_type='Vic Rank' AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( )) GROUP BY tourn_id order by rank_pts DESC";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_rp_1yr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND tournaments.ranking_type='Vic Rank' AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( ))-1 GROUP BY tourn_id order by rank_pts DESC";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_rp_2yr = "Select tourn_memb_id, rank_pts, tourn_name FROM tourn_entry  LEFT JOIN tournaments ON tourn_id=tournament_number WHERE tourn_memb_id='$rank' AND tournaments.ranking_type='Vic Rank' AND tournaments.tourn_type='Snooker' AND entry_cal_year = YEAR(CURDATE( ))-2 GROUP BY tourn_id order by rank_pts DESC";
...
$query_S1curr = "Select SUM(total_RP) as TotalRP FROM scrs  WHERE MemberID='$rank' AND current_year_scrs = YEAR(CURDATE( )) AND game_type='Snooker' AND scr_season='S1' GROUP BY memberID";
...
//echo("Season 2, 2 yaer " . $query_S2_2year . "<br>");
...
$total_15_pc = Round((((ROUND($row_S1curr['TotalRP']))+(ROUND($row_S2curr['TotalRP']))+(ROUND($row_S1_1year['TotalRP']*65/100))+(ROUND($row_S2_1year['TotalRP']*65/100))+(ROUND($row_S1_2year['TotalRP']*35/100))+(ROUND($row_S2_2year['TotalRP']*35/100)))*0.15));
...
<div id="Wrapper"><!-- Wrapper contains content to a max width of 1200px--> 
  ...
  <div class="Page_heading_container">
 		<div class="page_title">Victorian Rankings - Snooker Tournaments, Player detail</div>
  </div>  	
...
<div class="table-responsive center-block" style="max-width:600px" > 
	<table class="table">
        ...
        <tr>
          <td class="text-center"> Tournament Points =  <strong><?php echo $row_RPall['tourn_total']; ?></strong> which includes 15% of Weekly points = <strong><?php echo $total_15_pc; ?></strong></td>
        </tr>
    </table>
</div>
<div class="table-responsive center-block" style="max-width:600px" > 
...
<table class="table">
  ...
  else
  {
    ...
    { 
      ...
      { 
        echo("<tr>");
        echo("<td class='text-left'>" . $row_rp_curr['tourn_name'] . "</td>");
        echo("<td class='text-center'>" .  $row_rp_curr['rank_pts'] . "</td>");
        echo("<td class='text-center'>" . $row_rp_curr['rank_pts'] . "</td>");
        echo("</tr>");
      }
    } 
...
<table class="table">
  ...
  else
  {
    ...
    { 
      ...
      { 
        echo("<tr>");
        echo("<td class='text-left'>" . $row_rp_1yr['tourn_name'] . "</td>");
        echo("<td class='text-center'>" .  $row_rp_1yr['rank_pts'] . "</td>");
        echo("<td class='text-center'>" . (ROUND($row_rp_1yr['rank_pts']*65/100)) . "</td>");
        echo("</tr>");
      }
    } 
...
<table class="table">
  ...
  else
  {
    ...
    { 
      ...
      { 
        echo("<tr>");
        echo("<td class='text-left'>" . $row_rp_2yr['tourn_name'] . "</td>");
        echo("<td class='text-center'>" .  $row_rp_2yr['rank_pts'] . "</td>");
        echo("<td class='text-center'>" . (ROUND($row_rp_2yr['rank_pts']*35/100)) . "</td>");
        echo("</tr>");
      }
    } 
...
<div class="table-responsive center-block" style="max-width:600px" > 
	<table class="table">
        ...
        <tr>
          ...
          <td class="text-center"><?php echo $row_S1curr['TotalRP']; ?></td>
          <td class="text-center">x</td>
          <td class="text-center">100%</td>
          <td class="text-center">=</td>
          <td class="text-center"><?php if(isset($row_S1curr['TotalRP'])) echo round($row_S1curr['TotalRP']); else echo "0"; ?></td>
        </tr>
        <tr>
          <td><span class="red_bold"><?php echo date("Y", strtotime(date("Y-m-d"))); ?></span> - S2 Best total Ranking Points</td>
          <td class="text-center"><?php echo $row_S2curr['TotalRP']; ?></td>
          <td class="text-center">x</td>
          <td class="text-center">100%</td>
          <td class="text-center">=</td>
          <td class="text-center"><?php if(isset($row_S2curr['TotalRP'])) echo round($row_S2curr['TotalRP']); else echo "0"; ?></td>
          </tr>
        <tr>
          <td><span class="red_bold"><?php echo date("Y", strtotime(date("Y-m-d")." -1 year")); ?></span> - S1 Best total Ranking Points </td>
          <td class="text-center"><?php echo $row_S1_1year['TotalRP']; ?></td>
          <td class="text-center">x</td>
          <td class="text-center">65%</td>
          <td class="text-center">=</td>
          <td class="text-center"><?php if(isset($row_S1_1year['TotalRP'])) echo round($row_S1_1year['TotalRP']*65/100); else echo "0"; ?></td>
          </tr>
        <tr>
          <td><span class="red_bold"><?php echo date("Y", strtotime(date("Y-m-d")." -1 year")); ?></span> - S2 Best total Ranking Points </td>
          <td class="text-center"><?php echo $row_S2_1year['TotalRP']; ?></td>
          <td class="text-center">x</td>
          <td class="text-center">65%</td>
          <td class="text-center">=</td>
          <td class="text-center"><?php if(isset($row_S2_1year['TotalRP'])) echo round($row_S2_1year['TotalRP']*65/100); else echo "0"; ?></td>
          </tr>
        <tr>
          <td><span class="red_bold"><?php echo date("Y", strtotime(date("Y-m-d")." -2 year")); ?></span> - S1 Best total Ranking Points </td>
          <td class="text-center"><?php echo $row_S1_2year['TotalRP']; ?></td>
          <td class="text-center">x</td>
          <td class="text-center">35%</td>
          <td class="text-center">=</td>
          <td class="text-center"><?php if(isset($row_S1_2year['TotalRP'])) echo round($row_S1_2year['TotalRP']*35/100); else echo "0"; ?></td>
          </tr>
        <tr>
          <td><span class="red_bold"><?php echo date("Y", strtotime(date("Y-m-d")." -2 year")); ?></span> -  S2 Best total Ranking Points</td>
          <td class="text-center"><?php echo $row_S2_2year['TotalRP']; ?></td>
          <td class="text-center">x</td>
          <td class="text-center">35%</td>
          <td class="text-center">=</td>
          <td class="text-center"><?php if(isset($row_S2_2year['TotalRP'])) echo round($row_S2_2year['TotalRP']*35/100); else echo "0"; ?></td>
          </tr>
      </table>
</div>
...
<table class="table">
    <tr>
      ...
      <td class="text-right"><span class="italic">Total Weekly Ranking Points <?php echo $row_RPall['scr_total']; ?> X 15% = </span> &nbsp; <strong><?php echo $total_15_pc; ?></strong></td>
    </tr>
</table>
</div>
