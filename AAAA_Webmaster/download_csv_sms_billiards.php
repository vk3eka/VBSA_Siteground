...
function prepare_data() {
    ...
    $sql = sprintf("	
							SELECT 
								MobilePhone AS Mobile, members.LastName, members.FirstName
								
							FROM 
								scrs 
								
							LEFT JOIN 
								members ON members.MemberID = scrs.MemberID
								
							WHERE 
								game_type='Billiards'  
								AND MobilePhone is not null  
								AND (scrs.current_year_scrs = YEAR( CURDATE( ) )  OR scrs.current_year_scrs = YEAR( CURDATE( ) ) -1)
								AND ReceiveSMS=1
								
							GROUP BY scrs.MemberID
							HAVING SUM(count_played) >0
