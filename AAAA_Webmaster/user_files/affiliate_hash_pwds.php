...
{
  $sql_select = "Select * from vbsaorga_users2";
  $result = $dbcnx_client->query($sql_select);
  while ($row_data = $result->fetch_assoc()) 
  {
    if((substr($row_data['hashed_password'], 0, 6) != '$2y$10') && ($row_data['password'] != ''))
    {
      $sql_authorise = "Update vbsaorga_users2 Set hashed_password = '" . password_hash($row_data['password'], PASSWORD_DEFAULT) . "' Where vbsa_id = " . $row_data['vbsa_id'];
      ...
      {
        echo($row_data['vbsa_id'] . " password updated.<br>");
      }
    }
  }
}
...
$sql = "Select * from vbsaorga_users2 Order By board_member_id";
