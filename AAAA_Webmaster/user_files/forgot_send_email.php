...
$sql = "Select board_member_id from vbsaorga_users where username  = '" . $_GET['Email'] . "'";
$result_select_rego = $dbcnx_client->query($sql);
$row_email = $result_select_rego->fetch_assoc();
if($row_email['board_member_id'] == '')
{
     echo("No such email address!");
     return;
}
//echo($sql . "<br>");
...
// update new password
$sql = "Update vbsaorga_users SET hashed_password = '" . password_hash($_GET['Password'], PASSWORD_DEFAULT) . "' WHERE board_member_id = '" . $row_email['board_member_id'] . "'";
$update = $dbcnx_client->query($sql);
//echo($sql . "<br>");
// get members name from rego number
$sql = "Select * from members where MemberID  = " . $row_email['board_member_id'];
