...
//$query_RPall = "SELECT ranknum, memb_id, FirstName, LastName, Email, MobilePhone, total_tourn_rp, date_format(last_update,'%D %b %Y') AS last_update FROM rank_S_open_tourn LEFT JOIN members ON MemberID = memb_id ORDER BY total_tourn_rp DESC";
 $query_RPall = "Select 
        ...
        rank_aa_snooker_master.memb_id, FirstName, LastName, junior, Female, last_update  
        FROM rank_aa_snooker_master 
        LEFT JOIN members ON rank_aa_snooker_master.memb_id = MemberID
        where Junior != 'na' and Female != 1 ORDER BY (weekly_percent+total_tourn) DESC";
...
<table width="746" align="center">
    ...
    <tr>
      <td align="center" ><span class="red_bold"><?php echo date("Y"); ?> Victorian Tournament Snooker (Junior) Rankings</span> Last updated: <?php echo $row_RPall['last_update']; ?></td>
  ...
  <table align="center" cellpadding="5" cellspacing="5">
    ...
    do { 
      ...
      <tr>
        ...
        <td align="center" class="page"><a href="rankings_vic_snooker_detail.php?rank=<?php echo $row_RPall['memb_id']; ?>&cohort=junior">How awarded</a></td>
        </tr>
      <?php
      $i++;
      }
