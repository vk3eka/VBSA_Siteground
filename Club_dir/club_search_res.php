...
if (isset($_GET['clubfind'])) {
  $colname_Clubs = $_GET['clubfind'];
}
mysql_select_db($database_connvbsa, $connvbsa);
$query_Clubs = sprintf("SELECT * FROM clubs WHERE ClubTitle LIKE %s AND clubs.Club_Aff_Assoc='Club' AND inactive=1 ORDER BY ClubTitle ASC", GetSQLValueString($colname_Clubs . "%", "text"));
...
<div id="Wrapper"><!-- Wrapper contains content to a max width of 1200px--> 
  ...
  <div class="Page_heading_container">
 		<div class="page_title">Victorian Club Directory - Search results  </div>
  </div>  	
...
<!-- Club Search -->
  ...
  <div class="center-block" style="max-width:600px; height:35px; clear:both"> 
   ...
   <?php if($totalRows_Clubs>0 ) echo "Results for". ' "' .$colname_Clubs. '"'; else echo "No Results found"; ?></div>
   
   
   <!-- Club Information in repeated table -->
<?php if($totalRows_Clubs>0 ) do { 
