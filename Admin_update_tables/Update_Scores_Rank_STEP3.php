...
function GetTeamID($team_name)
{
  ...
  $query_id = "Select team_id, team_name from Team_entries where team_name = '" . $team_name . "' and team_season = '" . $current_season . "' and team_cal_year = YEAR(CURDATE())";
...
if(isset($_POST["submit"]))
{
  ...
  echo "<font face='arial' size='3'>STEP 3 completed go to ".'<span class="greenbg"><a href="Update_Scores_Rank_STEP4.php?season=' . $current_season . '">STEP 4</a></span>';
  echo '<br/><br/>';
  echo '<br/>';
  ...
  (   SELECT team_id, SUM(EF1) AS elim1tot 
      FROM scrs where current_year_scrs = YEAR( CURDATE( ) ) and scr_season = '" . $current_season . "' GROUP BY team_id
  ) T2
  ...
  // clear previous forfeits
  $query_clear = "Update vbsa3364_vbsa2.Team_entries SET count_team_forfeits = 0, count_player_forfeits = 0 WHERE team_cal_year = YEAR(CURDATE( )) and team_season = '" . $current_season . "'";
  //echo("Clear Forfeit " . $query_clear . "<br>");
  $result_delete = mysql_query($query_clear, $connvbsa) or die("<br><br><font face='arial' color='red'>Error setting forfeits to zero</font>");
  ...
  SET T1.count_team_forfeits = T2.forfeits
  WHERE team_cal_year = YEAR(CURDATE( ))";
  ...
  if (isset($result)) echo "<br><br><font face='arial' color='green'>3. Table was successfully updated - calculated count_team_forfeits and count_player_forfeits (Forfeits) OK</font>";
 ...
 $querytoexecute = "UPDATE vbsa3364_vbsa2.Team_entries  T1
  ...
  + COUNT( r17s ) + COUNT( r18s ) as forfeits
    FROM scrs
    WHERE scrs.MemberID = 100
    GROUP BY team_id
  ) T2 ON T1.team_id = T2.team_id
  SET T1.count_player_forfeits = T2.forfeits
  WHERE team_cal_year = YEAR(CURDATE( ))";
    
  $result=mysql_query($querytoexecute, $connvbsa) or die("<br><br><font face='arial' color='red'>Error count_player_forfeits was not calculated</font>");

  // added for opposition player forfeits

  $sql_billing_player = "Select count(memberID) as player_forfeits, opposition FROM vbsa3364_vbsa2.tbl_scoresheet where firstname = 'Player' and lastname = 'Forfeit' and year = YEAR(CURDATE( )) and season = '" . $current_season . "' Group by opposition";
  //echo("Player Forfeit " . $sql_billing_player . "<br>");
  $result_billing_player = mysql_query($sql_billing_player, $connvbsa) or die(mysql_error());
  while ($build_billing_player = mysql_fetch_assoc($result_billing_player))
  { 
    $query_player = "Update vbsa3364_vbsa2.Team_entries SET count_player_forfeits = " . $build_billing_player['player_forfeits'] . " WHERE team_id = " . GetTeamID($build_billing_player['opposition']);
    //echo("Player Update " . $query_player . "<br>");
    $result_player = mysql_query($query_player, $connvbsa) or die("<br><br><font face='arial' color='red'>Error count_player_forfeits was not calculated</font>");
  }
  // added for opposition team forfeits
  $sql_billing_team = "Select count(memberID) as team_forfeits, opposition FROM vbsa3364_vbsa2.tbl_scoresheet where firstname = 'Team' and lastname = 'Forfeit' and year = YEAR(CURDATE( )) and season = '" . $current_season . "' Group by opposition";
  //echo("Team Forfeit " . $sql_billing_team . "<br>");
  $result_billing_team = mysql_query($sql_billing_team, $connvbsa) or die(mysql_error());
  while ($build_billing_team = mysql_fetch_assoc($result_billing_team))
  { 
    $query_team = "Update vbsa3364_vbsa2.Team_entries SET count_team_forfeits = " . $build_billing_team['team_forfeits'] . " WHERE team_id = " . GetTeamID($build_billing_team['opposition']);
    //echo("Team Update " . $query_team . "<br>");
    $result_team = mysql_query($query_team, $connvbsa) or die("<br><br><font face='arial' color='red'>Error count_team_forfeits was not calculated</font>");
  }
  // end opposition forfeits

  //tested ok calculates rounds_played in the Team entries

  $querytoexecute = "UPDATE `vbsa3364_vbsa2`.`Team_entries` 	
  SET `rounds_played` = 	
  (SELECT SUM(count_matches-count_byes-count_team_forfeits-count_player_forfeits)/players+count_team_forfeits+count_player_forfeits)";
  ...
  for($i = 0; $i < 18; $i++)
  {
    ...
    $sql = "Select count(r" . $rnd_no . "pos) as Count, scr_season FROM scrs WHERE current_year_scrs = '" . $current_year . "' AND scr_season = '" . $current_season . "'";
    //echo($sql . "<br>");
  ...
  SET Junior = 	
  (SELECT CASE
  WHEN dob_year>=year(curdate())-21 AND dob_year<=year(curdate())-19 THEN 'U21'
...
//echo($querytoexecute . "<br>");
  ...
  if (isset($result)) echo "<br><br><font face='arial' color='green'> 4. Set last year paid members to null (last)</font>";
    mysql_close ($connvbsa);
}
else
{
  echo "<form id='form1' name='form1' method='post' action='../Admin_update_tables/Update_Scores_Rank_STEP3.php?submit=1&season=" . $current_season . "'>";
