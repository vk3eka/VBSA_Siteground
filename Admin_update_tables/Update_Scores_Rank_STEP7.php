...
if(isset($_POST["submit"]))
{
  ...
  //**************************************************************//
  //  Get last round for previous billiards ranking calculation   //
  //**************************************************************//

  $query_snooker_round = "Select round, date from tbl_fixtures where season = '$season' and year = $year and type = 'Snooker' and date < curdate() Order by date DESC Limit 1";
  //echo($query_snooker_round . "<br>");
  $result_snooker_round = mysql_query($query_snooker_round, $connvbsa) or die(mysql_error());
  $build_snooker_round = mysql_fetch_assoc($result_snooker_round);
  $last_snooker_round = $build_snooker_round['round'];
  //echo("Last Snooker Round Played " . $last_snooker_round . "<br>");
  if($last_snooker_round > 18)
  {
    $last_snooker_round = 18;
  }
  else
  {
    if($build_snooker_round['round'] == 1)
    {
      //$last_billiard_round = ($build_billiard_round['round']-1);
      $last_snooker_round = $build_snooker_round['round'];
    }
    //$last_billiard_round = ($build_billiard_round['round']-1);
    //break
  ...
  //}
    ...
    {
      //$last_billiard_round = $build_billiard_round['round'];
      $last_snooker_round = ($build_snooker_round['round']-1);
    }
  }
  $previous_round = ($last_snooker_round-1);
  //echo '<br/>'.'<br/>';
  //echo "<font face='arial' color='green'>Last Billiard Round Played " . $last_billiard_round . "</font><br>";
  //echo '<br/>';
  //echo("Last Snooker Round Played " . $last_snooker_round . "<br>");

  $rp_rounds = '';
  //$rp_tier_rounds = '';
  for($i = 1; $i <= $last_snooker_round; $i++)
  {
    if($i < 10 )
    {
      $i = '0' . $i;
  ...
  //echo("RP Rounds " . $rp_tier_rounds . "<br>");

  
  //************************************************************//
  //  Get last round for previous snooker ranking calculation   //
  //************************************************************//
/*
  for ($i = 1; $i < 19; $i++)
  {
    ...
    $query_round = "Select sum(r" . $i . "s) as total FROM vbsa3364_vbsa2.scrs where scr_season = '" . $season . "' and current_year_scrs = $year and game_type = 'Snooker'";
    //echo($query_round . "<br>");
    $result_round = mysql_query($query_round, $connvbsa) or die(mysql_error());
    $build_round = mysql_fetch_assoc($result_round);
    $last_round_total = $build_round['total'];
    //echo("Last Round Total " . $build_round['total'] . "<br>"); 
    if($last_round_total == '')
    {
      $last_round = ($i-1);
      break;
  ...
  //echo("Last Round " . $last_round . "<br>"); 

  // added to cater for start ofseason where no rounds played
  if(($last_round == '') || ($last_round == 0))
  {
    $last_round = 1;
  }
  //echo("Last Round " . $last_round . "<br>"); 
...
*/
  //echo("RP " . $i . "<br>"); 
  //echo("RP " . $rp_rounds . "<br>"); 

  


  //***********************************************************************//
  //  Get last round date for previous tournament ranking calculation    //
  //***********************************************************************//

  $query_tourn_date = "Select distinct finishdate FROM vbsa3364_vbsa2.calendar where finishdate > '$year-01-01' and finishdate < curdate() order by finishdate desc limit 1, 1;";
  $result_tourn_date = mysql_query($query_tourn_date, $connvbsa) or die(mysql_error());
  $build_tourn_date = mysql_fetch_assoc($result_tourn_date);  
  $last_tourn_date = ($build_tourn_date['finishdate']);

  //********************************************************************//
  //  Get last round date for previous pennant ranking calculation      //
  //********************************************************************//

  $query_breaks_round = "Select round, date from tbl_fixtures where season = '$season' and year = $year and round = " . $previous_round . " Order by date DESC Limit 1";
  ...
  while($build_result_snooker_rp = $result_previous_snooker_rp->fetch_assoc())
  {
      ...
      {
        $S1_value = $build_result_snooker_rp['scr_curr_S1'];
        $S2_value = 0;
      }
      ...
      scr_curr_S1 = " . $S1_value . ", 
      scr_curr_S2 = " . $S2_value . ", 
  ...
  while($build_result_billiards_rp = $result_previous_billiards_rp->fetch_assoc())
  {
    ...
    {
      $S1_value = $build_result_billiards_rp['brks_curr_S1'];
      $S2_value = 0;
      $S1_breaks_value = 0;
      $S2_breaks_value = 0;
    }
    ...
    brks_curr_S1 = " . $S1_breaks_value . ",
    brks_curr_S2 = " . $S2_breaks_value . "
    Where memb_id = " . $build_result_billiards_rp['memb_id'];
    //echo($sql_update_billiards . "<br>");

    // need to add back for S2
  ...
  $query_members = 'Select memb_id from temp_snooker_master';
  $result_members = mysql_query($query_members, $connvbsa) or die(mysql_error());
  while($build_members = $result_members->fetch_assoc())
  {
    $query_rounds_rp = "Select ((" . $rp_rounds . ")*allocated_rp) as total_rp, MemberID from scrs where scr_season = '$season' and current_year_scrs = $year and MemberID = " . $build_members['memb_id'] . " and game_type = 'Snooker' Order By total_rp Desc LIMIT 1";
    //echo($query_rounds_rp . "<br>");
    $result_rounds_rp = mysql_query($query_rounds_rp, $connvbsa) or die(mysql_error());
    $build_result = $result_rounds_rp->fetch_assoc();
    if($build_result['total_rp'] > 0)
    {
      $sql_insert = "Update temp_snooker_master Set scr_curr_" . $season . " = " . $build_result['total_rp'] . " Where memb_id = " . $build_result['MemberID'];
      //echo($sql_insert . "<br>");
  ...
  $query_tournament = "Select tourn_memb_id, SUM(rank_pts) as points, tourn_date_ent, tourn_entry.tourn_type as comptype, tournament_number FROM vbsa3364_vbsa2.tournaments left Join calendar on calendar.tourn_id = tournaments.tourn_id left Join tourn_entry on tourn_entry.tournament_number = tournaments.tourn_id where tourn_entry.tourn_type = 'Snooker' and finishdate > '2025-01-01' and finishdate <= '" . $last_tourn_date . "' and rank_pts > 0 group by tourn_memb_id";
  //echo($query_tournament . "<br>");
  $result_tournament = mysql_query($query_tournament, $connvbsa) or die(mysql_error());
  //echo("Rows " . $result_tournament->num_rows);
  $i = 1;
  while($build_tournament = $result_tournament->fetch_assoc())
  {
    //echo("Tourn Pts " . $build_tournament['rank_pts'] . "<br>");
    $sql_insert = "Update temp_snooker_master Set tourn_curr = " . $build_tournament['points'] . " Where memb_id = " . $build_tournament['tourn_memb_id'];
    //echo($sql_insert . "<br>");
    $result_insert = mysql_query($sql_insert, $connvbsa) or die(mysql_error());
    $i++;
  }
  ...
  $query_tournament = "Select tourn_memb_id, rank_pts, tourn_date_ent, tourn_entry.tourn_type as comptype, tournament_number FROM vbsa3364_vbsa2.tournaments left Join calendar on calendar.tourn_id = tournaments.tourn_id left Join tourn_entry on tourn_entry.tournament_number = tournaments.tourn_id LEFT JOIN members ON members.MemberID = tourn_entry.tourn_memb_id where tourn_entry.tourn_type = 'Snooker' and Junior != 'na' and Gender != 'Female' and finishdate > '". $year . "-01-01' and finishdate <= '" . $last_tourn_date . "' and rank_pts > 0 group by tourn_memb_id";
  //echo($query_tournament . "<br>");
  $result_tournament = mysql_query($query_tournament, $connvbsa) or die(mysql_error());
  $i = 1;
  while($build_tournament = $result_tournament->fetch_assoc())
  ...
  $query_billiard_round = "Select round, date from tbl_fixtures where season = '$season' and year = $year and type = 'Billiards' and date < curdate() Order by date DESC Limit 1";
  //echo($query_billiard_round . "<br>");
  $result_billiard_round = mysql_query($query_billiard_round, $connvbsa) or die(mysql_error());
  $build_billiard_round = mysql_fetch_assoc($result_billiard_round);
  $last_billiard_round = $build_billiard_round['round'];

  if($last_billiard_round > 18)
  {
    $last_billiard_round = 18;
  }
  else
  {
    if($build_billiard_round['round'] == 1)
    {
      //$last_billiard_round = ($build_billiard_round['round']-1);
      $last_billiard_round = $build_billiard_round['round'];
    }
  ...
  //echo "<font face='arial' color='green'>Last Billiard Round Played " . $last_billiard_round . "</font><br>";
  //echo '<br/>';
  //echo("Last Billiard Round Played " . $last_billiard_round . "<br>");

  //$rp_rounds = '';
  $rp_tier_rounds = '';
  for($i = 1; $i <= $last_billiard_round; $i++)
  {
    if($i < 10 )
    {
      $i = '0' . $i;
  ...
  //echo("RP Rounds " . $rp_tier_rounds . "<br>");
  //************************************************************//
  //  Get last round date for previous ranking calculation      //
  //************************************************************//

  $query_breaks_round = "Select round, date from tbl_fixtures where season = '$season' and year = $year and type = 'Billiards' and round = " . $last_billiard_round . " Order by date DESC Limit 1";
  //echo($query_breaks_round . "<br>");
  $result_breaks_round = mysql_query($query_breaks_round, $connvbsa) or die(mysql_error());
  $build_breaks_round = mysql_fetch_assoc($result_breaks_round);  
  $last_billiard_date = ($build_breaks_round['date']);

  //echo("Last Date Played " . $last_biiliard_date . "<br>");
  ...
  //$rp_rounds =  rtrim($rp_rounds,'+');
  $rp_tier_rounds =  rtrim($rp_tier_rounds,'+');

  $query_members = 'Select memb_id from temp_billiards_master';
  ...
  //echo($query_members . "<br>");
  $result_members = mysql_query($query_members, $connvbsa) or die(mysql_error());
  //$result_rounds_rp = mysql_query($query_rounds_rp, $connvbsa) or die(mysql_error());

  while($build_members = $result_members->fetch_assoc())
  {
    $query_rounds_rp = "Select (" . $rp_tier_rounds . ") as total_rp, MemberID from scrs where scr_season = '$season' and current_year_scrs = $year and MemberID = " . $build_members['memb_id'] . " and game_type = 'Billiards' Order By total_rp Desc LIMIT 1";
    //$query_rounds_rp = "Select total_RP, MemberID from scrs where scr_season = '$season' and current_year_scrs = $year and MemberID = " . $build_members['memb_id'] . " and game_type = 'Billiards' Order By total_RP Desc LIMIT 1";
    //echo($query_rounds_rp . "<br>");
    $result_rounds_rp = mysql_query($query_rounds_rp, $connvbsa) or die(mysql_error());
    $build_result = $result_rounds_rp->fetch_assoc();
    if($build_result['total_rp'] > 0)
    {
      $sql_insert = "Update temp_billiards_master Set scr_curr_$season = " . $build_result['total_rp'] . " Where memb_id = " . $build_result['MemberID'];
      //echo($sql_insert . "<br>");
      $result_insert = mysql_query($sql_insert, $connvbsa) or die(mysql_error());
    }
  }
...
/*
  ...
  $last_date = ($build_round['date']);
*/
  $query_breaks = "Select sum(bill_rp) as total_rp, member_ID_brks FROM vbsa3364_vbsa2.breaks where recvd > '2025-01-01' and recvd <= '" . $last_billiard_date . "'  and brk_type = 'Billiards' and season = '$season' group by member_ID_brks order by member_ID_brks";
  //$query_breaks = "Select sum(bill_rp) as total_rp, member_ID_brks FROM vbsa3364_vbsa2.breaks where  recvd <= '" . $last_billiard_date . "' and brk_type = 'Billiards' and season = 'S1' and season = 'S1' Group By member_ID_brks order by member_ID_brks";
  ...
  {
    $sql_breaks = "Update temp_billiards_master Set brks_curr_$season = " . $build_breaks['total_rp'] . " Where memb_id = " . $build_breaks['member_ID_brks'];
    //echo($sql_breaks . "<br>");
  ...
  $query_tournament = "Select tourn_memb_id, rank_pts, tourn_date_ent, tourn_entry.tourn_type as comptype, tournament_number FROM vbsa3364_vbsa2.tournaments left Join calendar on calendar.tourn_id = tournaments.tourn_id left Join tourn_entry on tourn_entry.tournament_number = tournaments.tourn_id where tourn_entry.tourn_type = 'Billiards' and finishdate > '2025-01-01' and finishdate <= '" . $last_billiard_date . "' and rank_pts > 0 group by tourn_memb_id";
  ...
  $query_tournament = "Select tourn_memb_id, rank_pts, tourn_date_ent, tourn_entry.tourn_type as comptype, tournament_number FROM vbsa3364_vbsa2.tournaments left Join calendar on calendar.tourn_id = tournaments.tourn_id left Join tourn_entry on tourn_entry.tournament_number = tournaments.tourn_id LEFT JOIN members ON members.MemberID = tourn_entry.tourn_memb_id where tourn_entry.tourn_type = 'Billiards' and Gender = 'Female' and finishdate > '2025-01-01' and finishdate <= '" . $last_billiard_date . "' and rank_pts > 0 group by tourn_memb_id";
  //echo($query_tournament . "<br>");
  $result_tournament = mysql_query($query_tournament, $connvbsa) or die(mysql_error());
  $i = 1;
  while($build_tournament = $result_tournament->fetch_assoc())
  ...
  $querytoexecute = "Update temp_billiards_master SET total_rp=(scr_2yr_S2)+(scr_2yr_S1)+(scr_1yr_S2)+(scr_1yr_S1)+scr_curr_S2+scr_curr_S1+(brks_2)+(brks_1)+brks_curr_S1+(tourn_2)+(tourn_1)+tourn_curr";
  ...
  echo '<br/>'.'<br/>';
  //echo '<br/>'.'<br/>';
  echo "<font face='arial' color='green' size='3'>Calculations completed.</font>";
  echo '<br/><br/>';
...
{
  echo '<center/>';
  echo "<form id='form1' name='form1' method='post' action='../Admin_update_tables/Update_Scores_Rank_STEP7.php?submit=1&season=" . $season . "'>";
