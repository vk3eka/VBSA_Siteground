...
$query_contact = "SELECT * FROM SBSA_contact WHERE contact_current=1 ORDER BY contact_order";
$contact = mysql_query($query_contact, $connvbsa) or die(mysql_error());
$row_contact = mysql_fetch_assoc($contact);
$totalRows_contact = mysql_num_rows($contact);

mysql_select_db($database_connvbsa, $connvbsa);
$query_about = "SELECT item_title, news_content FROM SBSA WHERE SBSA_id =7";
...
mysql_select_db($database_connvbsa, $connvbsa);
$query_header = "SELECT item_title, news_content, SBSA.pagezone_header_desc, pagezone_header FROM SBSA WHERE SBSA_id=8";
...
<title>
	<?php 
	if(!empty($row_contact['contact_id']) && !empty($row_about['item_title']))  echo "SBSA Contact & About"; 
	if(empty($row_contact['contact_id']) && !empty($row_about['item_title']))  echo "SBSA About"; 
	if(!empty($row_contact['contact_id']) && empty($row_about['item_title']))  echo "SBSA Contact";
	?>
</title>
...
<div class="table-responsive center-block" style="max-width:600px; padding-left:3px"> <!-- class table-responsive -->
  <table class="table">
  	<tbody>
      <tr>
        <td colspan="4" class="text-center italic">Board Members and contact details</td>
        </tr>
      <tr>
        <th>&nbsp;</th>
        <th>Position</th>
