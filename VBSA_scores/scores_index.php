...
$query_page_items = "SELECT webpage_items.ID, webpage_items.Header, webpage_items.`Comment`, webpage_items.`By`, webpage_items.created_on, webpage_items.blocked, webpage_items.img_orientation, webpage_items.item_image,  webpage_items.event_id,  webpage_items.page_scores, webpage_items.OrderScores, webpage_items.img_size FROM webpage_items WHERE webpage_items.blocked='No' AND webpage_items.page_scores='Y' ORDER BY OrderScores, created_on DESC";
...
<div class="italic center-block" style="width:75%">
      <p class="text-center right_title">Check Scores</p>
      <p class="text-center">Pennant Season 1 - (S1)</p>
      <p class="text-center">1st half of Year</p>
      <p class="text-center"><a href="scores_index_detail.php?season=S1&year=<?php echo(date('Y')); ?>" class=" btn-sm btn-primary btn-responsive" role="button">S1</a></p>
      <p class="text-center">Pennant Season 2 - (S2)</p>
      <p class="text-center">2nd half of Year</p>
      <p class="text-center"><a href="scores_index_detail.php?season=S2&year=<?php echo(date('Y')); ?>" class=" btn-sm btn-primary btn-responsive" role="button">S2</a></p>
      <p class="text-center">Problem with the scores?</p> 
      <p class="text-center"><a href="mailto:scores@vbsa.org.au">Email the Score Registrar</a></p>
...
<?php include '../includes/buttons_left.php';?> 
  ...
  <div ID="News_container">
    ...
    <div class="Page_heading_container">
 		<div class="index_title"><span class="h4">VBSA Scores</span></div>
		...
		
		{
			...
			$path_info_att = pathinfo($row_att['up_pdf_name']); 
			   	...
			   	if ($row_att['up_type'] == 'Attachment'):
					...
					// pdf viewer link
          echo '<td style="width:20%"><a href="../pdfjs/web/viewer.html?file=../../Front_page_upload/' . rawurlencode($row_att['up_pdf_name']) . '" title="View"><span class="glyphicon glyphicon-eye-open"></span></a></td>';
					}
					else echo '<td nowrap="nowrap" style="width:60%">'.$row_att['up_desc'].'</td>'.'<td style="width:20%">&nbsp;</td>' ;
					
					// download link
          echo '<td style="width:20%"><a href="../pdfjs/web/viewer.html?file=../../Front_page_upload/' . rawurlencode($row_att['up_pdf_name']) . '" title="Download"><span class="glyphicon glyphicon-download"></span></a></td>';
					echo '</tr>';
				endif;
