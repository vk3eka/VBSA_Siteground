...
// email scores director a reminder of the semi/final
/*
if($title == 'Semi Final')
{
    $sql_alert = "Select * from tbl_fixtures where team_grade = '" . $teamgrade . "' and season = '" . $season . "' AND year = " . $current_year . " Order By round DESC Limit 1";
    //echo($sql_alert . "<br>");
    $result_alert = $dbcnx_client->query($sql_alert) or die("Couldn't execute fixture query. " . mysqli_error($dbcnx_client));
    $build_alert = $result_alert->fetch_assoc();
    $semi_finals_round = ($build_alert['round']-2);
    $round_date = date('Y-m-d', strtotime($build_alert['date'] . ' - 7 days'));

    if((date('Y-m-d', strtotime($build_alert['date'] . ' - 2 days')) == (date('Y-m-d'))))
    {
        AuthoriseEmail('scores@vbsa.org.au', $teamgrade, $round_date);
    }

}

function AuthoriseEmail($email, $team_grade, $round_date)
{
    $subject = 'VBSA Scoring APP System'; 
    $message = '<html><body>';
    $message .= "<p>Mark,</p>";
    $message .= "<p>Semi Finals are due to take place for " . $team_grade . " on " . $round_date . ".</p>";
    $message .= "<p>Thanks.</p>";
