...
$sql_grades = "Select * From tbl_team_grade Where grade = '" . $team_grade . "' AND season = '" . $season . "'";
...
while ($build_data = $result_scoresheet->fetch_assoc()) 
{
  ...
  elseif($build_data['type'] == 'Snooker')
  {
    ...
    {   
      ...
      {
        echo("document.getElementById('score_" . $j . "_" . $i . "').value = '0';");
        ...
        for($i = 0; $i < $no_of_players; $i++) // no of players
        {
          ...
          for($j = 0; $j < $no_of_games; $j++) // no of games
          {
            ...
            elseif($type == 'Billiards')
            {
              ...
              echo("<td align=center><input type='text' id='billiard_stick_" . $i . "' name='billiard_stick_" . $i . "' style='width:50px' " . $readonly . " tabindex=" . $no_of_games . "  value='0' onchange='this.value = parseInt(this.value);'></td>");
            }
...
$(document).ready(function()
{
  ...
  */
    ...
    $.ajax({
      ...
      success : function(data){
        ...
        for(k = 0; k < obj.length; k++)
        {
          ...
          final_sub = fullobj[5];
          tier  = fullobj[6];
          //console.log("Obj " + tier + ", " + fullname);
          if(fullobj[2] != 1) // remove Bye
          {
            output += ("<tr>");
            if((count_played < 4))
            //if((count_played < 4) && ((title == 'Semi Final') || (title == 'Grand Final')))
            {
        ...
        $("input[type='radio']").click(function(){
          ...
          {
            alert("Player " + $('#fullname' + row).html() + " is not qualified to play finals.\n\nContact the Scores Registrar to obtain permission to play.");
            $("input[data-col='" + col + "']").prop("checked", false);
            return;
          }
        });
  ...
  {
    if(($.isNumeric($("#brk7").val())) && ($("#brk7").val() >= min_break))
    {
  ...
  {
    if(($.isNumeric($("#brk8").val())) && ($("#brk8").val() >= min_break))
    {
