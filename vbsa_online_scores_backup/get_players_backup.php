...
$sql = "Select scrs.scrsID, scrs.MemberID, scrs.team_grade, scrs.team_id, scrs.final_sub, members.MemberID, members.FirstName, members.LastName, Team_entries.team_id, Team_entries.team_name, Team_entries.team_grade, count_played FROM scrs, members, Team_entries WHERE members.MemberID=scrs.MemberID AND Team_entries.team_id=scrs.team_id AND Team_entries.team_id=" . $team_id . " ORDER BY members.FirstName";
...
if ($num_rows != 0) 
{
  ...
  {
    $players[$i] = ((trim($build_data['FirstName'])) . ", " . (trim($build_data['LastName'])) . ", " . $build_data['MemberID'] . ", " . $build_data['team_id'] . ", " . $build_data['count_played'] . ", " . $build_data['final_sub']); 
    $i++;
  }
