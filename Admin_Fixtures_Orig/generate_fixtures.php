...
while($build_data_grades = $result_grades->fetch_assoc())
{
    ...
    while($build_data_club = $result_club_display->fetch_assoc())
    {
        ...
        echo('<tr data-index="' . $index . '" id=' . $build_data_club["team_grade"] . ',' . $build_data_club["team_id"] . '> 
...
// get dates for rounds/date row
echo('<tr>');
for($k = 0; $k < $max_no_of_teams; $k++)
{
    $sql_dates = "Select distinct(date), round From tbl_create_fixtures Where season = '" . $season . "' and year = " . $year . " and dayplayed = '" . $dayplayed . "' and round = " . ($k+1);
    //echo($sql_dates . "<br>");
    $result_dates = mysql_query($sql_dates, $connvbsa) or die(mysql_error());
    $build_dates = $result_dates->fetch_assoc();
    echo("<td align='center' id='date_" . $k . "'>" . $build_dates['date'] . "</td>");
}
echo('</tr>');
/*
echo('<tr>');
...
while($build_data_menu = $result_grades_menu->fetch_assoc())
{
    ...
    {   
        echo("<tr>");
        echo('<td colspan=4 align="center" id="' . $build_data_menu['team_grade'] . '_round_' . ($y+1) . '">' . $build_data_menu['team_grade'] . '</td>');
        for($j = 0; $j < $no_of_rounds; $j++)
        {
            $sql_analysis_data = 'Select date, team_grade, round, fix1home, fix2home, fix3home, fix4home, fix5home, fix6home, fix7home from tbl_create_fixtures Where team_grade = "' . $build_data_menu['team_grade'] . '" and season = "' . $season . '" and year = ' . $year . ' and round = ' . ($j+1);
            $result_analysis_data = mysql_query($sql_analysis_data, $connvbsa) or die(mysql_error());
            $build_analysis_data = $result_analysis_data->fetch_assoc();
            if(($build_analysis_data['fix' . ($y+1) . 'home'] == "NA") || ($build_analysis_data['fix' . ($y+1) . 'home'] == "TBA"))   
            {
                $team_name = '';
            } 
            else
            {
                $team_name = $build_analysis_data['fix' . ($y+1) . 'home'];
            } 
            echo('<td align="center" class="round_' . $j . '" id="' . $build_data_menu['team_grade'] . '_round_' . ($j+1) . '_pos_' . ($y+1) . '">' . $team_name . '</td>');
        }
        echo("</tr>");
        $i++;
    }
...
for($k = 0; $k < $max_no_of_teams; $k++)
{
    ...
    $result_dates = mysql_query($sql_dates, $connvbsa) or die(mysql_error());
    $build_dates = $result_dates->fetch_assoc();
    echo("<td align='center' id=' table_date_" . $k . "'>" . $build_dates['date'] . "</td>");
}
echo('</tr>');
/*
echo('<tr>');
