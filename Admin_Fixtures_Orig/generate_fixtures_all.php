...
{   
    ...
    $AwayTeamArray = '';
    $numOfTrue = 0;
    $HomeTeamArray = array_filter(explode(", ", $home_array));
    $AwayTeamArray = array_filter(explode(", ", $away_array));
...
{   
    ...
    //echo("<pre>");
    //echo(var_dump($teamArray) . ", Round " . $round . "<br>");
    //echo("</pre>");
...
<table class='table dt-responsive nowrap display' align='center' width='800px'>
  ...
  <tr>
    ...
    <!--<td align="center" nowrap="nowrap" class="greenbg"><a href="create_fixtures_pdf.php?Year=<?= $year ?>&Season=<?= $season ?>";>Generate Fixtures PDF all <?= $season ?> grades.</a></td>-->
...
function show_fixtures($names, $team_grade, $form_no, $year, $season)
{
    ...
    if($teams == 4) // needs checking with a four team fixture.
    {
        ...
        for ($i = 0; $i < sizeof($rounds); $i++) {
            ...
            foreach ($rounds[$i] as $r) { // set 5
                $round_data = explode(" v ", $r);
                echo("<tr>");
                echo("<td align='center'><input class='float-child' type='text' id='A_" . $team_grade . "_home_" . ($i+12) . "_" . ($x+1) . "' value='" . $round_data[0] . "' style='width:200px'></td>");
                echo("<td align='center'>v</td>");
                echo("<td align='center'><input class='float-child' type='text' id='A_" . $team_grade . "_away_" . ($i+12) . "_" . ($x+1) . "' value='" . $round_data[1] . "' style='width:200px'></td>");
                echo("</tr>");
                $x++;
            }
        }
    ...
    for ($i = 0; $i < sizeof($rounds); $i++) {
        echo("<tr><td>&nbsp;</td></tr>");
        echo("<td colspan=3 align='center'><b>Round (1) " . ($i+1)  . "</b></td></tr>");
        echo("<tr><td align='right'><b>Date</b></td>");
        echo("<td colspan=2 class='text-left'><input type='text' id='A_" . $form_no . "_date_" . $i . "' style='width:100px'></td></tr>");
        $x = 0;
        foreach ($rounds[$i] as $r) {
    ...
    for ($i = 0; $i < sizeof($rounds); $i++) {
        echo("<tr><td>&nbsp;</td></tr>");
        echo("<td colspan=3 align='center'><b>Round (2) " . ($round_counter)  . "</b></td></tr>");
        echo("<tr><td align='right'><b>Date</b></td>");
    ...
    for ($i = 0; $i < sizeof($rounds); $i++) {
        echo("<tr><td>&nbsp;</td></tr>");
        echo("<td colspan=3 align='center'><b>Round (3) " . ($i+1)  . "</b></td></tr>");
        echo("<tr><td align='right'><b>Date</b></td>");
        echo("<td colspan=2 class='text-left'><input type='text' id='B_" . $form_no . "_date_" . $i . "' style='width:100px'></td></tr>");
        $x = 0;
        foreach ($rounds[$i] as $r) {
            $round_data = explode(" v ", $r);
            echo("<tr>");
    ...
    for ($i = 0; $i < sizeof($rounds); $i++) {
        echo("<tr><td>&nbsp;</td></tr>");
        echo("<td colspan=3 align='center'><b>Round (4) " . ($round_counter)  . "</b></td></tr>");
        echo("<tr><td align='right'><b>Date</b></td>");
        echo("<td colspan=2 class='text-left'><input type='text' id='B_" . $form_no . "_date_" . $round_counter . "' style='width:100px'></td></tr>");
        $x = 0;
        $round_counter += 1;
        foreach ($rounds[$i] as $r) {
            $round_data = explode(" v ", flip($r));
    ...
    for ($i = 0; $i < ((sizeof($rounds)/2)-1); $i++) {
        echo("<tr><td>&nbsp;</td></tr>");
        echo("<td colspan=3 align='center'><b>Round (5) " . ($i+11)  . "</b></td></tr>");
        echo("<tr><td align='right'><b>Date</b></td>");
        echo("<td colspan=2 class='text-left'><input type='text' id='A_" . $form_no . "_date_" . ($i+11) . "' style='width:100px'></td></tr>");
        $x = 0;
        //$round_counter += 1;
        foreach ($rounds[$i] as $r) {
            $round_data = explode(" v ", $r);
            echo("<tr>");
            echo("<td align='center'><input class='float-child' type='text' id='B_" . $team_grade . "_home_" . ($i+1) . "_" . ($x+1) . "' value='" . $round_data[0] . "' style='width:200px'></td>");
            echo("<td align='center'>v</td>");
            echo("<td align='center'><input class='float-child' type='text' id='B_" . $team_grade . "_away_" . ($i+1) . "_" . ($x+1) . "' value='" . $round_data[1] . "' style='width:200px'></td>");
            echo("</tr>");
            $x++;
        }
    }
