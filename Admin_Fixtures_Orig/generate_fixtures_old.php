...
foreach($data['teams'] as $fix)
{
    echo "Name = " . $fix['name'] . "<br>";
    echo "Club = " . $fix['club'] . "<br>";
}
*/

//$jsonData = json_encode($fixture);

//echo "FixtureData = '" . $jsonData . "';";
...
foreach($team_grade as $grade)
{
    if($grade != '')
    {
        $sql = "Select team_name FROM Team_entries WHERE team_season ='$season' AND team_cal_year = $year AND team_grade = '$grade' AND day_played = '$dayplayed' ORDER BY team_grade, team_name";
...
?>
<!--<script src="jquery_shuffle.js"></script>-->
<script>

$(document).ready(function()
{
    $.fn.shuffle = function(sortorder) {
        //var allElems = this.get(),
        getRandom = function(max) {
        return Math.floor(Math.random() * max);
        },
        shuffled = $.map(sortorder, function(){
            var random = getRandom(sortorder.length),
            randEl = $(sortorder[random]).clone(true)[0];
            sortorder.splice(random, 1);
            return randEl;
        });
    ...
    $.fn.save_fixtures = function (form_no, action) {
        ...
        //index = sortdata[1].split(', ');
        //console.log("Before " + index[1]);
        //sortdata = $.fn.shuffle(index[1]);
        //console.log("After " + sortdata);
    ...
    $('.savebutton').click(function()
    {
        ...
        $.fn.save_fixtures(form_no, 'NoResponse');
        window.location.href = '<?= $_SERVER['PHP_SELF'] ?>?DayPlayed=' + dayplayed + '&season=' + season;
    });
    ...
    $('.shufflebutton').click(function()
    {
        ...
        $('#table_util').empty();
        //form_no = 1;
        for (form_no = 1; form_no < 4; form_no++)
        {
            $.fn.save_fixtures(form_no, 'NoResponse');
        }
        ...
        //alert(team_grades);
        $.ajax({
          url:"get_table_util.php?dayplayed=" + dayplayed+ "&team_grades=" + team_grades + "&max_teams=" + max_no_of_teams + "&year=" + current_year + "&season=" + current_season,
        ...
        /*
        $.ajax({
          url:"get_analysis.php?dayplayed=" + dayplayed+ "&team_grades=" + team_grades + "&max_teams=" + max_no_of_teams + "&year=" + current_year + "&season=" + current_season,
              success : function(data){
                //obj = jQuery.parseJSON(data);
                console.log(data);
                $($.parseHTML(data)).appendTo('#analysis');
...
{   
    ...
    $sql_create = 'Select * from tbl_create_fixtures Order By team_grade, round';
...
var shuffle_page = false;
//var shuffle_page = true;

function FirstLoad() {
    if(shuffle_page === true)
    {
        //console.log("Page 10");
...
<table class='table dt-responsive nowrap display' align='center' width='800px'>
  ...
  <tr>
    <td colspan="3" align="center">Refresh Count <?= $_SESSION['refresh_count'] ?></td>
  </tr>
  <tr>
...
/*
    foreach($data['teams'] as $fix)
    {
        $fixtures = $fix['name'] . ", " . $fixtures;
        $team_name = $fix['name'] . "<br>";
        $club_name = $fix['club'] . "<br>";
    }
*/
 ...
 ?>
    ...
    echo("</tr>");
    echo("<tr>");
    echo("<td align='center' colspan='2' class='greenbg' style='height: 20px'><a href='create_fixtures_pdf.php?Year=" . $year . "&Season=" . $season . "&Team_Grade=" . $team_grade . "&DayPlayed=" . $dayplayed . "&Rounds=" . $total_rounds . "';>Generate Fixtures PDF for " . $team_grade . "</a></td>");
    echo("</tr>");
    echo("</table>");
...
/*
    ...
    echo("<table class='table table-striped table-bordered dt-responsive nowrap display' border='0'>");
    echo("<tr>");
    ...
    {    
        echo("<tr>");
        echo("<td align='center'>Team " . ($i+1) . "</td>");
        echo("<td align='center'><input type='text' id='C_" . $team_grade . "_home_" . ($i+1) . "_" . ($x+1) . "' value='" . $team[$i] . "' style='width:200px'></td>");
        echo("<td align='center'>Team " . ($i+5) . "</td>");
        echo("<td align='center'><input type='text' id='C_" . $team_grade . "_away_" . ($i+5) . "_" . ($x+1) . "' value='" . $team[$i+4] . "' style='width:200px'></td>");
    ...
    echo("<table class='table table-striped table-bordered dt-responsive nowrap display' border='1'>");
    echo("<tr>");
    echo("<td align='center'><input type='text' value='1 v 5' style='width:50px'></td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td align='center'><input type='text' value='2 v 8' style='width:50px'></td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td align='center'><input type='text' value='3 v 6' style='width:50px'></td>");
    echo("</tr>");
    echo("<tr>");
    echo("<td align='center'><input type='text' value='4 v 7' style='width:50px'></td>");
    echo("</tr>");
    echo("</table>");
...
// added
    ...
    echo('<br>');
    echo($team_grade . " ". $form_no . " ". $year . " ". $season . " ". $dayplayed . " ". $rounds . "<br>");
...
echo('<table class="table table-striped table-bordered dt-responsive display text-center" border="1">
<tr>
    <td colspan=' . ($max_no_of_teams+4) . ' align="center"><b>Table Utilisation (' . $dayplayed . ')</td>
</tr>
<tr>
