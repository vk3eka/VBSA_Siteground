...
//echo($current_season . "<br>");
//echo($dayplayed . "<br>");

$fixture = new Fixture();
$fixture->LoadFixture($current_year, $current_season, $dayplayed);
$jsonData = json_encode($fixture);
$data = json_decode($jsonData, true);
//echo($data['non_dates'] . "<br>");
$public_holiday = $data['non_dates'];
//echo("<pre>");
//echo(var_dump($data['non_dates']));
//echo("</pre>");
//echo($jsonData . "<br>");
...
function PopulateCalendar() 
{
    ...
    for(i = 0; i < no_of_rounds; i++)
    {
        ...
        myCalendar.attachEvent("onClick",function(date){
               DateChange(i, no_of_rounds);
        });
    }
}

//function getFinalsDate(publicHoliday, lastDate) 
//{
    //alert("Here");
    /*
  let finalsDate;

  publicHoliday.forEach(vbsaHoliday => {
    // Add 7 days to lastDate
    const lastPlus7 = addDays(lastDate, 7);
    if (lastPlus7 === vbsaHoliday.date) {
      // If match, finals date is holiday + 7 days
      finalsDate = addDays(vbsaHoliday.date, 7);
    } else {
      // Otherwise finals date is lastDate + 7 days
      finalsDate = lastPlus7;
    }
  });
...
{
    console.log(r + ", " + no_of_rounds);
    // need to take PH into account
    for(i = r; i < (no_of_rounds); i++)
    {
...
//$sql = "Select * FROM Team_grade where fix_cal_year = 2025 and season = 'S2' and current = 'Yes' Order By grade";
//echo($sql . "<br>");
$result_fixture = mysql_query($sql, $connvbsa) or die(mysql_error());
