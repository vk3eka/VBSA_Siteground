...
{
    // get number of rounds based on number of teams (no finals)
    switch ($teams) {
        case 4:
            $rounds = 15;
            break;
...
//echo($_SESSION['session_season'] . "<br>");
...
if ($_POST['ButtonName'] == "SaveFixtures") 
{
    ...
    for ($i = 0; $i < (count($scoredata)+1); $i++) 
    {
        ...
        if($k == 0)
        {
            ...
            $team_grade . "', '" . 
            $dayplayed . "')";
            //echo($sql_insert . ", Round " . $scoresheet[3] . ", K = " . $k . "<br>");
...
$sql_grades = "Select * From Team_grade Where grade = '" . $team_grade . "' and season = '" . $current_season . "' and fix_cal_year = " . $current_year;
//echo($sql_grades . "<br>");
...
if($_POST['Select'] == 'true')
{
    ...
    if($num_rows > 0)
    {
        ...
        $sql = "Select team_name FROM Team_entries WHERE team_season ='$current_season' AND team_cal_year = '$current_year' AND team_grade = '$team_grade' AND day_played = '$dayplayed'";
        //echo($sql . "<br>");
        $result_teams = mysql_query($sql, $connvbsa) or die(mysql_error());
        $no_of_teams = $result_teams->num_rows;
        //$no_of_fixtures = ($no_of_teams/2);
        $no_of_rounds = (GetRounds($no_of_teams)+2);

        // get last date before finals
        //$sql_last = 'Select date from tbl_fixtures where year = ' . $current_year . ' and dayplayed = "' . $dayplayed . '" and season = "' . $current_season . '" and team_grade = "' . $team_grade . '" and round = ' . (GetRounds($no_of_teams)+1);
        $sql_last = 'Select date from tbl_fixtures where year = ' . $current_year . ' and dayplayed = "' . $dayplayed . '" and season = "' . $current_season . '" and team_grade = "' . $team_grade . '" and round = ' . $no_of_rounds;
        echo($sql_last . "<br>");
        ...
        for($r = 0; $r < ($no_of_rounds+1); $r++)
        {
            ...
            if($r == ($no_of_rounds-1))
            {
                ...
                $no_of_fixtures = 2;
                $fixture_date = addDays($last_date, 7, 1);
            }
            elseif($r == $no_of_rounds)
            {
                $final_title = "Grand Final";
                $no_of_fixtures = 1;
                $fixture_date = addDays($last_date, 7, 2);
            }
            else
            {
                $final_title = "Round " . ($r+1);
                $no_of_fixtures = (($no_of_teams/2)-1);
                $fixture_date = date_format($date, 'Y-m-d');
            }
            ...
            echo("<td colspan=2 class='text-left'><input type='text' id='round" . $r . "_date'  value='" . $fixture_date . "' style='width:100px'></td></tr>");
        ...
        echo("<tr>");
        echo("<td align='center' colspan='3'><button type='button' class='btn btn-primary'  onclick='SaveFixtures()'>Save Fixtures for " . $team_grade . "</button></td>");
        echo("</tr>");
        echo("<tr>");
        echo("<td align='center' colspan='3'>&nbsp;</td>");
        echo("</tr>");
