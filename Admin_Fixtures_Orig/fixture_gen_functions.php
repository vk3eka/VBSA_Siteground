...
function addWeek($date, $days, $round) 
{
    ...
    $dateInterval = DateInterval::createFromDateString(($days*$round) . ' days');
    $outputDate = date_add( $date1, $dateInterval );
    $start_date = $outputDate -> format( 'Y-m-d' );
    return $start_date;
}
...
{
    echo show_fixtures(isset($teams) ?  nums(intval($teams)) : explode(", ", ($fixtures)), $team_grade, $form_no, $year, $season, $start_date);
    //echo show_fixtures(explode(", ", ($fixtures)), $team_grade, $form_no, $year, $season, $start_date);
}
...
function show_fixtures($names, $team_grade, $form_no, $year, $season, $start_date)
{
    ...
    elseif($teams == 6)
    {
        ...
        //for ($i = 0; $i < sizeof($rounds); $i++) {
            ...
            echo("<td colspan=3 align='center'><b>Round " . $round_counter  . "</b></td>");
            echo("<tr><td align='right'><b>Date</b></td>");
            echo("<td colspan=2 class='text-left'><input type='text' id='A_" . $form_no . "_date_" . ($round_counter-1) . "' style='width:100px' value=" . addWeek($start_date, 7, ($round_counter-1)) . "></td></tr>");
            $round_counter += 1;
            $y = 0;    
            foreach ($rounds[$b] as $r) {
                $round_data = explode(" v ", $r);
        ...
        for ($round_counter = 0; $round_counter < sizeof($rounds); $round_counter++) {
            ...
            echo("<tr><td align='right'><b>Date</b></td>");
            echo("<td colspan=2 class='text-left'><input type='text' id='A_" . $form_no . "_date_" . ($round_counter + 10) . "' style='width:100px' value=" . addWeek($start_date, 7, ($round_counter + 10)) . "></td></tr>");
            $x = 0;
            //$six_teams = [];
    ...
    //elseif(($teams == 8) || ($teams == 10) || ($teams == 4) || ($teams == 12))
    {
        ...
        for ($i = 0; $i < sizeof($rounds); $i++) {
            ...
            foreach ($rounds[$i] as $r) {
                ...
                echo("<tr>");
                echo("<td align='center'><input class='float-child' type='text' id='A_" . $team_grade . "_home_" . ($i+1) . "_" . ($x+1) . "' value='" . $round_data[0] . "' style='width:200px'></td>");
                echo("document.getElementById('BVS1_fix_1_pos_1').value = 'Test'");
                echo("<td align='center'>v</td>");
                echo("<td align='center'><input class='float-child' type='text' id='A_" . $team_grade . "_away_" . ($i+1) . "_" . ($x+1) . "' value='" . $round_data[1] . "' style='width:200px'></td>");
                echo("</tr>");
                $x++;
